...
| 3 | Version published flow definitions | `definition_json` is immutable but parsed as broad JSON in runtime while built in `FlowService`. | Add `domain/flow_definition.py` with `PublishedFlowDefinition`, parser/writer, `schema_version` policy, checksum input; add first-class `schema_version` column. | `flow_tables.py`, `flow_service.py`, `step_definition_parser.py`, `executor.py`, migrations | L | Medium-high | status lifecycle optional | Stored and new definitions can evolve without hidden JSON conventions. |
