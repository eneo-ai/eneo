...
| `warn-domain-any-dict` | Keep typed contracts meaningful. | New `Any`, `dict[str, Any]`, or broad JSON aliases in `backend/src/intric/flows/domain`, `application`, or `runtime` owned envelopes. | Arbitrary user/model output explicitly named `JsonValue` with owner and parser boundary. | `payload: dict[str, Any]` in a runtime command. | `PublishedFlowDefinitionV1` parser plus `freeform_output: JsonValue`. |
