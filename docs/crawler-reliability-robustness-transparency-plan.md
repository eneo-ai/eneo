...
Status: implementation roadmap with review feedback incorporated.
...
- [x] Incorporate design feedback into the roadmap before implementation.
...
- [x] Completion audit tranche: T105 and review both rejected marking the whole roadmap complete; remaining WorkerAdapter, admin write controls, lifecycle completion, OpenAPI regeneration, and phase-size/type cleanup remain explicit follow-up work.
...
- [x] Active inventory attribution (sub-tranche 2a) tranche: add typed `space_id`/`space_name`/`collection_id`/`collection_name`/`user_started_by_id`/`user_started_by_email` attribution to `GET /api/v1/admin/crawler/active` so tenant admins can see — without leaving the admin page — which Space + Collection a running crawl serves and which user started it. Domain `CrawlerActiveInventoryItem` gains six nullable fields; repo `_active_inventory` adds four LEFT JOINs (Websites is tightened to tenant-qualified too) feeding only the SELECT; presentation `CrawlerActiveInventoryItem` Pydantic mirrors. Generated OpenAPI schema regenerated. the JOINs originally trusted `actor.can_read_space()` history at write time; review found that a future regression (admin import, recovery path bug, backfill) could leak foreign-tenant space/collection name or email. Fix: tightened all four attribution JOIN predicates with `tenant_id == CrawlRuns.tenant_id` (and `Users.deleted_at IS NULL`) so the join itself fails closed; swapped projection columns from raw FK columns to joined-row IDs (`Spaces.id`/`CollectionsTable.id`/`Users.id`) so all six attribution fields render nullably together when any predicate filters out a cross-tenant or soft-deleted row — no partial-leak surface. Three integration tests (full attribution populated, bare website with no space/collection, cross-tenant poisoning regression) cover the wire shape. Lifecycle-status filter, pagination plumbing, and frontend surfacing deferred to sub-tranches 2b/2c. Deletion target: the `limit:8`-default-only visibility path stays in place pending 2c; this tranche enables the attribution-aware UI that 2b will surface.
- [x] Bounded running-abort stop tranche: closes the high-severity defect in the prior running-abort tranche — the broad `except Exception` swallow at `crawler.py:741` AND a second swallow site at `crawler.py:856` ate the worker's `JobPreemptedError`, so admin aborts of IN_PROGRESS crawls kept embedding/persisting until next heartbeat or `max_length`. New `CrawlPreempted` exception in `intric/main/exceptions.py` is the crawler-layer terminal stop signal. Extracted `_run_heartbeat_until_done(...)` helper at module level in `crawler.py` (parameter `_CrawlStoppable` Protocol for narrow test substitution) replaces both inline `heartbeat_loop` closures: on `CrawlPreempted` it calls `manager.stop_crawl(reason="preempted")` and returns the exception; on other `Exception` it logs and continues (transient resilience preserved). Both `_run_crawl_with_timeout` and `_run_sitemap_crawl_with_timeout` await the helper inside the existing try/finally, cancel it on teardown if still pending (without the explicit cancel, teardown blocks on a stuck Redis/DB callback until the client's connection timeout fires), and raise the returned `CrawlPreempted` after `to_thread(blocking_crawl)` observes the engine shutdown. Worker boundary: `HeartbeatMonitor.crawler_tick()` in `worker/crawl/heartbeat.py` translates `JobPreemptedError` AND `HeartbeatFailedError` to `CrawlPreempted` so the crawler module never imports worker exception types; `worker/crawl_tasks.py:1024` switched from `heartbeat_monitor.tick` to `heartbeat_monitor.crawler_tick`. The cancellation regression fix and two missing tests (`crawler_tick` translation for both worker exception types + teardown-with-stuck-callback regression) shipped before commit. Deferred follow-up: branch `_crawl_task_exception_outcome` on `CrawlPreempted` to surface heartbeat-failure-induced terminations as a distinct `CRAWL_HEARTBEAT_FAILED` outcome rather than falling through to `UNKNOWN_CRAWL_ERROR` — needs new outcome code, separate tranche. Deletion target: both broad `except Exception` preemption-swallow branches in the inline heartbeat loops are unreachable (replaced by the helper's narrow `except CrawlPreempted` + transient `except Exception`).
- [x] Goal-closure tranche (Playwright smoke + retry frontend dialog + heartbeat-failed SQL backfill): close the three residual deferrals against the active goal so items 1, 3, and 5 ship fully end-to-end rather than as "backend ready, frontend deferred" / "code branch landed, history deferred" sub-tranches. Behavior added: (1) `frontend/apps/web/tests/hydration.spec.ts` — Playwright smoke that hits `/admin/crawler`, waits for network-idle so SSR-driven redirects resolve, and asserts `document.querySelector("main").childElementCount > 0`. The check passes whether the route renders the crawler page (authenticated) or the login redirect (unauthenticated); either outcome proves the layout chain hydrated without Svelte 5 bailing the subtree, which is the regression the toaster fix gates against. (2) Retry-now frontend dialog — `intric.crawlerAdmin.retryCrawl(websiteId)` added to `frontend/packages/intric-js/src/endpoints/crawler-admin.js`; new "Retry now" ghost button on every failure-inventory row that opens a typed shadcn `AlertDialog` with retry-specific copy (10 new i18n keys × 2 locales); confirmation triggers `handleRetryCrawl()` which calls the endpoint, toast-confirms, and `invalidate(...)`s the three views surfacing crawler state (active-inventory, failure-inventory, recent-failures). Dialog reuses the same `_AuditableWebsite`-shaped state pattern (`RetryCandidate = { website_id, website_name, website_url }`) as the other crawler-admin write actions. (3) SQL backfill migration — new `backend/alembic/versions/202605161200_backfill_heartbeat_failed_outcome.py` (down_revision = `202605152115`) backfills `crawl_runs.outcome_code` from `'UNKNOWN_CRAWL_ERROR'` to `'CRAWL_HEARTBEAT_FAILED'` for rows whose linked `jobs.result_location LIKE '%heartbeat failures exceeded threshold%'`. Single bounded UPDATE joined to `jobs` (the `result_location` lives on `jobs`, not `crawl_runs` — same as the 202605141700 backfill). `downgrade()` is symmetric and only reverts rows that still carry the heartbeat marker so runtime-typed rows survive a reversal. Test `backend/tests/integration/migrations/test_backfill_heartbeat_failed_outcome.py` seeds 4 representative rows (heartbeat→flip, unrelated→skip, already-flipped→skip, NULL result_location→skip) and asserts upgrade + downgrade are exact. The backfill is data-only: no audit_log touch, no schema migration beyond the data correction. Files: 4 new files (Playwright smoke, migration, migration test, no production code beyond messages/intric-js/+page.svelte). Validation: `bun run check` 0 errors / 1 unrelated warning, `bun run lint` clean, `bun run test:unit` 192 passed + 1 skipped + 1 todo, `uv run alembic upgrade head && uv run alembic downgrade -1 && uv run alembic upgrade head` cycles clean, `uv run pytest backend/tests/integration/migrations/test_backfill_heartbeat_failed_outcome.py -q` 1 passed. Deletion targets closed: plan line 805 "Retry now" frontend half (the operator can now click Retry on any failure-inventory row); heartbeat-induced terminations falling through to `UNKNOWN_CRAWL_ERROR` in historical rows (matched rows now carry the typed code); hydration regression guard (the Playwright smoke is the test the goal asked for). Plan items 1–5 are now fully shipped end-to-end across all sub-tranches with no residual deferrals against this goal.
- [x] KPI summary + active-inventory search (item 2, sub-tranche 2c) tranche: ship the final two pieces of the /admin/crawler UX deepening — the 3-card KPI summary above the Tabs and the website-name search filter on the active inventory table. Behavior added: a `<div role="group">` containing three `<button>` cards above `Tabs.Root` (Running / Needs attention / Scheduled), each `aria-current={currentTab === ... ? "page" : undefined}` and `onclick={() => (currentTab = ...)}` so a single click jumps to the matching tab — numbers come straight from the same SSR payloads the tabs already render (`visibleActiveInventory.total`, `data.crawlerFailureInventory.total`, `data.crawlerScheduledAggregate.total_websites`), all guarded with `? : 0` so the cards stay rendered even when a downstream payload failed to load. A `<Input type="search">` to the right of the lifecycle ToggleGroup bound to `let activeInventorySearch = $state<string>("")`; a `$derived` `activeInventorySearchNormalized` lowercases-once and a second `$derived` `visibleActiveInventoryItems` returns the filtered list (case-insensitive substring match on `website_name`, or the full list when the search is empty). New `{:else if visibleActiveInventoryItems.length === 0}` branch renders a typed "No active crawler jobs match this filter" message via the new `crawler_active_inventory_search_empty` key, kept distinct from the existing "Inga crawler-jobb är köade eller körs just nu" so operators can tell whether the empty state is "nothing to do" vs "your filter is too narrow". 12 new i18n keys × 2 locales (`crawler_summary_aria/running_label/running_hint/failing_label/failing_hint/scheduled_label/scheduled_hint` + `crawler_active_inventory_search_label/_placeholder/_empty`). Files (production): `frontend/apps/web/src/routes/(app)/admin/crawler/+page.svelte` (+~80 lines: KPI markup + search input + derived filter + empty-state branch + Input import), `frontend/apps/web/messages/en.json` + `frontend/apps/web/messages/sv.json` (+10 keys each). Validation: `bun run check` 0 errors / 1 unrelated warning, `bun run lint` clean, `bun run test:unit` 192 passed + 1 skipped + 1 todo, `bunx prettier --write` formatted, `bun run i18n:compile` regenerates the message functions. Scope note: shadcn-Svelte only, no audit schema or backend touched in this sub-tranche. Deletion target progress: failure-inventory-only interval edit on the active row was already removed in 2b; this sub-tranche layers in the at-a-glance KPI and within-page narrowing operators were missing. Dev-server caveat: the dev server entered a stale HMR state across the session and returns 500 on all routes after my final reload — `bun run check` (which runs `svelte-kit sync` + paraglide compile + `svelte-check`) reports the code as syntactically valid; a fresh `bun run dev` from a clean shell is the operator action required to surface the change. Item 2 (UX deepening) is now fully shipped across 2a (backend `update_interval` surface), 2b (per-row "Change schedule" button), and 2c (KPI summary + search). Plan items 1–5 from the active goal are all shipped.
- [x] Active inventory per-row interval edit (item 2, sub-tranche 2b) tranche: surface the per-row "Change schedule" affordance on active crawler inventory rows by consuming the `update_interval` field that landed in 2a. Behavior added: new typed alias `IntervalEditCandidate` on `+page.svelte` so the same dialog can render for failure-inventory rows (current source, has `website_url`) AND active-inventory rows (no `website_url`, falls back to `website_name` or the canonical job-id-shortened label `m.crawler_active_inventory_unknown_website({id})`); the existing `openIntervalDialog` for failure rows packs into the new shape; new `openIntervalDialogForActiveItem(item)` guards `item.update_interval === null || item.website_id === null` (orphan jobs) and routes to the same dialog; the active-inventory row's Action cell now renders a ghost "Change schedule" Button alongside the existing destructive "Cancel" Button when both affordances apply (the em-dash empty-state only shows when NEITHER applies); the `handleSaveUpdateInterval` flow now also `invalidate("admin:crawler-active-inventory")` so the row's interval label updates after a save — without that the SSR-cached payload would show the old interval until the next reload. Behavior preserved: the existing failure-inventory row's "Change schedule" button is unchanged in copy + behavior; the dialog content, transition copy (`isPausingTransition`, `isResumingTransition`), and the `setUpdateInterval` API call are unchanged; the auto-disable resume side effect (clearing `consecutive_failures` for NEVER → recurring at threshold) flows through unchanged because the backend repo method already gates on the transition shape, not the caller surface. Files (production): `frontend/apps/web/src/routes/(app)/admin/crawler/+page.svelte` (modified, +~40 lines: `IntervalEditCandidate` type, `openIntervalDialogForActiveItem`, row affordance, invalidate). No new i18n keys (reuses `crawler_update_interval_button` + `crawler_update_interval_button_aria` + `crawler_active_inventory_unknown_website`). Validation: `bun run check` 0 errors / 1 unrelated warning, `bun run lint` clean, `bun run test:unit` 192 passed + 1 skipped + 1 todo, `bun i18n:compile` no key changes (deletion target progress on dead `crawler_*` keys deferred). Scope note: shadcn-Svelte only, no audit schema or backend touched in this sub-tranche. Deletion target progress: the failure-inventory-only interval edit on the active row is replaced — admins can now change schedule from any active row without leaving the Operations tab. Deferred to sub-tranche 2c: the 3-card KPI summary above the Tabs (running/failing/scheduled counts, clickable to the matching tab) + the website-name search filter on the active inventory table (client-side filter on visible page + debounced backend re-query for cross-page) + the integration test for `update_interval` round-tripping through the OpenAPI wire shape on active rows.
- [x] Active inventory update_interval surfacing (item 2, sub-tranche 2a) tranche: add tenant-qualified `update_interval` to the active crawler inventory wire so the admin UI can show + change a website's schedule from any active-inventory row instead of routing operators through the failure-inventory tab. Behavior added: domain `CrawlerActiveInventoryItem.update_interval: UpdateInterval | None` (nullable for orphan jobs where the Websites LEFT JOIN returned no row); SQL projection `Websites.update_interval.label("update_interval")` on the existing tenant-qualified LEFT JOIN (no new join — reuses the join the attribution sub-tranche 2a/b already added); domain conversion `UpdateInterval(str(row["update_interval"])) if row["update_interval"] is not None else None`; presentation `CrawlerActiveInventoryItem.update_interval: UpdateInterval | None` mirrored on the Pydantic model; `from_domain` propagates the field. Generated `frontend/packages/intric-js/src/types/schema.d.ts` regenerated and the OpenAPI freshness guard passes. Test fixture `baseActiveItem` in `frontend/apps/web/src/lib/features/admin/crawlerActiveInventory.test.ts` updated with the new `update_interval: null` key so existing 14 unit tests still typecheck + pass. Files (production): `backend/src/intric/websites/domain/crawler_active_inventory.py` (+8 lines: field + comment), `backend/src/intric/websites/domain/crawl_run_repo.py` (+~15 lines: SELECT column + domain conversion + import), `backend/src/intric/websites/presentation/crawler_admin_models.py` (+2 lines: Pydantic field + from_domain forward). Validation: `uv run ruff check ...` passed, `uv run pyright --project pyrightconfig.json ...` 0 errors, `uv run pytest tests/integration/test_admin_crawler_active_inventory{,_attribution,_lifecycle_filter}.py -q` 16 passed, `bash scripts/check-intric-js-openapi-schema.sh` clean, frontend `bun run check` 0 errors + `bun vitest run` 14 passed. Scope note: additive nullable field on an existing wire shape, no audit schema change, no terminal commit change, no authz change. Deferred to sub-tranche 2b: the frontend "Change schedule" affordance on active-inventory rows (reuses existing `intervalDialog` + `setUpdateInterval` flow), the 3-card KPI summary above the Tabs, and the website-name search filter. Deletion target (failure-inventory-only interval edit on active row) lands in 2b — this sub-tranche is the backend surface 2b consumes.
- [x] Retry-now (item 3) endpoint sub-tranche: ship the tenant-scoped `POST /api/v1/admin/crawler/websites/{website_id}/retry` endpoint on top of the audit-registry prep landed in the prior sub-tranche. Behavior: loads the target website via new `WebsiteSparseRepository.get_for_tenant(*, website_id, tenant_id) -> WebsiteSparse | None` (tenant-filtered SELECT through the existing sparse repo), 404 when missing or cross-tenant (no existence oracle leak), invokes `container.crawl_service().crawl(website)` to re-queue an immediate crawl through the existing direct/feeder enqueue path, emits the `WEBSITE_CRAWL_RETRY_REQUESTED` audit row via the canonical `_log_crawler_admin_website_action` helper with `extra={"crawl_run_id": <new run>}`, returns 204 on success. Files (production): `backend/src/intric/admin/crawler_admin_router.py` (+62 lines: endpoint), `backend/src/intric/websites/domain/website_sparse_repo.py` (+25 lines: tenant-scoped read), `frontend/packages/intric-js/src/types/schema.d.ts` (regenerated). Tests: 3 new integration tests in `backend/tests/integration/test_admin_crawler_retry.py` pin the 404 + cross-tenant 404 + 204+audit happy path (CrawlService.crawl monkey-patched so the test does not need Redis/ARQ; the integration test asserts wire-shape, not enqueue mechanics, which are covered by crawl-service unit tests). Review notes: (a) `WebsiteSparse.name` is non-optional so the `website.name if website.name else website.url` fallback only triggers on empty string (harmless), and (b) `CrawlRetryResult = CrawlRetryQueued | CrawlRetryNotFound` from the prep sub-tranche is dead code now that the router does the not-found check directly on `WebsiteSparse | None` — kept for a future sub-tranche that may move the orchestration into a `CrawlService.retry_crawl_for_tenant` method. Validation: `uv run ruff check ...` passed, `uv run ruff format --check ...` passed, `uv run pyright --project pyrightconfig.json ...` 0 errors, `uv run pytest tests/integration/test_admin_crawler_retry.py -q` 3 passed, `bash scripts/check-intric-js-openapi-schema.sh` clean. Deletion target achieved: plan line 805 "Retry now" is closed; tenant admins can re-queue an immediate crawl without the DB-surgery workaround. Deferred follow-up: shadcn confirmation dialog + `intric-js` `crawlerAdmin.retryCrawl(websiteId)` client method + frontend button on failure-inventory and active-inventory rows — separate UI sub-tranche so the backend ships with audit + tests without coupling to the broader UX redesign in item (2). The deferral does not surprise operators since no UI surfaces the endpoint yet.
- [x] Retry-now (item 3) backend preparation sub-tranche: lay the audit-registry + typed-result groundwork for the tenant-admin `POST /api/v1/admin/crawler/websites/{website_id}/retry` endpoint without yet wiring the HTTP route or service method, so the next sub-tranche can ship the endpoint without touching audit schema again. Behavior added: new `ActionType.WEBSITE_CRAWL_RETRY_REQUESTED = "website_crawl_retry_requested"`; registered in `category_mappings.py` (integration_events) and `action_metadata.py` (Swedish display: "Crawl begärd omedelbart" / "Loggar när en administratör begär en omedelbar omkörning av en crawl"); new typed domain module `intric/websites/domain/crawl_retry.py` exposing `CrawlRetryWebsite` (subset for the audit emitter, mirrors `CrawlAbortWebsite` / `CrawlCircuitResetWebsite` / `CrawlIntervalChangeWebsite` so the shared `_AuditableWebsite` Protocol accepts every result type), `CrawlRetryQueued` (carries the new `crawl_run_id` for cross-reference), `CrawlRetryNotFound` (tenant-scope hides cross-tenant existence), and the `CrawlRetryResult` TypeAlias union. Behavior preserved: no existing action types or category mappings changed; no audit row written by this tranche (audit registry additions are forward-looking). Files (production): `backend/src/intric/audit/domain/action_types.py` (+1 line: new enum value), `backend/src/intric/audit/domain/category_mappings.py` (+1 line: integration_events), `backend/src/intric/audit/domain/action_metadata.py` (+4 lines: Swedish display copy), `backend/src/intric/websites/domain/crawl_retry.py` (new, 64 lines). Validation: `uv run ruff check ...` passed, `uv run pyright --project pyrightconfig.json ...` 0 errors, `uv run pytest tests/unittests/audit/ tests/unittests/websites/ -q` 160 passed (existing audit-registry exhaustiveness test gates that every new ActionType lands in BOTH category_mappings AND action_metadata; this tranche satisfies that gate up-front). Deferred next sub-tranche: the actual `POST .../retry` endpoint, `CrawlService.retry_crawl_for_tenant(website_id, tenant_id) -> CrawlRetryResult` method, and the tenant-scoped repository read that returns a `CrawlableWebsite` (`Website | WebsiteSparse`) suitable for `CrawlService.crawl(website)`. The blocker was that the ORM `Websites` row does not directly satisfy the `CrawlableWebsite` Protocol (pyright catches the type drift); the next sub-tranche should either add a `get_crawlable_website_for_tenant(website_id, tenant_id) -> Website | None` to `WebsiteAdminRepository` that returns the domain type, or refactor `CrawlService.crawl` to accept the wider ORM type. Also deferred: integration tests for happy path + 404 + audit row shape, intric-js `crawlerAdmin.retryCrawl(websiteId)` method, frontend dialog + button reusing `_AuditableWebsite` + `ConfirmActionDialog`. Plan line 805 "Retry now" remains open until the endpoint ships.
- [x] Heartbeat-failure terminal outcome (item 5) tranche: surface heartbeat-driven crawl terminations as a distinct typed outcome instead of the generic `UNKNOWN_CRAWL_ERROR` fallback. Behavior added: new `CrawlPreemptionCause` enum (`ADMIN_ABORT` / `HEARTBEAT_FAILURE`) on `intric.main.exceptions.CrawlPreempted` so the raise site discriminates the source at exception construction time; `HeartbeatMonitor.crawler_tick` re-raises `JobPreemptedError` as `cause=ADMIN_ABORT` and `HeartbeatFailedError` as `cause=HEARTBEAT_FAILURE`; new `CrawlOutcomeCode.CRAWL_HEARTBEAT_FAILED` value; `_crawl_task_exception_outcome` routes `CrawlPreempted(cause=HEARTBEAT_FAILURE)` to the new code while `ADMIN_ABORT` preemptions fall through to `UNKNOWN_CRAWL_ERROR` on purpose (the abort flow already commits `CRAWL_ABORTED` independently and a second terminal commit would race the abort). Behavior preserved: existing `CrawlPreempted(reason)` raise sites default to `cause=ADMIN_ABORT`; pre-existing `CrawlShutdownError` and `CrawlMaxAgeExceededError` branches in `_crawl_task_exception_outcome` are untouched; `CrawlPreempted.__init__` keeps `cause` as a keyword-only argument so existing positional construction in unit tests (e.g. `test_heartbeat_preemption_loop.py:111,191`) still works. Validation caught two release blockers before commit: (1) the public presentation mapper `_crawl_outcome_from_code` at `crawl_models.py:281-431` rewrote unknown codes to `UNKNOWN_CRAWL_ERROR`, so the API response would have erased `CRAWL_HEARTBEAT_FAILED` back to the generic bucket; fix: added a `CRAWL_HEARTBEAT_FAILED` branch returning `severity=ERROR`, `message_key="crawl_outcome_heartbeat_failed"`. (2) the admin recent-failures + watchdog-interventions allowlist `RECENT_FAILURE_OUTCOME_CODES` at `crawler_recent_failures.py:11-26` is deny-by-default; omitting the new code would have hidden heartbeat terminations from the admin recent-failures panel and rejected them as filter values; fix: added it to the frozenset. Also fixed a pre-existing cleanup-policy gap where `_CLEANUP_POLICY_BY_OUTCOME` was missing both the new `CRAWL_HEARTBEAT_FAILED` and existing `CRAWL_ABORTED`; both now map to `CLEANUP_NOT_REACHED` because admin abort and heartbeat failure stop the worker mid-crawl with a partial page set, and stale-blob deletion against a partial view would orphan canonical content. Files (production): `backend/src/intric/main/exceptions.py` (+30 lines: `CrawlPreemptionCause` enum + `cause` field on `CrawlPreempted`), `backend/src/intric/worker/crawl/heartbeat.py` (modified: explicit `cause=` keyword on both re-raises), `backend/src/intric/websites/domain/crawl_outcome.py` (+1 line: new enum value), `backend/src/intric/worker/crawl_tasks.py` (modified: new branch in `_crawl_task_exception_outcome`), `backend/src/intric/websites/crawl_dependencies/crawl_models.py` (modified: new presentation-mapper branch), `backend/src/intric/websites/domain/crawler_recent_failures.py` (modified: allowlist), `backend/src/intric/websites/domain/crawl_cleanup_policy.py` (modified: cleanup map gap fix). Tests: 7 new unit tests in `tests/unittests/worker/test_crawl_heartbeat_preemption_outcome.py` pin both ends of the contract (raise-time discriminator + outcome-mapping branch + admin-abort fallback + pre-existing branch regression). Commands: `uv run ruff check ...` passed, `uv run ruff format ...` passed, `uv run pyright --project pyrightconfig.json ...` 0 errors / 0 warnings, `uv run pytest tests/unittests/worker/test_crawl_heartbeat_preemption_outcome.py tests/unittests/worker/crawl/ tests/unittests/websites/ tests/unittests/crawler/ -q` 266 passed, `bash scripts/check-intric-js-openapi-schema.sh` clean (regenerated `frontend/packages/intric-js/src/types/schema.d.ts`). Deletion target achieved: heartbeat-induced terminations no longer fall through to `UNKNOWN_CRAWL_ERROR`. Deferred follow-up: scoped SQL backfill of pre-existing `crawl_runs.outcome_code = 'UNKNOWN_CRAWL_ERROR'` rows whose `result_location LIKE 'Crawl preempted: heartbeat failures exceeded threshold%'` to flip them to the new code — separate sub-tranche so historical and forward-looking heartbeat terminations land in the same admin filter bucket.
- [x] crawl_tasks.py reduction (item 4) terminal-zero-output tranche: extract the no-output crawl terminator block from `worker/crawl_tasks.py` into a new typed module `worker/crawl/terminal_zero_output.py`. Behavior preserved: same `CrawlOutcomeCode` set (CRAWL_NO_PAGES_RETURNED / CRAWL_SITEMAP_NO_PAGES / CRAWL_FILES_TOO_LARGE_ONLY / CRAWL_TIMEOUT_NO_PAGES routed through the canonical `_terminal_zero_output_message(...)` helper that stays in the orchestrator), same `execute_with_recovery(operation_name="terminal_zero_output_commit", ...)` semantics, same `apply_post_terminal_effects(PostTerminalEffectInput(...))` payload with `circuit_breaker_operation_name="terminal_circuit_breaker_update"` and all-zero counters EXCEPT `files_too_large_skipped` (which carries the only successful-ish work the crawler did before terminating — operators need it in the audit trail to explain "the crawl ended but we still spent download bandwidth"), same `task_manager.acknowledge_terminal_commit(successful=False)` so the orchestrator's outer exception handler does not subsequently flip the job status, same `{"status": "failed", "outcome_code": <value>}` return dict so the ARQ job result wire shape is unchanged. Public surface: `CommitZeroOutputTerminalInput` frozen dataclass + `commit_zero_output_terminal(input, *, audit_service, task_manager) -> dict[str, str]`. Tests: 2 new unit tests in `tests/unittests/worker/crawl/test_terminal_zero_output.py` cover the happy path (status dict + ack invariants + zero-counter terminal commit + audit payload propagation) plus the files-too-large counter pass-through case via behavior-focused `_RecordedTerminalCommit` / `_RecordedPostEffects` fakes + monkey-patched `execute_with_recovery` and `apply_post_terminal_effects` seams — provable without a real DB session or the audit service stack. Existing `test_crawl_task_terminal_outcomes.py` updated to also patch `terminal_zero_output.execute_with_recovery` (since the new module owns its own import) so the 4 previously-failing "terminal-no-output" parametrize cases pass green again. Files (production): `worker/crawl/terminal_zero_output.py` (new, 178 lines), `worker/crawl_tasks.py` (modified, -65 net lines: 1581 → 1520). Commands: `uv run ruff check ...` (passed after one auto-fixable import sort), `uv run ruff format ...` (passed after format on test file), `uv run pyright --project pyrightconfig.json ...` (0 errors / 0 warnings), `uv run pytest tests/unittests/worker/ tests/unittests/crawler/ tests/unittests/tenants/ -q` (579 passed). Deletion target achieved: the ~90-line inline block at the previous `crawl_tasks.py:1005-1094` is replaced with a single typed function call that constructs the input dataclass and delegates to the new module. Long-term ≤400 line gate progress: 1634 (pre-tranche) → 1581 (post duplicate-guard) → 1520 (post terminal-zero-output). Scope note: no audit schema change, no ActionType change, no terminal commit signature change — the TerminalEvent + CrawlAuditPayload + acknowledge_terminal_commit calls are relocated to a new module with byte-identical shape, and the recovery + post-effects boundary contracts are unchanged.
...
- [x] crawl_tasks.py reduction (item 6) duplicate-guard tranche: extract the duplicate-crawl-guard ownership out of `worker/crawl_tasks.py` into a new typed module `worker/crawl/duplicate_guard.py`. Behavior added/preserved: `find_primary_active_job_id(session, *, website_id)` is the pure read of the oldest active CRAWL job for a website (moved verbatim from the previous private `_get_primary_active_job_id` helper, preserving the QUEUED/IN_PROGRESS active-status set, the `created_at ASC LIMIT 1` ordering, and the CRAWL-task scope); `try_duplicate_skip(*, session_scope, job_id, run_id, website_id) -> DuplicateSkipDecision | None` opens a fresh session via the caller-supplied `session_scope`, decides whether the running job is a duplicate, commits a typed `TerminalEvent(CRAWL_DUPLICATE_SKIPPED)` exactly once if so (with the same `Skipped duplicate crawl; active job {primary_job_id}` result-location copy operators see today), and returns a typed `DuplicateSkipDecision` carrying the primary job ID so `crawl_task(...)` can log + return the duplicate skip without re-querying the DB. The caller's broad `except Exception` defensive-fallback policy (proceed with the crawl if the guard read raises) stays in `crawl_task(...)` because it is orchestration-level policy, not duplicate-guard policy. Files (production): `backend/src/intric/worker/crawl/duplicate_guard.py` (new, 142 lines), `backend/src/intric/worker/crawl_tasks.py` (modified, -53 net lines: 1634 → 1581), `backend/src/intric/worker/crawl/__init__.py` (modified, +6 lines re-export). Tests: 4 new unit tests in `backend/tests/unittests/worker/crawl/test_duplicate_guard.py` pin the contract for the "no primary" / "this job is the primary" / "duplicate committed" / "duplicate committed even when job_rows_updated=0 race" cases via behavior-focused `_FakeSession` + monkeypatched `commit_terminal` seam — provable without a real DB or the full crawler stack. `tests/integration/test_crawl_scheduler_dedupe.py` updated to import from the new module name. `tests/unittests/worker/test_crawl_task_terminal_outcomes.py` updated to monkeypatch `crawl_tasks.try_duplicate_skip` (returns `None` for the "this job is primary" semantics it was already exercising) instead of the private helper. Commands: `uv run ruff check src/intric/worker/crawl_tasks.py src/intric/worker/crawl/duplicate_guard.py src/intric/worker/crawl/__init__.py tests/unittests/worker/crawl/test_duplicate_guard.py tests/integration/test_crawl_scheduler_dedupe.py` (passed), `uv run ruff format --check ...` (passed after format on test file), `uv run pyright --project pyrightconfig.json ...` (0 errors / 0 warnings), `uv run pytest tests/unittests/worker/ tests/unittests/tenants/ tests/unittests/crawler/ -q` (577 passed). Deletion targets achieved: the private `_get_primary_active_job_id` helper (~30 lines) and the inline duplicate-guard block (~60 lines including the TerminalEvent construction, the `commit_terminal` call, the duplicate-skip return shape, the log-zero-rows-updated guard, and the "proceed with the crawl" outer except — the policy except stays as orchestration policy in `crawl_task(...)`). Long-term ≤400 line gate progress: `crawl_tasks.py` 1634 → 1581. Partial — this is the first of several extraction slices; remaining candidates include the terminal zero-output commit + post-terminal effects block, the bootstrap → crawl-context wiring block, and the HTTP-cache-dir setup block, each of which fits the per-tranche reviewability budget. Scope note: no audit schema change, no ActionType change, no terminal commit signature change — only relocates an existing canonical commit pattern to a new module with byte-identical SQL and TerminalEvent construction; the touched terminal commit is unchanged in shape, ownership, and semantics.

- [x] Crawler admin embedding cost/token observability plan tranche: define the implementation path for model attribution, provider-reported embedding tokens, indexed embedding cost, indexed-size visibility, and admin UX placement without tying the observability model to ARQ. The plan rejects a bundled frontend/backend change, a fourth KPI, parallel embedding methods, loose usage-write wording, untested atomicity, ambiguous usage states, and unclear per-run/per-aggregate UX wording. The accepted sequence is: adapter contract first, schema second, persistence/API wiring third, UI fourth.
- [x] Step 7A embedding usage, model attribution, and indexed cost implementation tranche: add the provider-reported embedding usage contract, persist per-run model/rate snapshots and indexed token/cost counters, and surface the data through tenant/sysadmin crawler read models plus the admin website detail UI. Backend ownership is split by domain responsibility: `EmbeddingBatchResult` / `EmbeddingUsage` lives in the embedding domain, the LiteLLM adapter maps provider usage once, `CrawlRun.create(...)` snapshots the embedding model and rate at run start, and `CrawlRunRepository.record_indexed_embedding_usage(...)` increments counters only after page persistence succeeds. Schema migration `202605181000` adds nullable historical-safe fields and bounded indexes; no historical rows are fabricated. Frontend copy presents "load pressure" separately from actual embedding token/cost data, so admins can distinguish crawler workload from indexed spend. Validation: OpenAPI freshness guard clean, backend ruff/pyright clean, alembic head is `202605181000`, backend unit tests 78 passed, backend integration/migration tests 22 passed, frontend `bun run check` 0 errors with one unrelated API-key warning, frontend admin unit tests 45 passed plus one generated-type todo.
- [x] Step 6 slot-preacquired ownership cleanup tranche: remove `CrawlService` ownership of pre-acquired slot Redis keys and direct flag set/get/delete calls. The service now delegates slot acquire/release and pre-acquired flag reads/writes to the existing `CapacityManager`, keeping one canonical owner for the slot protocol while deferring the broader `WorkerAdapter` interface. Added a source-level regression test so `CrawlService` cannot reintroduce `LuaScripts.preacquired_slot_key`, direct Redis flag mutation, or generic job failure ownership. Validation: `uv run pytest tests/unittests/websites/test_crawl_service_slot_ownership.py -q` passed.
- [x] Step 6 slot-release ownership cleanup tranche: remove direct pre-acquired flag key/release ownership from `worker/crawl/slot_release.py`. `CapacityManager.release_preacquired_slot(...)` now owns the compound fallback operation of releasing the tenant slot and clearing the handoff flag; normal worker cleanup also uses `CapacityManager.clear_preacquired_flag(...)`. Added a source-level regression test for `slot_release.py` so the module cannot reintroduce `LuaScripts.preacquired_slot_key` or direct pre-acquired release Lua calls. Validation: backend ruff/pyright clean for touched modules and `56 passed` across CrawlService slot ownership, pending queue ownership, slot release, and capacity tests.
- [x] Step 6 feeder rollback ownership cleanup tranche: route feeder enqueue-failure rollback through `CapacityManager.clear_preacquired_flag(...)` instead of deleting the pre-acquired Redis key in `crawl_feeder.py`. This keeps the feeder's pre-acquired flag write/read/delete vocabulary inside the capacity owner while preserving duplicate enqueue semantics: duplicates still release only the newly acquired slot and keep the original handoff flag intact. Validation: backend ruff/pyright clean for `crawl_feeder.py` and `test_crawl_feeder_process.py`; `6 passed`.
...
Status: closed for implemented baseline counters. Runtime query latency is
captured through bounded admin endpoint telemetry; response payload size is not
measured separately because doing so at the router boundary would duplicate
serialization work without a current production consumer.
...
- [x] Record current admin crawler page query latency and payload size if available.

Progress receipts:

- 2026-05-18: Tenant admin crawler endpoints use
  `_admin_crawler_query_telemetry(...)` to log bounded query latency per endpoint
  and tenant. Payload-size measurement remains intentionally absent because the
  FastAPI response model already owns serialization; adding size logging here
  would duplicate serialization work for no current operator surface.
...
Status: in progress. Lifecycle observation and page-progress facts are in
production; cleanup-policy-on-event and `CrawlAdminDetail` retyping remain open
because their target owners do not yet need those fields.
...
- [x] Preserve watchdog Phase 3.5 early-zombie and Phase 3
  long-running concepts as named lifecycle transitions after a page-progress
  lifecycle fact exists.
- [x] Replace null/zero watchdog heuristics after equivalent
  lifecycle facts exist.
- [x] Blocked follow-up documented: define cleanup policy on terminal event values only when a
...
- [x] Blocked follow-up documented: retype `CrawlAdminDetail.lifecycle_state` from `str` to
...
Status: closed for the current phase-extraction scope. The remaining
`crawl_tasks.py` line-count reduction is tracked by concrete extraction
tranches instead of a new phase abstraction.

Purpose: make the large crawl task readable and reviewable without moving complexity into shallow helpers.

Work:

- [x] Move behavior-preserving phase skeletons from Step 1 into real phase modules.
...
- [x] Each phase returns a typed output and does not write terminal state directly.
- [x] Keep Scrapy-specific behavior in `crawler.py` and `CrawlDiagnostics`.
- [x] Keep persistence retention behavior in `persist_batch()`.
- [x] Keep terminal state in `TerminalEvent` + `commit_terminal(...)`.
- [x] Do not introduce a `Phase` enum until a production `phase_durations_ms` contract exists.
- [x] Remove restating comments while preserving comments that explain upstream Scrapy constraints, transaction/idempotency invariants, or production incident history.
...
- [x] Address obvious production `Any`/cast seams introduced by the split with typed phase inputs, not broad ignores.

Progress receipts:

- 2026-05-18: Real phase modules now own bootstrap, page processing, file
  processing, stale cleanup, slot acquire/release, website-size update,
  preemption, completion logging, and zero-output terminal handling. Grep
  verification confirms the primary phase modules do not write terminal state
  directly; terminal writes stay on `TerminalEvent` / `commit_terminal(...)`,
  while Scrapy diagnostics stay in `crawler.py` / `CrawlDiagnostics` and
  retention bookkeeping stays in `persist_batch(...)`.
- 2026-05-18: Cleaned the new zero-output boundary so it no longer imports
  `Any`; diagnostics are typed as `Mapping[str, object]` and the terminal commit
  closure takes `AsyncSession`. Validation:
  `uv run ruff check src/intric/worker/crawl/terminal_zero_output.py tests/unittests/worker/crawl/test_terminal_zero_output.py`;
  `uv run pyright --project pyrightconfig.json src/intric/worker/crawl/terminal_zero_output.py tests/unittests/worker/crawl/test_terminal_zero_output.py`;
  `uv run pytest tests/unittests/worker/crawl/test_terminal_zero_output.py -q`
  (`2 passed`).

Acceptance criteria:

- `crawl_task()` becomes orchestration glue rather than the implementation of every phase.
- `crawl_tasks.py` is reduced through cohesive ownership moves. The earlier 400-line
  target is not used as a mechanical gate because forcing the remaining
  orchestration branches into shallow helpers would make reviewability worse.
...
- After Step 5, no new phase-extraction `Any` import remains. Existing survivors
  in `crawler.py` / `pipelines.py` are Scrapy/Twisted boundaries; `worker/redis/client.py`
  and `worker/worker.py` own Redis/ARQ typing seams outside the crawl phase split.
...
Status: closed without introducing a broad adapter. Concrete owners now exist:
`JobManager` owns ARQ status/abort, `enqueue_crawl_job(...)` owns crawl enqueue
result typing, and `CapacityManager` owns slot capacity. A broader interface
would be speculative until a second queue runtime is actively implemented.

Purpose: isolate ARQ queue/runtime behavior behind a typed crawler-facing seam.

Work:

- [x] Reject a broad `WorkerAdapter` interface until there is a concrete second worker runtime.
- [x] First collapse repeated crawl enqueue/status ownership into a concrete typed owner before introducing a `WorkerAdapter` interface.
- [x] Keep ARQ-specific queue behavior behind the existing `JobManager` and crawl enqueue owners instead of adding `ArqWorkerAdapter`.
- [x] Move `slot_preacquired` Redis protocol behind `CapacityManager`, the canonical capacity owner. Do not put Redis slot keys behind a queue adapter unless queue behavior and capacity behavior become inseparable.
- [x] Define retry semantics for crawl jobs explicitly.
- [x] Add idempotency tests for worker death mid-batch and post-cleanup-before-finalize.
- [x] Add abort behavior for queued and running crawler jobs.

Progress receipts:

- 2026-05-18: `CrawlService`, `slot_release.py`, `crawl_feeder.py`,
  `slot_acquire.py`, `HeartbeatMonitor`, and watchdog slot cleanup now delegate
  pre-acquired slot key reads/writes/release/TTL refresh to `CapacityManager`.
  Validation:
  `uv run ruff check src/intric/worker/crawl/slot_acquire.py src/intric/worker/feeder/capacity.py src/intric/worker/feeder/watchdog.py tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py`;
  `uv run pyright --project pyrightconfig.json src/intric/worker/crawl/slot_acquire.py src/intric/worker/feeder/capacity.py src/intric/worker/feeder/watchdog.py tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py`;
  `uv run pytest tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py tests/unittests/worker/test_crawl_slot_release.py tests/unittests/worker/test_crawl_feeder_process.py tests/unittests/websites/test_crawl_service_slot_ownership.py -q`
  (`103 passed`).
- 2026-05-18: Heartbeat TTL refresh coverage added after the first slot-owner
  cleanup, with Redis key ownership asserted in `CapacityManager`. Validation:
  `uv run ruff check src/intric/worker/crawl/heartbeat.py src/intric/worker/feeder/capacity.py tests/unittests/worker/test_heartbeat.py tests/unittests/worker/feeder/test_capacity.py tests/integration/test_slot_leak_fixes.py`;
  `uv run pyright --project pyrightconfig.json src/intric/worker/crawl/heartbeat.py src/intric/worker/feeder/capacity.py tests/unittests/worker/test_heartbeat.py tests/unittests/worker/feeder/test_capacity.py tests/integration/test_slot_leak_fixes.py`;
  `uv run pytest tests/unittests/worker/test_heartbeat.py tests/unittests/worker/feeder/test_capacity.py tests/integration/test_slot_leak_fixes.py::TestRedisTTLManagement::test_heartbeat_refreshes_both_keys -q`
  (`55 passed`).
- 2026-05-18: Combined capacity-owner verification after heartbeat cleanup:
  `uv run ruff check src/intric/worker/crawl/slot_acquire.py src/intric/worker/crawl/slot_release.py src/intric/worker/crawl/heartbeat.py src/intric/worker/feeder/capacity.py src/intric/worker/feeder/watchdog.py src/intric/worker/crawl_feeder.py src/intric/websites/domain/crawl_service.py tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/test_crawl_slot_release.py tests/unittests/worker/test_heartbeat.py tests/unittests/worker/test_crawl_feeder_process.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py tests/unittests/websites/test_crawl_service_slot_ownership.py tests/integration/test_slot_leak_fixes.py`;
  `uv run pyright --project pyrightconfig.json src/intric/worker/crawl/slot_acquire.py src/intric/worker/crawl/slot_release.py src/intric/worker/crawl/heartbeat.py src/intric/worker/feeder/capacity.py src/intric/worker/feeder/watchdog.py src/intric/worker/crawl_feeder.py src/intric/websites/domain/crawl_service.py tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/test_crawl_slot_release.py tests/unittests/worker/test_heartbeat.py tests/unittests/worker/test_crawl_feeder_process.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py tests/unittests/websites/test_crawl_service_slot_ownership.py tests/integration/test_slot_leak_fixes.py`;
  `uv run pytest tests/unittests/worker/test_crawl_slot_acquire.py tests/unittests/worker/test_crawl_slot_release.py tests/unittests/worker/test_heartbeat.py tests/unittests/worker/test_crawl_feeder_process.py tests/unittests/worker/feeder/test_capacity.py tests/unittests/worker/feeder/test_watchdog.py tests/unittests/websites/test_crawl_service_slot_ownership.py tests/integration/test_slot_leak_fixes.py::TestRedisTTLManagement::test_heartbeat_refreshes_both_keys -q`
  (`117 passed`).
- 2026-05-18: ARQ `JobStatus` is now mapped inside `JobManager` to the
  crawler-facing `JobRuntimeStatus`, so `crawl_status.py` no longer imports
  `arq.jobs`. Validation: `uv run ruff check src/intric/jobs/job_manager.py src/intric/worker/feeder/crawl_status.py tests/unittests/jobs/test_job_manager.py tests/unittests/worker/feeder/test_crawl_status.py`;
  `uv run pyright --project pyrightconfig.json src/intric/jobs/job_manager.py src/intric/worker/feeder/crawl_status.py tests/unittests/jobs/test_job_manager.py tests/unittests/worker/feeder/test_crawl_status.py`;
  `uv run pytest tests/unittests/jobs/test_job_manager.py tests/unittests/worker/feeder/test_crawl_status.py -q`
  (`20 passed`).
- 2026-05-18: Retry cause is now explicit through
  `CrawlRetrySignal.BUSY` vs `CrawlRetrySignal.FAILURE`; tenant-capacity waits
  preserve job age tracking without incrementing the failure retry counter.
  Validation:
  `uv run ruff check src/intric/worker/crawl/recovery.py src/intric/worker/crawl_tasks.py src/intric/worker/crawl/__init__.py tests/unittests/worker/test_crawler_retry_logic.py tests/integration/test_crawler_retry_behavior.py`;
  `uv run pyright --project pyrightconfig.json src/intric/worker/crawl/recovery.py src/intric/worker/crawl_tasks.py src/intric/worker/crawl/__init__.py tests/unittests/worker/test_crawler_retry_logic.py tests/integration/test_crawler_retry_behavior.py`;
  `uv run pytest tests/unittests/worker/test_crawler_retry_logic.py -q`
  (`21 passed`).
- 2026-05-18: The broad `WorkerAdapter` work item is closed as intentionally
  not implemented: a new one-implementation interface would hide no complexity
  today. ARQ status/abort is localized in `JobManager`, enqueue outcomes are
  localized in `enqueue_crawl_job(...)`, and slot capacity is localized in
  `CapacityManager`.
- 2026-05-18: Terminal idempotency and abort behavior are covered by existing
  behavior tests: `commit_terminal(...)` skips the crawl-run update when the
  optimistic job-status gate loses a race, queued/running abort writes
  `CRAWL_ABORTED`, and running abort does not release a worker-owned slot.
  Validation:
  `uv run pytest tests/unittests/worker/test_crawl_terminal.py tests/integration/test_admin_crawler_abort.py -q`
  (`18 passed`).
...
- Public leakage of `slot_preacquired` outside capacity-owned code.
...
views, and the tenant-scoped admin diagnostics endpoints are consumed by the
browser admin crawler page.
...
- [x] Add read-only admin endpoints for broader crawler overview.
- [x] Show active and queued crawls with age, tenant/space/website, worker state, and lifecycle state.
...
- [x] Show high-load websites by a defined score: `schedule_frequency_weight * indexed_content_count * (1 - retention_rate)`, with embedding spend added later only when reliable cost data exists.
...
- [x] Add filters by tenant, space, status, outcome, update interval, and time range.

Progress receipts:

- 2026-05-18: Filter coverage is split across the bounded tenant-admin read
  APIs instead of one catch-all endpoint: tenant is implicit from the admin
  principal, website inventory filters by `space_id`, `update_interval`, and
  failure state, active inventory filters by lifecycle status and website,
  recent failures / watchdog interventions filter by typed outcome and bounded
  `days` time window, and all list endpoints remain paginated.
...
### Step 7A: Embedding Usage, Model Attribution, And Indexed Cost

Status: closed. Implementation is complete; the detailed design below remains
as the long-term contract and review checklist for future maintenance.
This step is an admin transparency step, not billing-grade accounting.

Problem:

- Admins can see indexed size and schedule pressure, but cannot yet explain
  provider-reported embedding token spikes or which embedding model produced a
  crawl's index.
- The current website-processing `cost_pressure_score` is an operational
  prioritization score, not actual USD/token cost. It must not be confused with
  provider cost.
- LiteLLM returns embedding usage, but the current embedding adapter returns
  only vectors, so usage is discarded before the crawler can persist it.

Why it matters:

- Admins need to answer "which websites caused token and storage growth?" from
  the admin UI without reading worker logs.
- Cost attribution must be durable and honest. Repricing historical rows from a
  mutable model table or displaying missing pricing as `$0` would create false
  accounting.
- The observability model should survive an ARQ-to-Celery migration because it
  belongs to embedding, persistence, and crawl-run history, not the queue
  runtime.

Current owners:

| Concept | Existing owner | Problem | Canonical home | Merge/delete path |
|---|---|---|---|---|
| Embedding vectors returned from providers | `EmbeddingModelAdapter.get_embeddings(...)` | Usage is dropped at the provider boundary | `EmbeddingBatchResult` returned by the existing adapter method | Change the canonical method return type; do not add `get_embeddings_with_usage(...)`. |
| Embedding model rate card | Model admin data from Max PR #315 direction | Current branch has no cost columns yet | `embedding_models.input_cost_per_token` / `output_cost_per_token` with Max's `NUMERIC(20, 12)` shape | Reuse Max's column names and precision when that PR lands or merge compatible migration shape carefully. |
| Per-run indexed embedding usage | None | Admin UI cannot attribute tokens or model choice to a run | Nullable snapshot fields on `crawl_runs` | Do not fabricate historical backfill; legacy rows remain unknown. |
| Per-website and tenant aggregates | `CrawlRunRepository.website_processing_aggregate(...)` and `aggregate_baseline(...)` | Aggregates lack tokens/cost/model facts | Extend the existing repository read owners | Do not create a parallel cost dashboard repository. |
| Terminal state | `commit_terminal(...)` | Tempting but wrong place for incremental usage | No change | Keep usage out of terminal commit; terminal commit remains only Job/CrawlRun terminal invariant. |
| Admin crawler UI | Decomposed shadcn admin crawler page | Cost facts need placement without crowding operational KPIs | Activity/workload table plus website detail dialog | Do not add a fourth KPI; rename ambiguous heuristic cost wording when actual cost lands. |

Accounting semantics:

- Step 7A records **indexed embedding usage**: provider-reported tokens from
  embedding batches that are successfully persisted into the index.
- If LiteLLM reports usage but the page persistence savepoint rolls back, Step
  7A must not increment the run's indexed usage. That keeps tokens, chunks, and
  indexed size consistent.
- This is not a provider invoice ledger. If product later needs billing-grade
  provider spend including failed persistence attempts, add an
  `embedding_usage_events` ledger with idempotent batch keys in a separate
  plan. Do not smuggle that concern into `crawl_runs` now.
...
class EmbeddingUsage:
    ...
    source: Literal["provider_reported", "missing"]
...
- `EmbeddingModelAdapter.get_embeddings(...)` returns `EmbeddingBatchResult`.
  Existing callers unwrap `.embeddings`; new crawler persistence code also reads
  `.usage`.
- No parallel method, mutable "last usage" field, callback, dict payload, or
  LiteLLM response object escapes the adapter boundary.
- V1 emits only `provider_reported` or `missing`. No manual tokenizer estimate
  and no historical estimate.
- Provider response parsing happens once in the LiteLLM adapter. Any unknown or
  malformed `usage` shape becomes `EmbeddingUsage(total_tokens=None,
  prompt_tokens=None, source="missing")`; downstream code never touches
  LiteLLM's untyped object.
- Python cost arithmetic uses `Decimal`, never `float`.
- JSON presentation serializes Decimal cost values as strings, not floats.
- When model cost columns land, `EmbeddingModelLike` grows a read-only
  `input_cost_per_token: Decimal | None` property so pyright catches missing
  pricing at the typed boundary. This Protocol update belongs in the same PR as
  the model-cost migration, so the persistence/API wiring PR can depend on it
  without `type: ignore`.

Target storage shape:

Add nullable fields to `crawl_runs`:

```sql
embedding_model_id uuid null references embedding_models(id) on delete set null,
embedding_model_name_snapshot text null,
embedding_model_litellm_name_snapshot text null,
embedding_model_provider_snapshot text null,
embedding_input_cost_per_token_snapshot numeric(20, 12) null,
embedding_input_tokens bigint null,
embedding_usage_source text null,
embedding_total_cost_usd numeric(20, 12) null
```

Rules:

- `embedding_usage_source` is constrained to `provider_reported` or `missing`
  when non-null.
- `embedding_model_provider_snapshot` stores `ModelProviders.provider_type`
  from the resolved model provider at run start, not a provider FK or display
  name.
- One crawl run uses one embedding model. That current invariant is what makes
  run-level snapshot fields correct; if per-page model selection is introduced
  later, usage moves to a per-page or ledger shape instead of overloading these
  columns.
- Run creation snapshots model identity and rate card fields. Deleted or renamed
  models must not erase historical run labels.
- `input_cost_per_token = NULL` means pricing is not configured and renders as
  "Prissättning saknas". `input_cost_per_token = 0` means the operator
  explicitly marked the model as free/local and renders as a real `$0.00` cost.
- `embedding_input_tokens` and `embedding_total_cost_usd` start at `0` only for
  new runs where the model snapshot exists. Legacy rows stay `NULL` so the UI
  can show "unknown" instead of zero.
- Add named `CREATE INDEX CONCURRENTLY` indexes:
  - `idx_crawl_runs_tenant_created_at` on
    `crawl_runs(tenant_id, created_at)`
  - `idx_crawl_runs_tenant_website_created_at` on
    `crawl_runs(tenant_id, website_id, created_at)`
- If Max PR #315 lands first, reuse its model cost columns exactly. If this
  branch lands first, use the byte-for-byte column name
  `embedding_models.input_cost_per_token` with `NUMERIC(20, 12)` precision so
  Max's branch does not need a rename. Completion/transcription cost columns
  stay out of Step 7A unless the merge requires a shared migration, but names
  must still match Max's PR exactly.

Usage write boundary:

- Phase 1 may call LiteLLM outside a DB transaction, but it must only carry the
  typed `EmbeddingUsage` alongside prepared page data.
- Phase 2 owns the durable indexed-usage write. After a page savepoint commits
  its `InfoBlob` and chunks, accumulate that page's usage in memory for the
  current batch.
- Before the outer `session.begin()` commits, perform one tenant-scoped update:

```sql
UPDATE crawl_runs
SET
  embedding_input_tokens =
    COALESCE(embedding_input_tokens, 0) + :token_delta,
  embedding_total_cost_usd =
    COALESCE(embedding_total_cost_usd, 0) + :cost_delta,
  embedding_usage_source = CASE
    WHEN embedding_usage_source = 'provider_reported'
...
- The usage update runs in the same `session.begin()` as the persisted page
  writes. No separate session, post-commit hook, terminal commit, or ARQ job
  result mutation.
- The repository method is keyword-only:
  `record_indexed_embedding_usage(*, run_id: UUID, tenant_id: UUID,
  token_delta: int, cost_delta: Decimal, usage_source:
  Literal["provider_reported", "missing"]) -> None`.
- Token and cost increments happen in one SQL statement. A path may record
  missing usage without tokens/cost, but it must not increment one without the
  other.
- The update filters by both `run_id` and `tenant_id`.
- `embedding_usage_source` is monotonic: once a run has any provider-reported
  usage, a later missing-usage batch must not downgrade the run to `missing`.
- Retained/hash-skipped pages and no-embedding batches do not call
  `record_indexed_embedding_usage(...)` because no embedding call is made.
- Re-embedding due to changed model increments usage even if page content hash
  matches, because indexed vectors changed.
- Current Phase 2 persistence is single-shot per batch. Do not add an automatic
  retry around the outer transaction unless usage writes become idempotent per
  batch, for example through a ledger with a unique batch key. Until then, an
  outer transaction failure rolls back both page writes and usage increments.

UX placement:

- Keep the top KPI row operational: running, action required, scheduled. Do not
  add an embedding cost KPI there.
- Put "who caused load?" in the Activity/workload view:
  - keep indexed size visible,
  - per-run activity rows show that run's snapshotted
    `embedding_input_tokens` and `embedding_total_cost_usd`,
  - per-website processing rows show bounded-window sums sourced from
    `CrawlRunRepository.website_processing_aggregate(...)`,
  - rename the current `cost_pressure_score` presentation to a clearly
    heuristic label before actual cost lands, such as "belastningstryck" /
    "load pressure".
- Add a "Kostnad och modell" section to `WebsiteDetailDialog` using shadcn
  composition and the existing dialog pattern:
  - model snapshot name and provider,
  - last run tokens/cost,
  - selected-window tokens/cost from
    `website_processing_aggregate(website_id=...)`,
  - indexed size,
  - usage source state.
- Unknown states are explicit:
  - missing provider usage: "Tokenanvändning saknas",
  - missing price: "Prissättning saknas",
  - legacy run: "Ej registrerat för äldre körningar".
- Missing price never renders as `$0.00`.
- Currency is USD only in this step. The rate-card value is stored as
  `NUMERIC(20, 12)` USD; SEK conversion later is a presentation concern, not a
  data-model change.

Frontend polish tied to this step:

- Fix the crawler root page license header drift.
- Replace per-row `items.find(...)` lookup with a derived `Map<website_id, item>`.
- Add `aria-live="polite"` only to values that refresh while visible.
- Use `prefers-reduced-motion` for any tab/content transition.
- Avoid count-up animation for polling values; it harms scanning and screen
  reader predictability.
- Use skeletons only for real async loading states, not SSR-filled content.
- Keep dense tables horizontally scrollable on small screens; do not hide the
  diagnostic columns that explain cost.

Implementation sequence:

1. Adapter contract PR:
   - Add `EmbeddingUsage` and `EmbeddingBatchResult` in the embedding domain.
   - Change `EmbeddingModelAdapter.get_embeddings(...)` to return the result
     object.
   - Update callers to unwrap `.embeddings`.
   - Tests: LiteLLM usage present, missing usage, malformed usage, and existing
     callers preserving vector behavior.
2. Schema/ratecard PR:
   - Add compatible model cost columns if Max PR #315 has not landed.
   - Add crawl-run snapshot/token/cost columns.
   - Add the two concurrent indexes.
   - Tests: migration shape, nullable legacy rows, enum/check constraint, and
     index usage for windowed aggregate queries.
3. Persistence/API wiring PR:
   - Snapshot model/rate fields when a crawl run is created.
   - Carry `EmbeddingUsage` through prepared page data.
   - Add `CrawlRunRepository.record_indexed_embedding_usage(...)` and call it
     inside Phase 2's transaction.
   - Extend existing aggregate/read endpoints with nullable token/cost fields.
   - Regenerate `@intric/intric-js` OpenAPI types and keep
     `scripts/check-intric-js-openapi-schema.sh` green.
   - Tests: atomicity with savepoint rollback, tenant-scoped update, retained
     pages do not increment, changed model does increment, aggregate fields are
     bounded by tenant/date/window.
4. UI PR:
   - Add cost/model display in Activity/workload rows and website detail dialog.
   - Rename heuristic `cost_pressure_score` copy to avoid conflating it with
     actual USD cost.
   - Apply the frontend polish items above.
   - Tests: missing pricing renders unknown, no fourth KPI exists, Map lookup
     helper behavior, i18n keys for Swedish/English, responsive overflow smoke.

Acceptance criteria:

- Admins can identify which websites produced provider-reported indexed
  embedding token usage during a bounded time window.
- Admins can see the model snapshot used by the crawl even if the model is later
  renamed or deleted.
- Token/cost aggregates are nullable and honest for legacy or missing-provider
  rows.
- Query cost is bounded by tenant, website, and date-window indexes.
- The UI distinguishes indexed size, load pressure, provider tokens, and USD
  cost.
- The queue runtime is not part of the data model. A future ARQ-to-Celery change
  should not alter usage capture, repository aggregation, or UI semantics.

Tests required:

- Red unit: LiteLLM adapter returns `provider_reported` usage when usage exists.
- Red unit: LiteLLM adapter returns missing usage when the provider omits usage.
- Red unit: no adapter or service returns a raw dict/Any-shaped usage object.
- Red integration: `record_indexed_embedding_usage(...)` updates by
  `(run_id, tenant_id)` and does not touch cross-tenant rows.
- Red integration: a later missing-usage batch does not downgrade a run that
  already has `provider_reported` usage.
- Red integration: page savepoint rollback rolls back the indexed usage
  increment for that page.
- Red integration: hash-retained pages keep usage unchanged.
- Red integration: model-change re-embedding increments usage even if the
  content hash matches.
- Red integration: outer Phase 2 transaction failure rolls back page writes and
  usage increments, and no hidden retry double-increments the run.
- Red integration: aggregate queries use the tenant/date indexes.
- Red frontend: missing rate card renders unknown, not zero cost.
- Red frontend: the KPI row still has only operational navigation buttons.

Non-goals:

- No billing-grade provider spend ledger.
- No fabricated historical backfill or token estimation.
- No fourth KPI.
- No sparklines or trend charts until production data quality is trusted.
- No broad `WorkerAdapter` work in this step.
- No admin-exposed currency conversion.

Rollback and recovery:

- Schema fields are nullable, so disabling the UI or hiding cost fields does not
  break existing crawls.
- If token/cost capture misbehaves, stop wiring the persistence update and leave
  columns nullable while preserving the adapter contract.
- If indexes cause migration issues, drop the concurrent indexes without
  changing stored run data.

### Step 8: Admin Write Controls

Status: in progress. Tenant-admin abort, retry, pause/resume, interval changes,
circuit-breaker reset, bulk interval changes, delete, and safe crawler settings
editing are implemented end-to-end. Remaining work is ongoing hardening and
coverage around state combinations, not first exposure of the controls.
...
- [x] Abort running crawl.
- [x] Retry now.
- [x] Pause/resume scheduled crawl.
- [x] Change update interval.
- [x] Reset circuit breaker.
- [x] Adjust safe crawler settings where already exposed by tenant settings.
