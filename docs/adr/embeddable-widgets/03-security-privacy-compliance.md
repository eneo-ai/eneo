...
| Widget used to reach tools, uploads, web search, MCP, other assistants | uploads and tool assistants are always off; MCP servers and capabilities follow the assistant's configuration unless the widget's `tools_enabled` is off (decision: the assistant as configured is what a widget publishes; the publish checklist lists what visitors get). Identity forwarding and per-call approval never apply to visitors. Scope is one target | `WidgetAskService`, `identity_headers.py` |
