...
3. Build the `AskAssistant` payload with `stream=True`, `files=[]` and `tools=None`. The assistant serves visitors as configured: its general MCP servers and enabled capabilities run for visitors, with no per-widget switch (decision 2026-09-22: what the assistant has is what the widget publishes; one less setting to explain and maintain). Visitors hold no permissions, so capability resolution takes the assistant's configuration as the grant; a visitor never triggers identity forwarding (`build_identity_headers` returns nothing for a visitor user) and never requests per-call tool approval. Governance `effective_config` still applies (model allowlist, prompt library, security classification).
   A short style addendum (`WIDGET_STYLE_PROMPT`) is appended to the assistant's prompt for widget turns only: brief plain prose, lists only for enumerations, no headings; the assistant's own prompt and its behaviour in the app are untouched.
