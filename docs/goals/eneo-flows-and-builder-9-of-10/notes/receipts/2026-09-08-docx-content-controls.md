...
## Landing and remaining verification
...
Still required: a live Builder plan with `rapport.docx` attached and a real flow's verbatim DOCX opened in Word. These checks await the existing local test-login/API-key setup requested from the owner. Browser automation of the local editor is blocked by administrator policy. Issue `eneo-ytn` remains in progress for these checks; the ten confirmed baseline unit failures are tracked separately in `eneo-wqq`.
