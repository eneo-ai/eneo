...
| Review checkpoint legacy snapshot fallback | Review checkpoint models and API schema | Existing API still exposes `step_snapshot_available` so clients can avoid depending on missing step metadata. See `backend/src/intric/flows/domain/flow.py:316` and `backend/src/intric/flows/domain/flow.py:368`. | If source and data preflight prove no current checkpoints need the fallback, delete it with the tests that only protect the compatibility behavior. No production users means the default answer should be deletion once current data is clean. |
