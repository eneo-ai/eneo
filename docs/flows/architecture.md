...
| Worker composition and recovery entry points | `runtime/tasks.py` and platform worker registration | `backend/src/eneo/flows/runtime/tasks.py::execute_flow_run_task`, `backend/src/eneo/flows/runtime/tasks.py::reconcile_stale_running_runs`, `backend/src/eneo/flows/runtime/tasks.py::redispatch_stale_queued_runs`, `backend/src/eneo/worker/platform_tasks.py` | The queue-neutral boundary parses typed dispatch payloads, invokes the runtime, contains escaped failures through secondary terminalization, and exposes scheduled recovery adapters. The platform owns ARQ queues, capacity, timeouts, scheduling, lifecycle, tracing, and health. Application services and repositories own run and recovery semantics. |
...
2. The router commits before dispatching the worker task, so the platform worker starts from
...
3. The platform task resolves the principal from `flow_runs.principal_type`,
...
or partial calls. Both maps remain sequential and rerun as a whole. The platform
...
| Every `output_mode` builds its expected StepHandler         | `backend/tests/unittests/flows/test_flow_runtime_step_handlers.py::test_executor_builds_expected_step_handler` |
| OutputFormatSpecs has a spec for every `output_type`        | `backend/tests/unittests/flows/test_flow_runtime_output_formats.py::test_output_format_registry_is_total_for_flow_output_types` |
