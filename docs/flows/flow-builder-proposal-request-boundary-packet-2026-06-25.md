...
| Proposal retry target | Proposal repair itself | `backend/src/eneo/flows/ai_builder/ai_builder_proposal_tool_contracts.py:150` shows `ToolRetryConfig` only carries target kind, prompt, and invocation processor. | `ToolRetryConfig.target_tool_name` was deleted. |
...
| Forced retry after text still forces `propose_flow`. | `backend/src/eneo/flows/ai_builder/ai_builder_proposal_repair.py:588` calls the same owner. |
| Retry transcript still preserves assistant tool calls and tool feedback. | `backend/src/eneo/flows/ai_builder/ai_builder_proposal_repair.py:232` returns `list[LLMMessageParam]` with assistant/tool messages. |
