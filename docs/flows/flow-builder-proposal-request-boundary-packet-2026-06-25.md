...
3. The dead `target_tool_name` retry setting was deleted because proposal repair only targets `propose_flow`.
...
| Behavior | Proof |
| --- | --- |
| First proposal attempt still forces `propose_flow`. | `backend/src/intric/flows/ai_builder/ai_builder_proposal_submission.py:263` calls `forced_tool_choice(PROPOSE_FLOW_TOOL_NAME)`. |
| Forced retry after text still forces `propose_flow`. | `backend/src/intric/flows/ai_builder/ai_builder_proposal_repair.py:588` calls the same owner. |
| Retry transcript still preserves assistant tool calls and tool feedback. | `backend/src/intric/flows/ai_builder/ai_builder_proposal_repair.py:232` returns `list[LLMMessageParam]` with assistant/tool messages. |
| Duplicate forced tool-choice literals cannot return silently. | `backend/tests/unittests/flows/ai_builder/test_ai_builder_import_ownership.py:2112` guards submission and repair. |
