...
| Decision | Choice |
|---|---|
| App location | `frontend/apps/web-next` in the existing Bun monorepo, running side-by-side with `frontend/apps/web` until cutover |
...
| Design system | UI is composed exclusively from shadcn/ui + AI Elements primitives. All primitives and generic composites live in a shared workspace package `frontend/packages/ui-react` (`@intric/ui-react`), installed there via the shadcn CLI monorepo pattern and shared across the application; app code contains feature compositions only, never bespoke primitives. Theme: a fresh, cohesive shadcn palette (new-york); the Eneo accent color is the ONLY token carried over from the old OKLCH set, mapped to shadcn `--primary`. The Svelte app's full token mapping is intentionally NOT transcribed. |
...
- **Env vars today**: `PUBLIC_ENEO_BACKEND_URL` (browser), `ENEO_BACKEND_URL` / `ENEO_BACKEND_SERVER_URL` (server), `JWT_SECRET` (shared with backend; eliminated), `ZITADEL_*` (dropped), `PUBLIC_ORIGIN`, feature flags `SHOW_WEB_SEARCH`, `SHOW_HELP_CENTER`, `FORCE_LEGACY_AUTH`, `REQUEST_INTEGRATION_FORM_URL`, `HELP_CENTER_URL`.
...
```
frontend/packages/ui-react/              @intric/ui-react — shared React component library
├─ src/components/ui/                    shadcn/ui primitives (new-york, CLI-installed)
├─ src/components/ai-elements/           AI Elements (CLI-vendored, editable)
├─ src/components/composites/            generic composites (data-table, form fields, page
│                                        header, confirm dialog, empty state, …)
└─ src/styles/globals.css                Tailwind v4 @theme: fresh shadcn palette + Eneo accent

frontend/apps/web-next/                  Next.js 16 App Router, TS strict
├─ src/app/
...
├─ src/features/                        feature compositions (built from @intric/ui-react)
└─ proxy.ts                             optimistic auth gating (cookie presence) + locale
```

Principles:
- All UI is built from shadcn/ui and AI Elements primitives that live in `@intric/ui-react` and are imported across the app; if a primitive is missing, install it into the package via the shadcn CLI rather than hand-rolling it. App code composes; it does not define primitives.
