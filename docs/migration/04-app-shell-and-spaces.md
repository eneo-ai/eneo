...
- Component sourcing (standing rule from Phase 1): primitives only from `@intric/ui-react`; missing shadcn components are CLI-installed into the package, never hand-rolled. This phase is where most generic composites are born; put them in `packages/ui-react/src/components/composites/` (expected: data-table with server-side pagination/filter wiring, form-field wrapper, page-header, confirm-dialog, empty-state, secret-reveal for API keys) so Phases 5–7 reuse them. Route-specific compositions stay in the app under `src/features/`.
