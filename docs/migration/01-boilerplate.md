...
8. **Env module**: `src/lib/env.ts` validating server env with zod at import time: `ENEO_BACKEND_URL` (required), `SESSION_SECRET` (required from Phase 2, optional now), feature flags `SHOW_HELP_CENTER`, `HELP_CENTER_URL`, `REQUEST_INTEGRATION_FORM_URL` (`SHOW_WEB_SEARCH` is legacy and is NOT ported). No `NEXT_PUBLIC_*` backend URL: the browser never calls the backend directly in this architecture. Add `.env.example`.
