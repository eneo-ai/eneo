"""Phase 1 invariants for the same-IdP MCP OAuth schema rollout.

Phase 1 lands ``auth_scope`` + ``expected_idp_issuer`` on ``mcp_servers``
and the admin-API gate that rejects ``per_user`` / ``per_tenant`` until
the token-exchange broker ships (Phase 3). The invariants verified here:

1. Pre-existing rows have a non-null ``auth_scope`` after migration and it
