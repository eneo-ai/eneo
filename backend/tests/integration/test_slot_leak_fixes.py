...
class TestSessionLifecycle:
    ...
    def test_session_close_location_documented(self):
        """
        Verify that session.close() is called before the Hybrid v2 loop.
        ...
        # The session close should appear within the Hybrid v2 block
        assert "await session.close()" in source, (
            "session.close() should be present in crawl_task"
        )

        # Verify it's in the Hybrid v2 path
        hybrid_v2_start = source.find("if settings.hybrid_v2_enabled and crawl_context:")
        session_close = source.find("await session.close()")

        assert hybrid_v2_start != -1, "Hybrid v2 block should exist"
        assert session_close != -1, "session.close() should exist"
        assert session_close > hybrid_v2_start, (
            "session.close() should be after Hybrid v2 block starts"
        )
...
class TestRedisTTLManagement:
    ...
    def test_heartbeat_refreshes_both_keys(self):
        ...
        This is documented in the Hybrid v2 heartbeat code.
        """
        import inspect
        from intric.worker import crawl_tasks
        ...
        assert "pipe = redis_client.pipeline(transaction=True)" in source
        assert "pipe.expire(concurrency_key, semaphore_ttl)" in source
        assert "pipe.expire(flag_key, semaphore_ttl)" in source
...
class TestZombieJobPrevention:
    ...
    def test_preemption_check_exists(self):
        ...
        # Verify preemption check exists
        assert "current_job.status == JobStatus.FAILED" in source
