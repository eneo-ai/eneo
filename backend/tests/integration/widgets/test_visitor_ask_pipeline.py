...
async def visitor_pipeline(
    ...
    injected knowledge on a tool-calling model, an MCP server that forwards
    identity, and web search and image generation from external providers."""
    ...
    async with sessionmanager.session() as db, db.begin():
        ...
        assistant.knowledge_mode = "inject"
        await db.execute(
...
async def test_visitor_runs_the_assistant_as_configured_and_sees_only_its_answer(
    ...
    # Injected knowledge: retrieved once, no knowledge tool.
