...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("file_type", "variant", "media_type"),
    [
        (FileType.TEXT, FileContentVariant.EXTRACTED_TEXT, "text/plain"),
        (FileType.IMAGE, FileContentVariant.LEGACY_IMAGE, "image/png"),
    ],
)
async def test_missing_original_is_typed_and_never_falls_back(
    ...
    payload = b"available processing content"
    file_id = await _persist_prepared(
        db_container,
        PreparedFileUpload(
            name="legacy.pdf",
    ...
    processing = await _signed_download(
        client, {"X-API-Key": admin_user_api_key.key}, file_id, original=False
    )
    assert processing.status_code == 200 and processing.content == payload
