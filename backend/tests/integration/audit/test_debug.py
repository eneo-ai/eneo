...
async def test_debug_full_trace(client, admin_user_api_key, admin_user, db_container):
    ...
    async with db_container() as container:
        ...
        result = await session.execute(text("SELECT key, user_id FROM api_keys WHERE user_id = :user_id"), {"user_id": str(admin_user.id)})
    ...
    print(f"8. /audit/access-session response: {response2.status_code} - {response2.text}")
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_debug_http_database_connection(client, admin_user_api_key, test_settings):
...
async def test_debug_trace_authenticate(client, admin_user_api_key, monkeypatch):
    ...
    async def traced_authenticate(self, token=None, api_key=None, with_quota_used=False):
        call_count["count"] += 1
        print(f"\n>>> AUTHENTICATE CALL #{call_count['count']}")
        print(f"    token: {token[:20] if token else None}...")
        print(f"    api_key: {api_key[:20] if api_key else None}...")
        
        result = await original_authenticate(self, token=token, api_key=api_key, with_quota_used=with_quota_used)
...
async def test_debug_api_key_header_instances():
    ...
    from eneo.server.dependencies.auth_definitions import API_KEY_HEADER as auth_def_header
