"""Invariants: /api/v1/me/mcp-connections status reporting.
...
async def test_per_user_server_with_wrong_issuer_reports_idp_mismatch(
    ...
    server_issuer = "https://idp.example/realms/eneo"
    user_issuer = "https://other-idp.example/v2.0"
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_per_user_server_with_matching_sso_session_reports_connected(
    client, db_container, default_user, default_user_token
):
    """A matching IdP session with no cached token yet is usable: the broker
    mints on first tool call, so the read-out must not scare the user."""
    issuer = "https://idp.example/realms/eneo"
    sid = await _insert_server(
        db_container,
        tenant_id=default_user.tenant_id,
        name=f"ready-{uuid4().hex[:6]}",
        auth_scope="per_user",
        expected_idp_issuer=issuer,
    )
