...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_module_service_key_catalog_uses_effective_key_eligibility(
    client, db_container, default_user_token
):
    ...
    eligible_lookup = await client.get(
        f"/api/v1/admin/modules/service-keys/{eligible_write}/",
        headers={"Authorization": f"Bearer {default_user_token}"},
    )
    assert eligible_lookup.status_code == 200, eligible_lookup.text

    ineligible_lookup = await client.get(
        f"/api/v1/admin/modules/service-keys/{expired}/",
        headers={"Authorization": f"Bearer {default_user_token}"},
    )
    assert ineligible_lookup.status_code == 404, ineligible_lookup.text
