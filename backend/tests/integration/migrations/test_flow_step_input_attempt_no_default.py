"""Migration tests for explicit Flow step-input projection attempts.

Run explicitly:
    pytest -m migration_isolation tests/integration/migrations/test_flow_step_input_attempt_no_default.py -q
"""
...
PRIOR_REVISION = "20260607_flow_runtime_uploads"
MIGRATION_REVISION = "20260608_step_input_no_default"
...
def _insert_step_input_without_attempt(conn) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
def test_upgrade_removes_attempt_default_and_downgrade_restores_it(migration_db):
    ...
    with pytest.raises(psycopg2.errors.NotNullViolation):
        _insert_step_input_without_attempt(conn)
