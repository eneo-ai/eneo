...
def test_context_builder_records_prompt_inclusion_trace(
    monkeypatch: pytest.MonkeyPatch,
    context_builder: ContextBuilder,
):
    monkeypatch.setattr(
        "intric.completion_models.infrastructure.context_builder.CONTEXT_SIZE_BUFFER",
        0,
    )
    monkeypatch.setattr(
        "intric.completion_models.infrastructure.context_builder.count_tokens",
    ...
    assert context.knowledge_trace is not None
    assert context.knowledge_trace.included_source_ids == ["1"]
    assert context.knowledge_trace.not_included_source_ids == ["2"]
    assert context.knowledge_trace.included_source_count == 1
    assert context.knowledge_trace.not_included_source_count == 1
    assert context.knowledge_trace.truncated_by_token_budget is True
    assert context.knowledge_trace.included_groups[0].source_title == "blob 1"
