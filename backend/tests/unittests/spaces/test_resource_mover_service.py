...
async def test_assistant_with_a_live_widget_cannot_move_between_spaces() -> None:
    assistant_id = uuid4()
    assistant = MagicMock(id=assistant_id)
    source_space = MagicMock()
    source_space.id = uuid4()
    ...
    widget_repo.list_by_target.return_value = [MagicMock()]
    space_repo = AsyncMock()
    ...
    with pytest.raises(BadRequestException, match="Archive.*web widget"):
        await service.move_assistant_to_space(
            assistant_id=assistant_id, space_id=uuid4()
        )
    widget_repo.list_by_target.assert_awaited_once_with(assistant_id)
    target_space.add_assistant.assert_not_called()
    space_repo.update.assert_not_awaited()

    widget_repo.list_by_target.return_value = []
    await service.move_assistant_to_space(assistant_id=assistant_id, space_id=uuid4())
