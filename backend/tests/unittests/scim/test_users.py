...
class TestCreateUser:
    ...
    async def test_returns_user_schema(self, client: AsyncClient, mock_service: AsyncMock):
        mock_service.create_user.return_value = _make_scim_user()
        res = await client.post("/scim/v2/Users", json=CREATE_PAYLOAD, headers=AUTH)
    ...
    async def test_conflict_returns_status_as_string(self, client: AsyncClient, mock_service: AsyncMock):
        mock_service.create_user.side_effect = ScimUserConflictError("already exists")
        res = await client.post("/scim/v2/Users", json=CREATE_PAYLOAD, headers=AUTH)
...
class TestGetUser:
    ...
    async def test_returns_correct_user(self, client: AsyncClient, mock_service: AsyncMock):
        user = _make_scim_user()
        mock_service.get_user.return_value = user
        res = await client.get(f"/scim/v2/Users/{user.id}", headers=AUTH)
...
class TestListUsers:
    ...
    async def test_passes_filter_to_service(self, client: AsyncClient, mock_service: AsyncMock):
        mock_service.list_users.return_value = ([], 0)
        await client.get('/scim/v2/Users?filter=userName eq "jane@example.com"', headers=AUTH)
        mock_service.list_users.assert_called_once_with(
            filter_str='userName eq "jane@example.com"', sort_by=None, sort_order=None, start_index=1, count=None
        )

    async def test_passes_sort_to_service(self, client: AsyncClient, mock_service: AsyncMock):
        mock_service.list_users.return_value = ([], 0)
        await client.get("/scim/v2/Users?sortBy=userName&sortOrder=descending", headers=AUTH)
        mock_service.list_users.assert_called_once_with(
            filter_str=None, sort_by="userName", sort_order="descending", start_index=1, count=None
        )
    ...
    async def test_default_pagination_returns_all(self, client: AsyncClient, mock_service: AsyncMock):
...
class TestReplaceUser:
    async def test_returns_200(self, client: AsyncClient, mock_service: AsyncMock):
        ...
        res = await client.put(f"/scim/v2/Users/{user.id}", json=CREATE_PAYLOAD, headers=AUTH)
        assert res.status_code == 200

    async def test_returns_updated_user_body(self, client: AsyncClient, mock_service: AsyncMock):
        user = _make_scim_user()
        mock_service.replace_user.return_value = user
        res = await client.put(f"/scim/v2/Users/{user.id}", json=CREATE_PAYLOAD, headers=AUTH)
        assert res.json()["id"] == user.id
...
class TestPatchUser:
    async def test_returns_200(self, client: AsyncClient, mock_service: AsyncMock):
        ...
        res = await client.patch(f"/scim/v2/Users/{user.id}", json=payload, headers=AUTH)
        assert res.status_code == 200

    async def test_deactivate_user_returns_active_false(self, client: AsyncClient, mock_service: AsyncMock):
        user = _make_scim_user()
        user.active = False
        mock_service.patch_user.return_value = user
        payload = {"schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
    ...
    async def test_deactivate_user_returns_state_inactive(self, client: AsyncClient, mock_service: AsyncMock):
        user = _make_scim_user()
        user.active = False
        mock_service.patch_user.return_value = user
        payload = {"schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
