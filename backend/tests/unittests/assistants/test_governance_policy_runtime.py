...
async def test_ask_without_tools_injects_tool_mode_knowledge():
    """Widget visitors ask with ``allow_tools=False``. A TOOL-mode assistant on
    a tool-capable model must then ground the answer by retrieve-and-inject
    instead of a loopback knowledge server, whose scoped token has no user."""
    ...
    with patch(
        ...
        await service.ask(
            question="hello", assistant_id=assistant.id, allow_tools=False
        )
        build_server.assert_not_awaited()
        kwargs = assistant.ask.await_args.kwargs
        assert kwargs["knowledge_mcp_server"] is None
        assert kwargs["mcp_servers_override"] == []
        assert kwargs["capability_mcp_servers"] == []

        # The same assistant still gets the knowledge tool for ordinary users.
        await service.ask(question="hello", assistant_id=assistant.id)
