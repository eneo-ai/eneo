...
def test_capped_required_slot_is_not_asked_and_recorded_as_blocked() -> None:
    policy = build_planner_action_policy(
        session_state=PlanningState.empty(),
        unresolved_architectural_choices=frozenset(),
        selected_discovery_question_ids=frozenset(),
        ask_counts={"primary_runtime_input": 2},
    )

    # primary_runtime_input hit the re-ask cap: no longer asked, recorded as a
    # capped required target; terminal_output (below cap) stays askable.
    assert "primary_runtime_input" not in policy.allowed_ask_question_targets
    assert "primary_runtime_input" in policy.capped_required_ask_targets
    assert "terminal_output" in policy.allowed_ask_question_targets
    assert policy.allowed_action_kinds == ("ask_question",)


def test_capped_optional_discovery_question_is_dropped_not_blocked() -> None:
    policy = build_planner_action_policy(
        session_state=PlanningState.empty(),
        unresolved_architectural_choices=frozenset(),
        selected_discovery_question_ids=frozenset({"document_material_scope"}),
        ask_counts={"document_material_scope": 2},
    )

    # An optional discovery question (not required to commit on an empty state)
    # is dropped at the cap, not recorded as blocked-required.
    assert "document_material_scope" not in policy.allowed_ask_question_targets
    assert "document_material_scope" not in policy.capped_required_ask_targets
    assert "primary_runtime_input" in policy.allowed_ask_question_targets


def test_all_core_targets_capped_yields_blocked_policy() -> None:
    policy = build_planner_action_policy(
        session_state=PlanningState.empty(),
        unresolved_architectural_choices=frozenset(),
        selected_discovery_question_ids=frozenset(),
        ask_counts={"primary_runtime_input": 2, "terminal_output": 2},
    )

    # Both required core slots are capped: nothing askable, commit still blocked.
    assert policy.allowed_ask_question_targets == ()
    assert policy.allowed_action_kinds == ()
    assert set(policy.capped_required_ask_targets) == {
        "primary_runtime_input",
        "terminal_output",
    }


def test_below_cap_targets_remain_askable() -> None:
    policy = build_planner_action_policy(
        session_state=PlanningState.empty(),
        unresolved_architectural_choices=frozenset(),
        selected_discovery_question_ids=frozenset(),
        ask_counts={"primary_runtime_input": 1},
    )

    assert "primary_runtime_input" in policy.allowed_ask_question_targets
    assert policy.capped_required_ask_targets == ()
    assert policy.allowed_action_kinds == ("ask_question",)
