...
def _foreign_key_constraint(name: str) -> ForeignKeyConstraint:
    for constraint in FlowRunWebhookDeliveries.__table__.constraints:
...
def test_webhook_deliveries_reference_step_attempt_natural_key() -> None:
    constraint = _foreign_key_constraint("fk_flow_run_webhook_deliveries_step_attempt")
