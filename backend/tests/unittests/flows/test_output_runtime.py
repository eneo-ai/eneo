...
async def test_process_typed_output_json_with_contract_validation() -> None:
    ...
    deps = OutputRuntimeDeps(
        ...
        render_document=lambda text, output_type, step_order, title: (b"", ""),
        render_structured_document=lambda data,
        output_type,
        step_order,
        title,
        schema=None: (
...
async def test_process_typed_output_docx_creates_artifact_file() -> None:
    ...
    render_titles: list[str] = []

    def render_document(text, output_type, step_order, title):
        render_titles.append(title)
    ...
    assert render_titles == ["Nämndmöte 2026-09-23"]
...
async def test_process_typed_output_pdf_bytes_obeys_document_render_limits() -> None:
    ...
    file_service.save_generated_file.assert_not_called()
...
async def test_process_typed_output_docx_renders_validated_structured_contract() -> (
    ...
    render_calls: list[tuple[object, str, int, str, dict | None]] = []

    def _validate(data, schema, label):
        validate_calls.append((data, schema, label))

    def _render_structured(data, output_type, step_order, title, schema=None):
        render_calls.append((data, output_type, step_order, title, schema))
    ...
    assert render_calls == [
        ({"structured": 1}, "docx", 4, "Nämndmöte 2026-09-23", {"type": "object"})
    ]
    assert result.artifacts is not None
...
async def test_process_typed_output_docx_treats_empty_contract_as_structured() -> None:
    ...
    run = _Run(tenant_id=uuid4())
    file_service = SimpleNamespace(
        save_generated_file=AsyncMock(return_value=SimpleNamespace(id=uuid4()))
    )
    render_calls: list[tuple[object, str, int, str, dict | None]] = []
    ...
    assert render_calls == [(["legacy", "data"], "docx", 4, "Nämndmöte 2026-09-23", {})]
    assert result.artifacts is not None
...
@pytest.mark.asyncio
async def test_process_typed_output_file_service_failure_propagates() -> None:
    step = _Step(step_order=9, output_type="pdf", output_contract=None)
    run = _Run(tenant_id=uuid4())
    file_service = SimpleNamespace(
        save_generated_file=AsyncMock(side_effect=RuntimeError("storage down"))
    )
