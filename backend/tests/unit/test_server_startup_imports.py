...
def test_flow_layer_packages_reexport_existing_symbols() -> None:
    ...
    flow_models = importlib.import_module("intric.flows.flow")
    flow_service = importlib.import_module("intric.flows.flow_service")
    flow_run_service = importlib.import_module("intric.flows.flow_run_service")
    flow_repo = importlib.import_module("intric.flows.flow_repo")
    flow_run_repo = importlib.import_module("intric.flows.flow_run_repo")
    flow_version_repo = importlib.import_module("intric.flows.flow_version_repo")
    flow_dispatch = importlib.import_module("intric.flows.flow_dispatch")

    assert domain.Flow.__name__ == flow_models.Flow.__name__
    assert domain.Flow.__module__ == flow_models.Flow.__module__
    assert application.FlowService.__name__ == flow_service.FlowService.__name__
    assert application.FlowService.__module__ == flow_service.FlowService.__module__
    assert application.FlowRunService.__name__ == flow_run_service.FlowRunService.__name__
    assert application.FlowRunService.__module__ == flow_run_service.FlowRunService.__module__
    assert infrastructure.FlowRepository.__name__ == flow_repo.FlowRepository.__name__
    assert infrastructure.FlowRepository.__module__ == flow_repo.FlowRepository.__module__
    assert infrastructure.FlowRunRepository.__name__ == flow_run_repo.FlowRunRepository.__name__
    assert (
        infrastructure.FlowRunRepository.__module__
        == flow_run_repo.FlowRunRepository.__module__
    )
    assert (
        infrastructure.FlowVersionRepository.__name__
        == flow_version_repo.FlowVersionRepository.__name__
    )
    assert (
        infrastructure.FlowVersionRepository.__module__
        == flow_version_repo.FlowVersionRepository.__module__
    )
    assert (
        application.dispatch_flow_run_after_commit.__name__
        == flow_dispatch.dispatch_flow_run_after_commit.__name__
    )


def test_flow_consumer_router_split_modules_reexport_existing_handlers() -> None:
    ...
    assert (
        ...
        == flow_consumer_router.get_flow_run_contract.__name__
    )
    assert (
        flow_upload_router.get_flow_input_policy.__name__
        == flow_consumer_router.get_flow_input_policy.__name__
    )
    assert (
        flow_upload_router.upload_flow_file.__name__
        == flow_consumer_router.upload_flow_file.__name__
    )
    assert (
        flow_upload_router.upload_flow_runtime_file.__name__
        == flow_consumer_router.upload_flow_runtime_file.__name__
    )
    assert (
        flow_run_router.create_flow_run.__name__
    ...
    assert (
        flow_run_router.get_flow_run_alias.__name__
    ...
    assert (
        flow_run_router.redispatch_flow_run_alias.__name__
...
def test_flow_and_ai_builder_routes_have_unique_contracts_and_docs() -> None:
    ...
    flow_routes = [
        ...
        and route.path.startswith("/api/v1/flows")
    ]
...
def test_flow_and_ai_builder_request_models_expose_openapi_examples() -> None:
    ...
    ai_builder_models = importlib.import_module("intric.flows.ai_builder.ai_builder_models")
...
def test_flow_and_ai_builder_openapi_documents_parameters_and_error_examples() -> None:
    ...
    send_message_operation = schema["paths"]["/api/v1/flows/ai-builder/sessions/{session_id}/messages"]["post"]
    assert send_message_operation["tags"] == ["ai-builder"]
    send_message_params = {param["name"]: param for param in send_message_operation["parameters"]}
    assert send_message_params["session_id"]["description"]
    assert (
        send_message_operation["responses"]["200"]["content"]["text/event-stream"]["example"]
    )
    assert (
        send_message_operation["responses"]["403"]["content"]["application/json"]["example"]["code"]
        == "insufficient_scope"
    )
    assert (
        send_message_operation["responses"]["404"]["content"]["application/json"]["example"]["code"]
        == "not_found"
    )

    apply_plan_operation = schema["paths"]["/api/v1/flows/ai-builder/plans/{plan_id}/apply"]["post"]
    assert apply_plan_operation["tags"] == ["ai-builder"]
    apply_plan_params = {param["name"]: param for param in apply_plan_operation["parameters"]}
    assert apply_plan_params["plan_id"]["description"]
    assert (
        apply_plan_operation["responses"]["409"]["content"]["application/json"]["example"]["code"]
        == "stale_revision"
    )

    get_flow_operation = schema["paths"]["/api/v1/flows/{id}/"]["get"]
    get_flow_params = {param["name"]: param for param in get_flow_operation["parameters"]}
    ...
    list_flows_params = {param["name"]: param for param in list_flows_operation["parameters"]}
    ...
    get_flow_assistant_operation = schema["paths"]["/api/v1/flows/{id}/assistants/{assistant_id}/"]["get"]
    ...
    assert (
        get_flow_assistant_operation["responses"]["404"]["content"]["application/json"]["example"]["code"]
        == "not_found"
    )
    ...
    signed_url_operation = schema["paths"]["/api/v1/flows/{id}/template-files/{file_id}/signed-url/"]["post"]
    signed_url_params = {param["name"]: param for param in signed_url_operation["parameters"]}
    assert signed_url_params["id"]["description"]
    assert signed_url_params["file_id"]["description"]

    template_list_operation = schema["paths"]["/api/v1/flows/{id}/template-files/"]["get"]
    assert (
        template_list_operation["responses"]["403"]["content"]["application/json"]["example"]["code"]
        == "insufficient_scope"
    )
    assert (
        template_list_operation["responses"]["404"]["content"]["application/json"]["example"]["code"]
        == "not_found"
    )

    run_contract_operation = schema["paths"]["/api/v1/flows/{id}/run-contract/"]["get"]
    run_contract_params = {param["name"]: param for param in run_contract_operation["parameters"]}
    assert run_contract_params["id"]["description"]

    input_policy_operation = schema["paths"]["/api/v1/flows/{id}/input-policy/"]["get"]
    input_policy_params = {param["name"]: param for param in input_policy_operation["parameters"]}
    ...
    upload_runtime_file_operation = schema["paths"]["/api/v1/flows/{id}/steps/{step_id}/runtime-files/"]["post"]
