...
class TestFailureSemantics:
    ...
    async def test_clean_eof_without_done_event_raises(self, service):
        # A clean EOF without the required terminal event is not a completed
        # crawl and must never expose its prefix to the persistence pipeline.
        service.body = ndjson(page("https://k.se/a"))
        crawler = RemoteCrawler(base_url=service.base_url)

        with pytest.raises(CrawlerException, match="without a terminal done event"):
            async with crawler.crawl(url="https://k.se"):
                pass

    async def test_unrecognized_done_status_raises(self, service):
        service.body = ndjson(
            page("https://k.se/a"),
            {"type": "done", "status": "mystery", "outcome": {}},
        )
        crawler = RemoteCrawler(base_url=service.base_url)

        with pytest.raises(CrawlerException, match="unrecognized terminal status"):
            async with crawler.crawl(url="https://k.se"):
                pass

    async def test_event_after_done_raises(self, service):
        service.body = ndjson(DONE, page("https://k.se/a"))
        crawler = RemoteCrawler(base_url=service.base_url)

        with pytest.raises(CrawlerException, match="after the terminal done event"):
            async with crawler.crawl(url="https://k.se"):
                pass
