...
INTENTIONALLY_UNGUARDED = {
    ...
    "/users": "Admin mutation endpoints (POST /admin/invite/, PATCH /admin/{id}/, DELETE /admin/{id}/, /api-keys/) are on users_admin_router with admin scope + admin key guards. GET / is deliberately unguarded for member-picker use cases (UserSparse-only, tenant-scoped at repo layer). /me/ and /tenant/ safe for any scoped key.",
