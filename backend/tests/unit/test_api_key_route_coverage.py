...
INTENTIONALLY_UNGUARDED = {
    ...
    "Two surfaces are session-only and reject API keys outright: /admin/modules "
    "(require_session_auth + Permission.MODULES) and the /admin/spaces "
    "oversight mutations (admin scope + admin key guards + require_session_auth "
    "+ Permission.ADMIN), because a key must never make itself a space member",
