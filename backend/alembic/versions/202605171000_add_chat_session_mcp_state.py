...
protocol's ``mcp-session-id`` (server-assigned on ``initialize``) is what
custom MCP servers like file-workbench use to scope per-session state
(uploaded files, etc.). Persisting it keyed to (chat_session_id, mcp_server_id)
lets the next turn resume the same logical MCP session by sending the stored
id as the initial ``Mcp-Session-Id`` header.

Generic shape: not file-workbench-specific. Any MCP server that returns a
session id on initialize can use the same row.
