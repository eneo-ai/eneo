"""create personal_chat_policies + m2m tables

Tenant-level policy for personal chat (default assistants in user-owned
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint(
            "tenant_id", name="uq_personal_chat_policies_tenant_id"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        "personal_chat_policy_completion_models",
        ...
        sa.ForeignKeyConstraint(
            ["policy_id"], ["personal_chat_policies.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        "personal_chat_policy_mcp_servers",
        sa.Column("policy_id", sa.UUID(), nullable=False),
...
def downgrade() -> None:
    op.drop_table("personal_chat_policy_mcp_servers")
    op.drop_index(
        "uniq_policy_default_model",
        table_name="personal_chat_policy_completion_models",
    )
    op.drop_table("personal_chat_policy_completion_models")
    op.drop_table("personal_chat_policies")
