...
async def seed_global_models():
    ...
    async with sessionmanager.connect() as conn:
        ...
        await conn.execute(
            text("DELETE FROM tenants WHERE name LIKE 'Migration Test Tenant%'")
        )

        # Clean global models (these will be recreated)
        await conn.execute(
            text(
                "DELETE FROM completion_model_settings WHERE completion_model_id IN (SELECT id FROM completion_models WHERE tenant_id IS NULL)"
            )
        )
        await conn.execute(
            text(
                "DELETE FROM embedding_model_settings WHERE embedding_model_id IN (SELECT id FROM embedding_models WHERE tenant_id IS NULL)"
            )
        )
        await conn.execute(
            text(
                "DELETE FROM transcription_model_settings WHERE transcription_model_id IN (SELECT id FROM transcription_models WHERE tenant_id IS NULL)"
            )
        )
        await conn.execute(
            text(
                "DELETE FROM spaces_completion_models WHERE completion_model_id IN (SELECT id FROM completion_models WHERE tenant_id IS NULL)"
            )
        )
        await conn.execute(
            text(
                "DELETE FROM spaces_embedding_models WHERE embedding_model_id IN (SELECT id FROM embedding_models WHERE tenant_id IS NULL)"
            )
        )
        await conn.execute(
            text("DELETE FROM completion_models WHERE tenant_id IS NULL")
        )
        await conn.execute(text("DELETE FROM embedding_models WHERE tenant_id IS NULL"))
        await conn.execute(
            text("DELETE FROM transcription_models WHERE tenant_id IS NULL")
        )
        ...
        # Get Owner role ID
        owner_role_result = await conn.execute(
        ...
        salt1 = salt1.scalar()
        hashed_pw1 = await conn.execute(
            text("SELECT crypt(:pw, :salt)"), {"pw": "test123", "salt": salt1}
        )
        ...
        salt2 = salt2.scalar()
        hashed_pw2 = await conn.execute(
            text("SELECT crypt(:pw, :salt)"), {"pw": "test123", "salt": salt2}
        )
        ...
        salt3 = salt3.scalar()
        hashed_pw3 = await conn.execute(
            text("SELECT crypt(:pw, :salt)"), {"pw": "test123", "salt": salt3}
        )
        ...
        completion_models = [
            ...
            (
                ...
                None,
                None,
                "https://llama-3-3-70b-instruct.endpoints.kepler.ai.cloud.ovh.net/api/openai_compat/v1",
                True,
                70,
            ),
            ...
            """),
                {
                    ...
                    "nr_billion_parameters": nr_billion_parameters,
                    "now": now,
                },
            )
        ...
        embedding_models = [
            ...
            (
                ...
                "swe",
                "GDM",
                "gdm/multilingual-e5-large-instruct",
                True,
            ),
        ...
        result = await conn.execute(
            text("SELECT id, tenant_id FROM users ORDER BY email")
        )
