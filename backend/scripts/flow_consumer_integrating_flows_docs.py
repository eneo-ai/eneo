...
ENDPOINT_SEQUENCES: tuple[EndpointSequence, ...] = (
    ...
    EndpointSequence(
        ...
        steps=(
            ...
            "For edit review, send `edited_value`: the corrected step output itself, a string for a `text` output step or a JSON object or array for a `json` one, satisfying `output_contract` and staying within the inline UTF-8 limit.",
            "Send nothing else. Runtime-owned payload keys carry over from the stored checkpoint, and the server derives a JSON step's `text` from the edited value so the two encodings cannot disagree.",
            "Offer edit review only when `review_mode` is `edit`, and expect `flow_review_edit_file_backed_unsupported` for an output already stored as a generated file. PDF and DOCX artifact steps support view review, not edit review.",
...
WORKED_EXAMPLE_TRANSCRIPTION_ORIGINAL: dict[str, object] = {
    "text": '{"transcription": "Hello and welcome to the annual review..."}',
    "structured": {"transcription": "Hello and welcome to the annual review..."},
}
WORKED_EXAMPLE_TRANSCRIPTION_EDITED: dict[str, object] = {
    "text": '{"transcription": "Corrected transcription for the annual review."}',
...
WORKED_EXAMPLE_CHECKPOINT_EDIT_REQUEST: dict[str, object] = {
    ...
    "edited_value": WORKED_EXAMPLE_TRANSCRIPTION_EDITED["structured"],
}
...
WORKED_EXAMPLE_HOPS: tuple[WorkedExampleHop, ...] = (
    ...
    WorkedExampleHop(
        ...
        request_intro="Submit the corrected step output value with the revision the reviewer saw. This step is `json`, so the value is the structured object; a `text` step takes a plain string.",
        request_language="json",
...
CLIENT_PITFALL_RULES: tuple[str, ...] = (
    ...
    "**Edit sends the value, not the payload.** `edited_value` is the corrected step output itself: a string when the step's `output_type` is `text`, a JSON object or array when it is `json`. The other shape returns `400 typed_io_validation_failed`. Do not send the checkpoint's `current_payload_json` back; the server rebuilds it, deriving a JSON step's `text` from the value so the two encodings cannot disagree, and carrying every runtime-owned key over from the stored checkpoint. Only a checkpoint whose `review_mode` is `edit` accepts the request (`400 flow_review_edit_not_allowed`), and an output whose text overflowed into a generated file cannot be edited at all (`400 flow_review_edit_file_backed_unsupported`) — approve or reject it instead.",
...
# 6. Correct the transcript. The edit sends the step's value, not the payload:
#    the server rebuilds the payload around it and keeps the runtime-owned keys.
#    This step is json, so the value is the structured object; a text step would
#    take a plain string instead.
#    An output whose text overflowed into a generated file cannot be edited at
#    all — approve or reject it instead.
if jq -e '.current_payload_json | has("text_overflow")' <<<"$CP" >/dev/null; then
  echo "overflow-backed transcript: not editable, approve or reject it instead"
  exit 0
fi
BODY=$(jq -n \
  --argjson revision "$REV" \
  --argjson structured "$(jq -c .current_payload_json.structured <<<"$CP")" \
  --arg text "Corrected transcript." \
  '{expected_checkpoint_revision: $revision,
    edited_value: ($structured + {transcription: $text})}')
...
/**
 * The persisted step output as the server reports it. This envelope is
 * response-only: an edit submits `edited_value` alone and the server rebuilds
 * this object around it, so a client never sends it back. The index signature
 * is here because the runtime owns keys a client is not meant to enumerate.
 */
export interface ReviewPayload {
  text: string;
  structured?: unknown;
  /** Present when the output was too large to inline; the output is then not editable at all. */
...
export class EneoFlows {
  ...
  /**
   * The edit sends the step's own value, not the payload the checkpoint
   * returned: a string for a `text` step, a JSON object or array for a `json`
   * one. The server rebuilds the payload around it, so runtime-owned keys are
   * never the client's to resend.
   */
  editCheckpoint(
    flowId: string,
    runId: string,
    checkpoint: ReviewCheckpoint,
    editedValue: string | Record<string, unknown> | unknown[],
  ): Promise<ReviewCheckpoint> {
    if (checkpoint.review_mode !== "edit") {
      // The flow author opened this step for viewing; approve or reject it.
      throw new Error("checkpoint is view-only: flow_review_edit_not_allowed");
    }
    if (checkpoint.current_payload_json.text_overflow !== undefined) {
      // The output lives in a generated file this endpoint cannot replace.
      throw new Error("file-backed output: flow_review_edit_file_backed_unsupported");
    }
    return this.call(
      "PATCH",
