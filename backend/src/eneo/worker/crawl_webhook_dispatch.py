...
async def dispatch_website(website_id: UUID, redis_client: aioredis.Redis) -> None:
...
async def dispatch_after_commit(website_id: UUID, redis_client: aioredis.Redis) -> None:
    try:
        await dispatch_website(website_id, redis_client)
    except Exception:
        logger.exception("Webhook dispatch failed", extra={"website_id": str(website_id)})


async def reconcile(redis_client: aioredis.Redis) -> None:
    async with sessionmanager.session() as session, session.begin():
        ids = list((await session.scalars(sa.select(Websites.id).where(
            Websites.webhook_pending.is_(True),
            Websites.webhook_token_hash.is_not(None),
            sa.or_(Websites.next_retry_at.is_(None), Websites.next_retry_at <= sa.func.now()),
        ))).all())
    for website_id in ids:
        await dispatch_after_commit(website_id, redis_client)
