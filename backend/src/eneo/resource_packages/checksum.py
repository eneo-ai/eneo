...
def json_object_from_model(model: BaseModel) -> JsonObject:
    return coerce_json_object(model.model_dump(mode="json"))
...
def coerce_json_object(value: object) -> JsonObject:
    ...
    for key, nested_value in mapping.items():
        ...
        result[key] = coerce_json_value(nested_value)
    return result
...
def coerce_json_value(value: object) -> JsonValue:
    ...
    if isinstance(value, list):
        return [coerce_json_value(item) for item in cast(list[object], value)]
    if isinstance(value, Mapping):
        return coerce_json_object(cast(Mapping[object, object], value))
