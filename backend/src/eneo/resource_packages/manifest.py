...
MAX_RESOURCE_PACKAGE_VERSION_LENGTH = 64
...
class ResourcePackageManifestMetadataFields(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)

    package_id: str
    package_version: str = Field(max_length=MAX_RESOURCE_PACKAGE_VERSION_LENGTH)
    name: str
    description: str = ""
    ...
    def validate_package_version(cls, value: str) -> str:
        normalized = value.strip()
        if not normalized or len(normalized) > MAX_RESOURCE_PACKAGE_VERSION_LENGTH:
...
class ResourcePackageManifestMetadata(ResourcePackageManifestMetadataFields):
    ...
    def with_content_checksum(self, content_checksum: str) -> ResourcePackageManifest:
...
class ResourcePackageManifest(ResourcePackageManifestMetadata):
...
def resource_package_filename(manifest: ResourcePackageManifestMetadataFields) -> str:
