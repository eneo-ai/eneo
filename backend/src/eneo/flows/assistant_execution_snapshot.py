...
def build_assistant_execution_snapshot_v2(
    *, assistant: Assistant
) -> FlowPersistedJsonObject:
    """Build v2 for reader tests and the later writer activation; publishers use v1."""
    ...
    surface = AssistantExecutionSurfaceV2.model_validate(
        {
            ...
            "knowledge_refs": [
                ...
                for ref in _assistant_knowledge_snapshot(assistant)
            ],
            "attachments": [
...
def validate_assistant_execution_snapshot(
    *,
    ...
    if type(snapshot.get("schema_version")) is int and snapshot["schema_version"] == 2:
        try:
            parsed = AssistantExecutionSnapshotV2.model_validate(dict(snapshot))
