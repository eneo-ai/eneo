...
def build_plan_proposal_system_prompt(
    *,
    ...
    lines = [
        ...
        "- Each output_fields item is an object with name, field_type, and description. Set name to an ASCII identifier folded from the user's own wording (å/ä→a, ö→o, spaces and dots→underscores); keep key names the user asked for, and put display wording in description.",
        "- Declare requested nested members in children on their object or array field. For an array of objects, children defines each item's fields; children: null produces an array of strings. Instructions and descriptions do not declare nested fields.",
        '- Example output_fields: [{"name":"summary","field_type":"string","description":"Main findings from the source.","required":true,"nullable":false,"children":null},{"name":"findings","field_type":"array","description":"Individual findings.","required":true,"nullable":false,"children":[{"name":"label","field_type":"string","description":"Name of this finding.","required":true,"nullable":false,"children":null}]}]',
