...
class Assistant(Entity):
    ...
    def build_knowledge_catalog(self) -> str:
        ...
        return (
            ...
            "knowledge__search_knowledge left the question unanswered or "
            "only partly answered. In that fallback, pick the suitable tool "
            "and call it yourself: never ask the user for permission to use "
            "a tool that is available to you, and never tell the user to "
            "ask again for something an available tool covers. Questions or "
            "sub-questions about the world outside the attached sources "
            "(other organizations, products, industry practice, current "
            "events) are not covered by the knowledge sources even when a "
            "search returns tangential passages: after the knowledge "
            "search, answer those parts with a suitable external tool (for "
            "example web search) and attribute accordingly.\n"
            ...
            "cover the rest with another suitable tool (rule 2) instead of "
            "stopping at the partial answer. Answers built on another "
            "tool's results must be attributed to that tool, not to the "
            "knowledge sources.\n"
