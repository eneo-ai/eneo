...
from eneo.token_usage.domain.token_usage_models import TokenUsageSummary
from eneo.token_usage.domain.user_token_usage_models import (
    UserTokenUsage,
...
class TokenUsageService:
    ...
    async def get_token_usage(
        ...
        """
        logger.info(
            f"Getting token usage for tenant {self.user.tenant_id} from {start_date} to {end_date}"
        )
    ...
    async def get_user_token_usage(
        ...
        """
        logger.info(
            f"Getting user token usage for tenant {self.user.tenant_id} from {start_date} to {end_date}, page {page}, per_page {per_page}"
        )
        # Default to last 30 days if no dates provided
        if not start_date:
            start_date = datetime.now() - timedelta(days=30)
    ...
    async def get_user_model_breakdown(
        ...
        """
        logger.info(
            f"Getting model breakdown for user {user_id} in tenant {self.user.tenant_id} from {start_date} to {end_date}"
        )
        # Default to last 30 days if no dates provided
        if not start_date:
            start_date = datetime.now() - timedelta(days=30)
    ...
    async def get_single_user_summary(
        ...
        """
        logger.info(
            f"Getting single user summary for user {user_id} in tenant {self.user.tenant_id} from {start_date} to {end_date}"
        )
        # Default to last 30 days if no dates provided
        if not start_date:
            start_date = datetime.now() - timedelta(days=30)
