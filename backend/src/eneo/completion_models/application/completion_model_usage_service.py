...
class CompletionModelUsageService:
    ...
    async def _count_entities_for_type(
        ...
        from eneo.completion_models.constants import get_entity_table
    ...
    async def get_all_models_usage_summary(
        ...
        try:
            ...
            stmt = (
                ...
                .order_by(CompletionModelUsageStats.total_usage.desc().nullslast())
            )
            ...
            logger.error(
                f"Error in get_all_models_usage_summary: {str(e)}", exc_info=True
            )
            raise
    ...
    def _build_tenant_filter_condition(
        ...
        from eneo.completion_models.constants import singular_entity_type
    ...
    async def update_usage_stats_incremental(
        ...
        try:
            self.logger.info(
                f"Starting update_usage_stats_incremental for tenant {tenant_id}, model {model_id}"
            )

            if model_id:
                # Update specific model
                self.logger.info(
                    f"Updating specific model {model_id} for tenant {tenant_id}"
                )
                ...
                for model_row in model_rows:
                    self.logger.info(
                        f"  - Model ID: {model_row.id}, Name: {model_row.name}"
                    )
                ...
                if not model_ids:
                    self.logger.warning(
                        f"No completion models found for tenant {tenant_id}"
                    )
                    return
                ...
                for i, mid in enumerate(model_ids, 1):
                    ...
                    self.logger.info(
                        f"Completed processing model {i}/{len(model_ids)}: {mid}"
                    )
    ...
    async def recalculate_all_usage_stats(self, tenant_id: UUID) -> None:
        ...
        for i, model_id in enumerate(model_ids, 1):
            try:
                ...
                processed_count += 1
                self.logger.info(
                    f"Successfully completed processing model {i}/{len(model_ids)}: {model_id}"
                )
    ...
    async def recalculate_all_usage_stats_in_transaction(self, tenant_id: UUID) -> None:
        ...
        """
        self.logger.info(
            f"Starting recalculate_all_usage_stats_in_transaction for tenant {tenant_id}"
        )

        # Get all models enabled for tenant (settings are now directly on model table)
        stmt = select(CompletionModels.id, CompletionModels.name).where(
            CompletionModels.tenant_id == tenant_id, CompletionModels.is_enabled == True
        )
        self.logger.debug(f"Query for completion models: {stmt}")
    ...
    async def _update_single_model_stats(self, model_id: UUID, tenant_id: UUID) -> None:
        ...
        self.logger.info(
            f"Starting _update_single_model_stats for model {model_id}, tenant {tenant_id}"
        )
    ...
    async def _recalculate_model_stats(self, model_id: UUID, tenant_id: UUID) -> None:
        ...
        self.logger.info(
            f"Starting _recalculate_model_stats for model {model_id}, tenant {tenant_id}"
        )
    ...
    async def _update_stats_counts(
        ...
        self.logger.info(
            f"Starting _update_stats_counts for model {model_id}, tenant {tenant_id}"
        )
        ...
        services_stmt = (
            ...
            .join(Spaces, Services.space_id == Spaces.id)
            .where(
                and_(
        ...
        spaces_stmt = (
            select(func.count(func.distinct(Spaces.id)))
            .select_from(Spaces)
            .join(
        ...
        assistant_templates_result = await self.session.execute(
            assistant_templates_stmt
        )
        ...
        self.logger.info(
            f"Total entities that use model {model_id}: {total_usage} (assistants: {assistants_count}, apps: {apps_count}, services: {services_count})"
        )
        self.logger.info(
            f"Additional metrics tracked separately - Spaces: {spaces_count}, Questions: {questions_count}, Assistant Templates: {assistant_templates_count}, App Templates: {app_templates_count}"
        )
        ...
        verification_total = (
            stats.assistants_count + stats.apps_count + stats.services_count
        )
        ...
        self.logger.info(
            f"Completed _update_stats_counts for model {model_id} - Total configured entities: {stats.total_usage}"
        )
        self.logger.info(
            f"Final verification: assistants={stats.assistants_count}, apps={stats.apps_count}, services={stats.services_count}, total={stats.total_usage}"
        )
