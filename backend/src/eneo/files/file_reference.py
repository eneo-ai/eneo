...
and the file's original bytes are reachable via a signed download URL — i.e. a
reference base URL is configured and the file has a ``storage_key``. Such a
...
def url_only_file_ids(files: Iterable["File"], inline_file_text: bool) -> set[UUID]:
    ...
    return {
        file.id
        for file in files
        if file.file_type == FileType.TEXT and file.storage_key
    }
