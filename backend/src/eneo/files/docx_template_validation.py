...
class DocxTemplateArchiveMetrics:
    entry_count: int
...
def _open_docx_archive(template_bytes: bytes) -> zipfile.ZipFile:
    try:
        return zipfile.ZipFile(io.BytesIO(template_bytes))
...
def _validate_docx_archive_metadata(
    ...
    return DocxTemplateArchiveMetrics(
        entry_count=len(infos),
        uncompressed_bytes=total_uncompressed,
    )


def docx_template_archive_metrics(
    template_bytes: bytes,
    *,
    filename: str,
) -> DocxTemplateArchiveMetrics:
    """Validate cheap archive metadata without expanding the DOCX payload."""

    validate_template_extension(filename=filename)
    with _open_docx_archive(template_bytes) as archive:
        return _validate_docx_archive_metadata(archive)


def validate_docx_template_archive(
    template_bytes: bytes,
    *,
    filename: str,
) -> DocxTemplateArchiveMetrics:
    ...
    with _open_docx_archive(template_bytes) as archive:
        metrics = _validate_docx_archive_metadata(archive)
