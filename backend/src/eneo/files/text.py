...
class TextMimeTypes(MimeTypesBase):
    ...
    # allowlist (see limits.limit_service). Crawling now runs in the external
    # crawler service, which maintains its own download policy, so keep the two
    # concerns separate (changing one never silently affects the other).
    # Supported formats
    MD = "text/markdown"
    TXT = "text/plain"
    PDF = "application/pdf"
