...
class FileService:
    ...
    async def get_download_no_auth(
        ...
        *,
        ...
        async with session.begin():
            ...
            references = await self.repo.get_content_references([file_id])
            reference = self._primary_content(metadata, references)
    ...
    async def get_original_download_no_auth(
        ...
        *,
        ...
        async with session.begin():
            ...
            self._require_token_tenant(metadata, expected_tenant_id)
            references = await self.repo.get_content_references([file_id])
            reference = self._original_content(references, metadata.file_type)
        return await self._open_download(
            metadata,
            reference,
            range_header=range_header,
    ...
    async def _open_download(
        ...
        *,
        ...
        return FileDownload(
            ...
            filename=(
                self._extracted_text_filename(metadata.name)
    ...
    @staticmethod
    def _extracted_text_filename(filename: str) -> str:
    ...
    def _primary_content(
        ...
        reference = select_primary_file_reference(
            file.file_type,
            references,
        )
    ...
    def _original_content(
        ...
        for variant in original_download_variants(file_type):
            reference = FileService._first_reference(references, variant)
            if reference is not None:
                return reference
        raise FileOriginalNotFoundError()
