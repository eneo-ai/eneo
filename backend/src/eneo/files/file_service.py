...
class FileService:
    ...
    def __init__(
        ...
        if user is not None and owner is not None:
            raise ValueError("FileService accepts either a user or an explicit owner")
        self.user = user
        self._explicit_owner = owner
        self.repo = repo
    ...
    async def prepare_document_upload(
        ...
        """Prepare exact document bytes under the configured upload admission."""

        self._owner()
        async with self.protocol.prepare_document_upload(
            upload_file,
            upload_admission_snapshot=self._require_upload_admission(),
        ) as prepared:
            yield prepared

    async def save_prepared_file(self, prepared: PreparedFileUpload) -> FileInfo:
        """Persist a validated prepared File through durable object content."""
    ...
    async def save_generated_file(
        ...
        *,
        ...
        """Persist server-generated bytes without routing them through upload policy."""

        return await self.save_prepared_file(
            PreparedFileUpload(
                name=name,
                file_type=file_type,
                ...
                family = tuple(
                    [
                        await self._capture_prepared_file(
                            ...
                            business_maximum_bytes=self._effective_maximum_bytes(
                                prepared.file_type, snapshot=snapshot, max_size=max_size
                            ),
                        )
                    ]
                    + [
        ...
        *,
        ...
        return await self.repo.add_metadata(
            FileMetadataCreate(
                ...
                owner_service_id=owner.owner_service_id,
    ...
    def _effective_maximum_bytes(
        ...
        *,
        ...
        admission_maximum = cls._maximum_bytes_for_file_type(
            file_type,
            snapshot=snapshot,
        )
        return (
            admission_maximum if max_size is None else min(admission_maximum, max_size)
        )
    ...
    async def get_owned_file_for_key_share(
        ...
        *,
        ...
        metadata = await self.repo.get_by_id_and_owner_for_key_share(
            file_id=file_id,
            owner=self._owner(),
        )
        if metadata is None:
    ...
    async def get_download_no_auth(
        ...
        *,
        ...
        async with session.begin():
            metadata = await self.repo.get_by_id(
                file_id=file_id,
                tenant_id=tenant_id,
            )
            references = await self.repo.get_content_references([file_id])
    ...
    def _owner(self) -> FileOwner:
        if self._explicit_owner is not None:
            return self._explicit_owner
        user = self._authenticated_user()
        if not is_service_api_key(user):
            return FileOwner(
        ...
        if key is None or key.service_principal_id is None:
            raise UnauthorizedException("Service key owner identity is unavailable")
        return FileOwner(
            tenant_id=user.tenant_id,
            owner_type=PrincipalType.SERVICE_KEY,
            owner_service_id=key.service_principal_id,
        )
