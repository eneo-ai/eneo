...
class MCPProxySession:
    ...
    def is_internal_tool(self, prefixed_tool_name: str) -> bool:
        """Return runtime provenance for a registered built-in tool.

        Unknown tools fail closed as external. The provenance comes from the
        ephemeral server entity, never from tenant-controlled names or URLs.
        """
        registered = self._tool_registry.get(prefixed_tool_name)
        return registered is not None and registered[0].is_internal
