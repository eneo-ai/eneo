...
def _require_oauth_enabled_for(auth_scope: str | None) -> None:
    """Reject per_user / per_tenant while the token broker is disabled.

    ``static_bearer`` (the existing behavior) is unaffected. The gate
    keeps rows the runtime cannot serve from being persisted while
...
# ============================================================================
# Tenant MCP OAuth configuration (service account + SSO defaults)
# ============================================================================
...
async def get_mcp_service_account(
    ...
    The plaintext ``client_secret`` is never returned. The broker decrypts
    it on demand for ``per_tenant`` exchanges; this endpoint is a
...
async def set_mcp_service_account(
    ...
    The broker reads these on every ``per_tenant`` exchange and treats
    them as the OAuth client-credentials subject.
    """
...
async def set_mcp_sso_default_target(
    ...
    explicit override. Use this to configure a shared audience or a
    shared API scope once instead of per server.
    """
...
async def clear_mcp_sso_default_target(
    ...
    ``target_resource_or_scope`` fall back to the per-server canonical
    resource from PRM discovery, or fail at exchange time for IdPs that
