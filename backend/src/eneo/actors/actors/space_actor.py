...
class SpaceActor:
    ...
    def tenant_permits(
        self, action: SpaceAction, resource_type: SpaceResourceType
    ) -> bool:
        """The tenant-level permission gate, independent of the space role.

        Applies to every space, personal and shared. Service API keys
        authorize via scope and permission, not user roles, so the gate does
        not apply to them. Reading websites and integration knowledge needs no
        tenant permission; every other permission-gated resource needs its
        mapped permission on the user's role.
        """
        ...
        if permission is not None and has_permission(self.user.permissions, permission):
            return True
        return (
            resource_type
    ...
    def can_perform_action(
        ...
        if not self.tenant_permits(action=action, resource_type=resource_type):
            return False
