"""Persistence for IdP-issued refresh / access tokens (Phase 2).
...
so the broker (Phase 3) can mint MCP-audience tokens via RFC 8693 token
exchange (Keycloak) or OBO (Entra) without sending the user through a
...
- :meth:`refresh_idp_token` — exchanges the stored refresh token for a
  ...
  rotates the refresh token if the IdP returns a new one. On
...
class StoredIdpTokens:
    ...
    The broker (Phase 3) consumes this; nothing else should.
    """
...
class OidcTokenStore:
    ...
    async def upsert(
        ...
        *,
        ...
        insert_stmt = sa.dialects.postgresql.insert(IdpUserTokens).values(**values)
        update_set = {
            "tenant_id": insert_stmt.excluded.tenant_id,
            "idp_subject": insert_stmt.excluded.idp_subject,
            "refresh_token_ciphertext": insert_stmt.excluded.refresh_token_ciphertext,
            "access_token_ciphertext": insert_stmt.excluded.access_token_ciphertext,
            "access_token_expires_at": insert_stmt.excluded.access_token_expires_at,
            "scopes_granted": insert_stmt.excluded.scopes_granted,
            "key_version": insert_stmt.excluded.key_version,
            "last_refresh_at": insert_stmt.excluded.last_refresh_at,
            "revoked_at": None,
    ...
    async def refresh_idp_token(
        ...
        *,
        ...
        Used by the broker (Phase 3) when the cached access_token has
        expired. On ``invalid_grant`` the row is marked revoked and the
        caller receives :class:`IdpRefreshFailedError` — the user must
        re-authenticate.
        """
        ...
        access_expires_at = (
            datetime.now(timezone.utc) + _seconds_to_timedelta(int(expires_in))
