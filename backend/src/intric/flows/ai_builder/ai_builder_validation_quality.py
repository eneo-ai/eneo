...
def lint_all_previous_steps_overuse(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
    count = sum(1 for s in spec.steps if s.input_source == InputSource.ALL_PREVIOUS_STEPS)
    if count > 1:
        result.add_warning(
            step_ref=None,
...
def lint_vague_step_names(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
...
def lint_multi_goal_prompts(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
    for step in spec.steps:
        instructions_lower = step.assistant_spec.instructions.lower()
        for indicator in _MULTI_GOAL_INDICATORS:
            if indicator in instructions_lower and len(step.assistant_spec.instructions) > 300:
                result.add_warning(
                    step_ref=step.plan_step_ref,
...
def lint_single_step_flow(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
    if len(spec.steps) == 1:
        result.add_warning(
            step_ref=None,
...
def lint_json_output_without_contract(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
...
def lint_contract_fields_without_descriptions(
    ...
    for step in spec.steps:
        ...
        if not isinstance(props, dict):
            continue
        missing = [k for k, v in props.items() if isinstance(v, dict) and "description" not in v]
        if missing:
            result.add_warning(
                step_ref=step.plan_step_ref,
...
def lint_contract_instruction_alignment(
    ...
    for step in spec.steps:
        ...
        if not property_names:
            continue
        missing = [name for name in property_names if name.casefold() not in instructions]
...
def lint_unused_form_fields(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
    declared_fields = {
        field.name.strip()
...
def lint_all_previous_with_specific_refs(spec: FlowDraftSpecCore, result: SpecValidationResult) -> None:
    step_refs = {step.plan_step_ref: index + 1 for index, step in enumerate(spec.steps)}
    form_fields = {
        field.name.strip()
        for field in (spec.form_fields or [])
    ...
    for step in spec.steps:
        ...
        if question is None:
            continue
        refs = analyze_template(question, step_refs=step_refs, form_field_names=form_fields)
...
def _iter_step_templates(step) -> list[str]:
...
def _question_binding(input_bindings: dict | None) -> str | None:
