...
class AdminService:
    ...
    async def get_tenant_users(self):
        ...
        logger.info(
            f"Successfully retrieved {len(users)} users for tenant {self.user.tenant_id}"
        )
        return users
    ...
    async def register_tenant_user(self, user: UserAddAdmin):
        ...
        if (
            ...
            and existing_user_by_email.tenant_id == self.user.tenant_id
        ):
            logger.warning(
                f"Email {user.email} already exists in tenant {self.user.tenant_id}"
            )
    ...
    async def update_tenant_user(self, username: str, user: UserUpdatePublic):
        ...
        if user_in_db.tenant_id != self.user.tenant_id:
            logger.warning(
                f"Cross-tenant access attempt: admin {self.user.username} tried to access user {username} in tenant {user_in_db.tenant_id}"
            )
            raise BadRequestException(
                "You do not have access to remove or add users on another tenant"
            )
        ...
        if user.email is not None and user.email != user_in_db.email:
            ...
            if (
                existing_user_by_email
                and existing_user_by_email.tenant_id == self.user.tenant_id
                and existing_user_by_email.id != user_in_db.id
            ):
                logger.warning(
                    f"Email {user.email} already exists in tenant {self.user.tenant_id}"
                )
        ...
        logger.info(
            f"Successfully updated user {username} in tenant {self.user.tenant_id}"
        )
        return result
    ...
    async def delete_tenant_user(self, username: str):
        ...
        if user_in_db.tenant_id != self.user.tenant_id:
            logger.warning(
                f"Cross-tenant deletion attempt: admin {self.user.username} tried to delete user {username} in tenant {user_in_db.tenant_id}"
            )
            raise BadRequestException(
                "You do not have access to remove or add users on another tenant"
            )
        ...
        if user_in_db.id == self.user.id:
            logger.warning(
                f"Self-deletion attempt: admin {self.user.username} tried to delete themselves"
            )
    ...
    async def deactivate_tenant_user(self, username: str):
        ...
        if user_in_db.tenant_id != self.user.tenant_id:
            logger.warning(
                f"Cross-tenant deactivation attempt: admin {self.user.username} tried to deactivate user {username} in tenant {user_in_db.tenant_id}"
            )
            raise BadRequestException(
                "You do not have access to manage users on another tenant"
            )

        if user_in_db.id == self.user.id:
            logger.warning(
                f"Self-deactivation attempt: admin {self.user.username} tried to deactivate themselves"
            )
    ...
    async def reactivate_tenant_user(self, username: str):
        ...
        user_in_db = await self.user_repo.get_user_by_username(
            username, with_deleted=True
        )
        ...
        if user_in_db.tenant_id != self.user.tenant_id:
            logger.warning(
                f"Cross-tenant reactivation attempt: admin {self.user.username} tried to reactivate user {username} in tenant {user_in_db.tenant_id}"
            )
            raise BadRequestException(
                "You do not have access to manage users on another tenant"
            )
        ...
        if user_in_db.deleted_at is not None:
            ...
            logger.info(
                f"Clearing deletion timestamp for user {username} during reactivation"
            )

        result = await self.user_service.update_user(
            user_in_db.id, UserUpdatePublic(state=UserState.ACTIVE)
        )

        logger.info(
            f"Successfully reactivated user {username} in tenant {self.user.tenant_id}"
        )
        return result
    ...
    async def get_inactive_tenant_users(self):
        ...
        inactive_users = [
            user for user in all_users if user.state == UserState.INACTIVE
        ]
        ...
        logger.info(
            f"Successfully retrieved {len(inactive_list)} inactive users for tenant {self.user.tenant_id}"
        )
    ...
    async def get_deleted_tenant_users(self):
        ...
        logger.info(
            f"Successfully retrieved {len(deleted_list)} deleted users for tenant {self.user.tenant_id}"
        )
        return deleted_list
    ...
    async def get_tenant_user(self, username: str):
        ...
        logger.info(
            f"Successfully retrieved user {username} in tenant {self.user.tenant_id}"
        )
        return user_in_db
    ...
    async def validate_admin_permission(self):
        ...
        """
        logger.info(
            f"Admin permission validated for user {self.user.username} in tenant {self.user.tenant_id}"
        )
