...
from sqlalchemy import CursorResult, and_, delete, func, select, true, update
...
class CompletionModelMigrationService:
    ...
    async def _migrate_entity_type(
        ...
        result = await self.session.execute(stmt)
        migrated_count = cast("CursorResult[Any]", result).rowcount or 0
    ...
    async def _migrate_spaces(
        ...
        migrated_count = cast("CursorResult[Any]", delete_result).rowcount or 0
