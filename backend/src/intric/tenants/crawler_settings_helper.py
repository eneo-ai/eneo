...
SELF_SERVICE_CRAWLER_SETTING_KEYS: frozenset[CrawlerSetting] = frozenset(
    {
        ...
        "retry_times",
        "closespider_itemcount",
        # These tenant-scoped runtime knobs are read at crawl start, so a
        # tenant-admin change affects only new crawls and cannot mutate an
        # already-running crawl's runtime contract.
        ...
        # crawl_page_batch_size (deferred because retention and cost should be
        # explained through observability before operators get another free
        # tuning knob).
        "crawl_max_length",
        "crawl_stale_threshold_minutes",
        "queued_stale_threshold_minutes",
