...
async def update_space(
    ...
    if (
        update_space_req.description is not None
    ...
    if is_provided(security_classification):
        old_sc = (
            old_space.security_classification.name
            if old_space.security_classification
            else None
        )
        new_sc = (
            space.security_classification.name
...
async def get_spaces(
    include_applications: bool = Query(
        default=False, description="Includes published applications on each space"
    ),
    include_personal: bool = Query(
        default=False, description="Includes your personal space"
    ),
    container: Container = Depends(get_container(with_user=True)),
):
    service = container.space_service()
    ...
    spaces = await service.get_spaces(
        include_personal=include_personal, include_applications=include_applications
    )
    spaces = [
