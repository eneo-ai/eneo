...
CRG was used as a reducer after the graph became available for this root. `get_minimal_context` reported 31,576 nodes / 281,929 edges across 3,772 files. `find_large_functions(kind=File, file_path_pattern=backend/src/eneo/flows, min_lines=800)` confirmed the main size pressure points, including `backend/src/eneo/flows/runtime/executor.py:1` at 2,223 lines, `backend/src/eneo/flows/api/flow_models.py:1` at 2,091 lines, and the Builder control-plane modules `ai_builder_critic_invariants.py:1`, `ai_builder_step_skeleton.py:1`, `ai_builder_framework_policy.py:1`, `ai_builder_repo.py:1`, and `ai_builder_router.py:1` above 1,300 lines. CRG is not a proof source by itself: every delete or merge must be re-verified at execution time with fresh `rg` source evidence, and CRG caller/importer checks are an optional second reducer when available.
...
| Builder frontend/backend error enums | Frontend manual sets at `frontend/apps/web/src/lib/features/flows/ai-builder/aiBuilderError.ts:16` and `:27`; generated backend enums in `frontend/packages/eneo-js/src/types/schema.d.ts:8593` and `:8659`. | Frontend may need explicit local-only `network` and `client` categories; server categories/phases should come from generated types. | Generated OpenAPI client types, with explicit frontend-only local extensions for `network` and `client`. | Reuse generated enums; keep local-only categories as named superset. | Builder/frontend state track. |
| Webhook delivery state | JSON payload writers at `backend/src/eneo/flows/runtime/step_execution_runtime.py:505` and `backend/src/eneo/flows/runtime/step_result_builder.py:128-130`; export scrub at `backend/src/eneo/flows/flow_run_export_json.py:1087`; relational delivery state is the intended owner per PG-11. | JSON mirror is stale-prone and leaks error-string shape into output payloads; relational delivery state is the durable owner. | `flow_run_webhook_deliveries` relational state and delivery repository/export projection. | Delete JSON mirror after confirming no frontend/manual consumer reads the flags. | PG-11. |
| Template asset/file dual identity | Authoring writes both keys at `backend/src/eneo/flows/application/flow_service.py:912-913`; runtime requires `template_file_id` at `backend/src/eneo/flows/runtime/template_fill_runtime.py:293-298` and cross-checks optional `template_asset_id` at `:350-369`; asset service owner starts at `backend/src/eneo/flows/flow_template_asset_service.py:23`. | `template_file_id` is still runtime load-bearing, so this is staged duplication, not a direct delete. | `FlowTemplateAssetService` and asset-backed runtime resolution. | Stage migration: asset-first runtime resolution, backfill, then delete `template_file_id` fallback. | PG-D4 / authoring track. |
| Template asset presentation capability flags | Domain flags at `backend/src/eneo/flows/domain/flow.py:127-130`; service projection mutates them at `backend/src/eneo/flows/flow_template_asset_service.py:137-150`. | Capability flags are API presentation derived from actor access, not persisted asset identity. | API/readiness projection, not domain entity. | Move presentation flags out of domain while preserving API response shape. | PG-15. |
| Evidence export summary | Legacy open dict in `backend/src/eneo/flows/api/flow_models.py:2076`; typed summary at `backend/src/eneo/flows/api/flow_models.py:2077`; builder emits both in `backend/src/eneo/flows/flow_run_export_json.py:137-159`; generated open dict leaks to `frontend/packages/eneo-js/src/types/schema.d.ts:15846-15850`. | Legacy summary still carries fields that must reach typed parity before deletion. | `EvidenceExportSummary` plus versioned export contract. | Expand typed parity, deprecate open dict, then delete legacy `summary`. | PG-D4 / API contract track. |
...
- Delete or wire `eneo.observability.failure_events` into real terminal failure/dead-letter paths. Tests-only logging contracts are dead architecture unless operators can actually rely on them.
...
| `module_registry.metadata_json` ownership | Flow data-model owner | C4 JSONB score and registry guard | Resolved locally: exact inventory found only Flow table/migration/docs/test references and no platform writer/reader. | Delete/squash the unused Flow-adjacent table and the deferred JSONB escape hatch; reintroduce a platform-owned table only with a real product owner. |
...
This addendum records the state after C9.0-C9.6. It does not replace the existing decision register or stop rules.
...
| Builder repair/fallback first-release posture | C9.4 records no branch as delete-ready without real branch-value data. C9.5 searched available local/log/artifact sources and stopped no-data with no commit. C9.6 accepts the current bounded branches for first release while keeping branch value unproven. | Do not run another repo-only pruning or no-data search slice. Do not prune until real branch-value data, a production-like controlled eval, or an explicitly scheduled live-eval artifact proves a branch is dead, harmful, redundant, or replaced. |
| Next code slice | PG-10b remains the next separate code slice after C9.6. | Scope PG-10b to main-app FastAPI `RequestValidationError` -> `GeneralError`, generated-client/OpenAPI impact, Flow endpoint tests, representative non-Flow endpoint tests, and SCIM/sub-app compatibility review. Do not implement it inside Builder governance work. |
...
| 9 | Remaining targeted JSONB guardrails | Runtime payloads, review checkpoint payloads, provenance, and package imports stay JSONB unless future query/lifecycle/FK pressure appears. |
