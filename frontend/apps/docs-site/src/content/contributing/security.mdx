...
Branch policy, the PR's **User-facing** section and the legacy-name check run
before the expensive checks. If a prerequisite fails, CI does not start the
test and build jobs. After those prerequisites pass, the selected checks run
in a fail-fast matrix: one failure cancels queued and running sibling checks.
E2E container cleanup still runs after cancellation, and the final `CI` check
reports failure or cancellation rather than accepting an incomplete run.
Fix the reported error before rerunning; cancelled checks have not passed.
Separate workflows, including Dependency Review and documentation publication,
...
Ruff currently checks `backend/src/eneo`. Tests and Alembic are not part of the Ruff gate yet.
