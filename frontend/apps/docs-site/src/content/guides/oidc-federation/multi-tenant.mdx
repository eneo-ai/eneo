...
<Callout type="warning">
  Multi-tenant federation **requires** `FEDERATION_ENABLED=true`. In this mode
  the `OIDC_*` environment variables are ignored - every tenant must be
  configured through the sysadmin API, and a tenant without a configuration
  cannot sign in.
</Callout>
...
<Callout type="info">
  `FEDERATION_PER_TENANT_ENABLED` is a deprecated alias for
  `FEDERATION_ENABLED`. It still works but logs a warning at startup; when both
  are set, `FEDERATION_ENABLED` wins.
</Callout>
...
<Callout type="info">
  The test does **not** fetch the JWKS, attempt actual authentication or
  validate client credentials - it only checks that the discovery endpoint is
...
<Callout type="info">
  When users report authentication issues, ask them for the correlation ID shown
  on the error screen. This allows you to quickly locate all related log
  entries.
</Callout>
...
<Callout type="warning">
  Never manually delete or modify state entries in production. This could cause
  active authentication flows to fail.
</Callout>
...
<Callout type="warning">
  **Back up your `ENCRYPTION_KEY`** securely. If you lose it, you'll need to
  re-register all tenant IdPs and reconfigure their client secrets.
</Callout>
