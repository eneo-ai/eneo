...
<Callout type="warning">
  Multi-tenant federation **requires** `FEDERATION_PER_TENANT_ENABLED=true`.
  Configuration is managed via API only - environment variables are not used for
  individual tenant IdPs.
</Callout>
...
Use the federation API to provide the tenant's OIDC configuration. `PUT` is the full-definition endpoint for a new federation setup or a full replacement of the current one.
...
<Callout type="info">
  The test does **not** attempt actual authentication or validate client
  credentials - it only checks that the IdP is reachable and properly
  configured.
</Callout>
...
```bash
curl -X PATCH "https://api.your-domain.com/api/v1/sysadmin/tenants/{tenant_id}/federation" \
  -H "X-API-Key: your-super-admin-api-key" \
  -H "Content-Type: application/json" \
  -d '{
...
To replace the whole federation definition, continue to use `PUT` and send the full required payload.
