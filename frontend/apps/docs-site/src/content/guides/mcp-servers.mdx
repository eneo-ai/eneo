...
## Using MCP Tools in Flows and AI Builder

Flows can attach MCP access to the assistant configuration used by an individual step. Prefer enabling only the tools a step needs. Server-level access is useful when a step genuinely needs the whole server surface, but tool-level access is easier to review and safer by default.

AI Builder can see the MCP servers and tools enabled in the current space. During planning it reads tool names, descriptions, and metadata so it can decide whether a step should use external access. It does **not** execute MCP tools while drafting a plan. Tool execution happens only when a published Flow run reaches the step that has MCP access.

Good MCP descriptions improve plan quality. Describe what the tool reads, what it writes, when it should be used, and any side effects. If a tool writes to an external system, make that explicit.

<Callout type="info">
  Keep knowledge retrieval and MCP execution on separate Flow steps when both
  are needed. That separation makes the source of each result easier to inspect
  and keeps retrieval context distinct from external tool execution.
</Callout>
