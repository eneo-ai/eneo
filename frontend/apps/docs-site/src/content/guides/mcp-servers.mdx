...
<Callout type="info">
  This guide covers administrator setup for external MCP servers. For the deeper
  architecture—including knowledge retrieval modes, Eneo's built-in loopback MCP
  servers, and how internal and external tools interoperate—see [Knowledge
  Retrieval and MCP Interoperability](/docs/knowledge-retrieval-and-mcp).
</Callout>
...
<Callout type="info">
  On successful creation, Eneo connects to the server and stores the tools
  visible to the creating admin. These initial tools are enabled by default
  because the admin is explicitly adding the server. When identity forwarding is
  enabled, other users may have a different catalog; their additional tools
  enter the approval flow described below.
</Callout>
...
or matching. Headers are sent during connection checks, tool discovery, and
tool calls.
...
<Callout type="warning">
  Disabling a tool in admin removes it globally for all spaces. To control tools
  per-space, manage them in the space settings instead.
</Callout>
...
<Callout type="info">
  **Why approval?** The approval step is a security measure. It prevents a
  compromised or misbehaving MCP server from silently injecting or modifying
  tool definitions. An admin must review and explicitly approve every change
...
## MCP boundary for Flows and AI Builder

MCP remains available to standalone assistants and conversations. Flow and AI
Builder do not support MCP servers or tools at authoring, publish, runtime, or
package boundaries.

<Callout type="warning">
  Do not configure MCP on an assistant managed by a Flow. Use supported Flow
  inputs for required data and the durable terminal webhook output for external
  delivery.
</Callout>
...
Eneo opens a fresh MCP connection for every completion and does not persist or
resume MCP protocol sessions. This follows the direction of the MCP
specification (revision 2026-07-28 removes protocol-level sessions): a server
that needs state to span several user turns should mint an explicit handle and
exchange it through tool calls, not rely on transport-session affinity.

The pattern:

1. The tool that creates the state (for example, ingesting an uploaded file)
   returns a handle in its result text, such as `workspace_id: ws_8a3b`.
2. Later tools accept that handle as an ordinary parameter. Eneo replays
   earlier tool calls and results to the model on every turn, so the model can
   read the handle from turn 1 and pass it on turn 5.
3. The server binds each handle to the forwarded identity headers
   (`X-Eneo-User-Id`, `X-Eneo-Tenant-Id`) and rejects calls where they do not
   match. Possession of a handle alone must never grant access: handles appear
   in conversation history and logs by design.
4. The server expires held state with its own TTL. Eneo does not signal
   conversation deletion to remote servers.

Two reliability details for server authors: repeat the handle in every tool
result (long conversations drop the oldest turns from model context first, so
a handle mentioned only once can age out), and state in the tool descriptions
that the model should reuse the handle from earlier results.
