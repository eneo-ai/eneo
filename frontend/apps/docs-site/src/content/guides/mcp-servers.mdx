...
<Callout type="info">
  On successful creation, Eneo automatically connects to the server and
  discovers all available tools. These initial tools are added as **enabled by
  default** without requiring approval, since the admin is explicitly adding a
  trusted server.
</Callout>
