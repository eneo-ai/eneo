...
Web widget visitors are anonymous. When an assistant is published as a web
widget, its MCP servers receive the visitors' tool calls without any `X-Eneo-*`
headers, and no call waits for an approval. Check that each server's tools are
acceptable for anyone on the public site before the widget is activated; see
