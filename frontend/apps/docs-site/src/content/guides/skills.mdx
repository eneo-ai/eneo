...
Remove all bindings from Assistants, Apps, and Personal Chat first, then wait for
any queued or running App jobs to finish. If any selected Skill is still in use,
the whole selection stays unchanged. The confirmation lets you exclude Skills
in use and review the remaining selection. A concurrent change can also refuse
the request; review the message before retrying.
...
| Remove          | Retires an unused Skill from the catalogue and retains its history      |
