...
1. Open a Space and select **Skills**. Tenant administrators can instead open
   **Organisation → Skills** for a reusable organisation Skill.
...
| Status                      | Meaning                                                                                                           |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Draft                       | No version has been published. Only tenant administrators can see it.                                             |
...
Organisation authoring and reuse have separate boundaries:

| Permission          | What it allows                                                                                                     |
| ------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Use Skills          | Preview approved organisation Skills and attach available Skills to resources the person may edit                  |
| Manage + Use Skills | Create, revise, compare, restore, and delete local Skills in Spaces where the person has the matching Space access |
| Tenant admin        | Author, revise, publish, unpublish, republish, and delete eligible organisation Skills; configure Personal Chat    |

Only the predefined **Owner** role receives **Use Skills** and **Manage Skills**
by default. Owners can delegate either capability through a custom role when the
organisation is ready to broaden access. The predefined User and AI Configurator
roles do not receive Skill permissions automatically. Existing custom roles
retain the Skill permissions already stored on them; because older releases did
not record how each grant was assigned, administrators should review custom roles
after upgrading.
...
Tenant admins manage organisation-wide Personal Chat behavior in the existing
Personal Chat administration page. They can select ordered, published
organisation Skill versions that supplement the base or enforced Personal Chat
instructions.
...
```mermaid
flowchart TB
    Admin["Tenant admin"] --> Draft["Organisation draft and version history"]
    Draft --> Review["Admin review and publication"]
    Review --> Approved["Exact approved version"]
    Approved --> Catalogue["Organisation catalogue"]
    Catalogue --> Builder["Use Skills + parent edit"]
...
Only a tenant admin. The same admin-owned workflow creates, revises, restores,
publishes, and unpublishes organisation Skills. **Manage Skills** governs local
Space authoring; it does not grant access to the organisation authoring area.
