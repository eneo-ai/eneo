...
An Assistant can use attached knowledge and MCP servers in the same request;
both travel through one tool catalogue at runtime. When Eneo validates whether
a saved on-demand Skill can fit, it includes every stored, enabled and approved
external MCP tool schema available to the Assistant—not a user-specific
subset—for Assistants without attached knowledge. For an Assistant that has
knowledge, the saved external MCP schemas are not counted in that fit check.
The check reserves the same question headroom used for attachments instead of
letting saved Skill context fill the model window. Live discovery, user
permissions, approvals, and tool availability can still change later, so
...
and **Show more resources** loads the next page. Search by resource, Space or
owner name, and filter by type or update status, to find one resource among
hundreds; the totals above the table always describe the whole Skill.

Rows in a personal Space name their owner. A row the administrator cannot open
(another user's personal Space, or a Space they are not a member of) is shown
as plain text instead of a link.

Tick rows (at most 100 at a time) to act on them without visiting each one:

- **Detach** removes the Skill from the selected Assistants and Apps only. The
  Skill, every unselected binding, and Personal Chat stay as they are. The
  audit log records which resources lost the binding.
- **Update to published version** moves the selected stale rows to the
  published version in one chunk. Rows already on it are skipped, each
  Assistant is validated first, and queued App runs are not affected.
...
- when audit logging for Skill binding updates is enabled, each chunk that
  changes anything writes one audit event with the run identity and the counts
  of moved, skipped, and excluded Assistants.

### Moving App bindings to the published version

App bindings are eager: every new App run includes the exact Skill versions
attached to the App. When publishing a version, a tenant admin can explicitly
include Apps in the binding update. This option is off by default because it
changes which Skill version future App runs use.

The App update follows the same bounded, restartable pattern as the Assistant
update, with two important guarantees:

- each App is checked against the published version before its binding moves;
  Apps whose resulting context is too large stay on their previous version;
- runs that are already queued or running keep the Skill snapshot captured when
  they were created. Only new runs use the updated binding.

Concurrent App or binding edits are skipped instead of overwritten. Stopping
between chunks is safe, and starting again picks up bindings that are still
behind. Unpublishing, publishing a different version, or blocking the Skill
ends the update and requires a fresh run.
...
Only tenant admins author, revise, publish, unpublish, restore versions of, and
remove organisation Skills. Removed Skills remain readable by admins.
...
| Removed           | Admin-only history; no editing, publishing, or new attachments         |
...
## Remove organisation Skills

Open **Skills** in the organisation Space. Each current Skill shows the number
of Assistants and Apps with a saved binding, the number of distinct Spaces
containing those resources, and whether Personal Chat uses it. These are
configuration counts, not run counts. Select a count to inspect the bindings.

Use **Remove** on a row or **Remove skill** in its detail page. To remove several,
select up to 100 Skills and choose **Remove selected**. Review the named Skills
in the confirmation before continuing. A selected Skill can be a draft,
published, unpublished, or execution-blocked.

If a selected Skill is still used by Assistants, Apps, or Personal Chat, the
confirmation says so and offers to remove those bindings in the same step. The
option is on by default: the Skill and every binding are removed together, in
one transaction, and the audit event for the Skill lists each Assistant, App,
and Personal Chat policy that lost it. Assistants and Apps continue without the
Skill; a Personal Chat change applies to everyone in the organisation. Untick
the option to keep today's stricter behaviour, where the whole selection stays
unchanged while any Skill is in use and you can exclude the Skills in use
instead.

Queued or running App jobs still refuse removal, with or without the option,
so a job never loses its Skill mid-run. A concurrent change can also refuse the
request; review the message before retrying.

Removed Skills leave the current and published catalogues. Under **Removed**,
admins can still read their instructions and version history. Completed App-run
provenance and audit events remain intact. Removal cannot be undone: editing,
publishing, and restoring versions are unavailable. An existing execution block
can still be closed with an auditable reason. A new Skill may reuse a removed
Skill's identifier, with a separate identity and history.

Choose the control that matches the intent:

| Control         | Effect                                                                  |
| --------------- | ----------------------------------------------------------------------- |
| Unpublish       | Stops new attachments; existing exact-version bindings remain usable    |
| Block execution | Stops runs that use the Skill; keeps bindings available for remediation |
| Remove          | Retires a Skill from the catalogue, optionally detaching it everywhere  |
...
| Skill is still attached             | The Skill is attached to an Assistant, an App, or the organisation Personal Chat policy, and the removal was sent without the detach option                                    | Tick the detach option in the confirmation, or remove the attachments |
| App run in progress                 | A queued or running App run needs the Skill                                                                                                                                   | Wait for the run to finish, then remove the Skill                    |
...
### When can I remove a Skill?

Whenever no queued or running App job needs it. The confirmation offers to
detach the Skill from every Assistant, App, and Personal Chat policy in the same
step (on by default), so prior manual detachment is not required. Organisation
Skills retain their instructions, immutable versions, and audit history under
**Removed**, including Skills that were published before removal. Local Space
Skills still use permanent deletion once their bindings and active App jobs have
been cleared.
