...
- User group membership: users added to or removed from a user group
...
- Go to **Admin** and use the **Enable Audit Logging** toggle on the admin overview page to enable or disable logging for your tenant
...
- **Space oversight**: Space Joined (Oversight), Space Left (Oversight), Space Member Added (Oversight), Space Member Role Changed (Oversight) and Space Member Removed (Oversight).
- **User groups**: User Group Member Added and User Group Member Removed. A user group's role in a space gives its members that space's content, so each entry lists the spaces where the group has a role. Changes that your identity provider makes to groups through SCIM are logged as SCIM Group Updated, which follows the audit configuration.
- **Web widget publishing**: Widget Activation Request Sent Back, Widget Activated, Widget Paused and Widget Archived.

These actions are recorded even when audit logging is turned off, whatever the category and action toggles say. In the configuration their checkboxes stay checked, cannot be changed and are marked **Always logged**, each category shows how many of its actions are always logged, and switching a whole category off leaves them on. The admin overview page says so under the **Enable Audit Logging** toggle. Each entry is saved together with the change it records: if the entry cannot be saved, the change is not made. These entries are kept for at least 365 days, even when the retention period is shorter; a longer retention period applies to them as to other entries. Exports include them.
...
The reason an administrator gives for joining a space, and the message sent back with a widget request, are stored in the entry's JSON metadata, not in its description. To review who has joined which spaces, filter on the action **Space Joined (Oversight)**. See [Space Oversight](/guides/space-oversight#what-the-audit-log-records) for what each entry records.
...
Audit log retention is configured per tenant, in days, on the Audit Logs page and enforced by a daily purge job. Logs older than the retention period are permanently deleted, except that [always-logged actions](#always-logged-actions) are kept for at least 365 days.
