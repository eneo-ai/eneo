...
- **Space oversight**: Joined Space Through Oversight, Left Space (Oversight), Member Added by Organisation Administrator, Role Changed by Organisation Administrator, and Member Removed by Organisation Administrator.
- **User groups**: User Group Member Added and User Group Member Removed. A user group's role in a space gives its members that space's content, so each entry lists the spaces where the group has a role.
...
The reason an administrator gives for joining a space, and the message sent back with a widget request, are stored in the entry's JSON metadata, not in its description. To review who has joined which spaces, filter on the action **Joined Space Through Oversight**. See [Space Oversight](/guides/space-oversight#what-the-audit-log-records) for what each entry records.
