...
> **Important:** This guarantee depends on the service account or tenant app being configured and active. If you import into a shared space before centralized authentication is configured — or after the tenant app is deactivated — Eneo falls back to the importing administrator's personal OAuth token, which re-introduces person-dependency. Always complete the integration setup before importing into shared spaces.
...
> **Note:** The `/api/v1` prefix comes from the `API_PREFIX` setting (default `/api/v1`); the paths above are exactly what Eneo registers with Microsoft Graph. If your gateway rewrites or strips path prefixes, allowlist the path it actually forwards to Eneo.
