...
- `Group.Read.All` – categorize SharePoint sites by Microsoft Teams membership, so sites belonging to teams the user is a member of are grouped under "My teams" at the top of the site list. Only the user's own group memberships are looked up – Eneo never enumerates the tenant's full group list. Without this permission, all sites are shown without categorization.
...
> **Note:** The `Group.Read.All` *application* permission is not needed. Site categorization is based on the signed-in user's own team memberships, which does not exist for app-only authentication – Tenant App previews list sites without categorization. The *delegated* `Group.Read.All` permission above is still requested on every user sign-in.
