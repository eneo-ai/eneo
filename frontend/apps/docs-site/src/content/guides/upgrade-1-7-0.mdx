...
The backend port is not published on the host in the reference stack, so query the health endpoint through your public domain:

```bash
curl -s https://your-domain.com/api/healthz | python3 -m json.tool
```

Or run the check inside the backend container, where `localhost:8000` is the API:
