...
3. **Mark the model for live transcription.** Look up the model's `id` with `GET /api/v1/transcription-models/`, then set `supports_realtime`:
   ...
   Only a provider of type vLLM qualifies. For any other provider, Eneo answers `400`. You can also send `"supports_realtime": true` when you create the model with `POST /api/v1/admin/tenant-models/transcription/`.
