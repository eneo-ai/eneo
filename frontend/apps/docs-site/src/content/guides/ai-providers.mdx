...
Eneo is provider-agnostic and routes all AI requests through **LiteLLM**, which supports 100+ providers out of the box. Common providers include:
...
Any provider supported by LiteLLM can be used — see the [LiteLLM providers list](https://docs.litellm.ai/docs/providers) for the full list. You can also add custom providers by implementing a provider class (see the [LiteLLM Provider README](/backend/src/intric/ai_models/litellm_providers/README.md)).
...
3. Navigate to **Model deployments** → **Create**
...
Run AI models on your own infrastructure or connect any OpenAI-compatible API endpoint. Since Eneo routes through LiteLLM, any provider with an OpenAI-compatible API can be integrated using the `hosted_vllm` provider type or by adding a custom provider class.
