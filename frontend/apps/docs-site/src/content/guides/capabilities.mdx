...
- **Three gates, all required.** The space must make the function available,
  ...
  model for that turn.
- **Web widget visitors have no role.** For an assistant published as a web
  widget, anonymous visitors get web search whenever the space and the
  assistant enable it, through the tenant's default source when that is an
  external search server. Removing the permission from every role does not stop
  it. Visitors never get image generation. See
  [What visitors can reach](/guides/embed-widget#what-visitors-can-reach).
...
- The tools popover in the chat input lists each enabled function with an
  on/off toggle. Each new conversation starts with its assistant's current
  defaults, including the organisation's governance defaults in Personal Chat.
  Tool choices and the approval preference belong to that conversation. They
  are stored on the server and restored when it is reopened or refreshed,
  including on another browser or device signed in to the same account.
  A function's toggle follows its purpose, so changing its provider does not
  change the saved choice. A newly available tool uses its current default.
- In Personal Chat, the completion model and, where policy allows it,
  reasoning effort also belong to the conversation. Changing them does not
  change the default assistant or later conversations. A model change resets
  reasoning effort to the organisation's default for the selected model.
  Ordinary assistant chats use their configured model and reasoning settings;
  group chats use the responding assistant's settings. Group chats do not
  offer per-call tool approval.
- Switching models after a conversation has messages asks for confirmation and
  recommends starting a new conversation to avoid mixing replies from different
  models. You can still choose to change models and continue in the same
  conversation. The new model may fit less history or support different tools
  and images. Messages remain stored; the backend prepares
  context for the selected model on each request. Changing the completion
  model does not reindex knowledge sources, which use a separate embedding
  model. Provider prompt caching can affect response time and cost, but a cache
  miss does not delete saved messages or require a new conversation. The model
  that produced each answer remains recorded with that message.
- Changes in an existing conversation are saved immediately, before the next
  message. A draft is saved together with its first question; refreshing an
  unsent draft restores current defaults. Older conversations without saved
  settings initialize once from current defaults when opened. Old browser
  preferences cannot reliably be attributed to a conversation and are not imported.
- Saved choices cannot grant access removed by an administrator. Availability,
  policy and security classification are checked again when answering. If a
  saved model is unavailable, choose an allowed model or start a new conversation.
  If another tab changes the settings, Eneo asks you to reload the saved choices
  instead of overwriting them. A turn already running keeps the settings it started with.
...
Otherwise the function is silently unavailable for that turn. For a
[web widget visitor](/guides/embed-widget#what-visitors-can-reach), step 1 does
not apply, step 2 always resolves to the tenant default, only an external web
search source qualifies, and image generation never does. The conversation
