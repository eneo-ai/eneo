...
- PostgreSQL inline is the complete ready-to-use default. Compatible object
  storage is optional.
- Operators configure storage infrastructure, outbound endpoint approval, and
  safety bounds. Platform admins connect an approved destination and choose one
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Platform admin | Connect one approved destination, rotate its access keys, and set the new-write target and business upload limits in **Admin > Storage** |
...
Allow Eneo to contact the exact external endpoint. Keep this operator-owned
network boundary in the deployment environment; a platform admin cannot extend
it from the application:

```dotenv
ENCRYPTION_KEY=<44-character Fernet key>
OBJECT_CONTENT_ADMIN_ALLOWED_ENDPOINT_ORIGINS=["https://minio.storage.example.eu:9000"]
```

Restart backend and worker after changing these deployment values. Then open
**Admin > Storage**, add the endpoint, signing region, bucket, and application
access keys, and let Eneo test and save the encrypted connection. Adding the
connection does not select it for new writes or move existing content. Keep
path-based bucket addressing unless the endpoint has wildcard DNS and TLS for
bucket subdomains. Do not remove an origin used by a saved admin connection:
the backend then refuses to start until the origin is restored.
...
Add the adopted endpoint's origin to the allowlist before rotating its access
keys in Admin.
...
After the connection reports **Ready**, a platform admin can select **Object
...
optional. When the endpoint reports **Ready**, a platform administrator can
...
delete former remote keys. Eneo keeps configuration required while remote
content, staged moves, orphan candidates, or multipart cleanup still depend on
the endpoint.
...
Use this when you change hosting provider, move from a local SeaweedFS or MinIO
installation to an external service, or move back. Eneo does not copy the
bucket itself: your own tooling transfers the objects, and Eneo owns the safe
switch around that copy. Object keys do not contain the endpoint or bucket
name, so a faithful copy is readable immediately after the switch.

Plan a short maintenance window. New files land in PostgreSQL while you work,
so uploads larger than the operator's inline ceiling fail until you select
object storage again.

**1. Prepare the new bucket.** Create one empty private bucket and an
application identity limited to it, exactly as for a first connection. Add the
new endpoint origin to `OBJECT_CONTENT_ADMIN_ALLOWED_ENDPOINT_ORIGINS` and
restart backend and worker. Keep the old origin in the list until you are
finished. Take a backup of the current bucket before you begin.

**2. Send new files to PostgreSQL.** In **Admin > Storage**, select PostgreSQL
as the target for new writes, and wait until no moves are queued and storage
health reports no work in progress. This is what makes the copy complete: no
new object can appear on the old destination behind your back.

**3. Find the content prefix.** Object keys live under
`v1/<deployment-id-without-dashes>/`. Take `OBJECT_CONTENT_DEPLOYMENT_ID` and
remove the dashes — for example `94e1a7e3-21a7-40b8-959b-d0ecdc8827e2` becomes
`94e1a7e321a740b8959bd0ecdc8827e2`. Confirm the prefix is right before you copy
anything:

```bash
rclone ls old-store:eneo-object-content/v1/94e1a7e321a740b8959bd0ecdc8827e2/ | head
```

If that lists nothing, the prefix is wrong. Do not continue: a copy of an empty
prefix succeeds, verifies clean, and leaves the new destination without your
files.

**4. Copy the content namespace.** Copy only that prefix. Never copy or
overwrite `v1/.eneo-bindings/` — it holds the marker that pairs a bucket with
one database, and Eneo manages it per destination.

```bash
rclone copy \
  old-store:eneo-object-content/v1/94e1a7e321a740b8959bd0ecdc8827e2/ \
  new-store:eneo-object-content/v1/94e1a7e321a740b8959bd0ecdc8827e2/ \
  --checksum --transfers 16 --progress
```

Use `copy`, never `sync`: `sync` deletes files at the destination that are
missing from the source, and a mistaken direction would destroy production
content. Preserve object metadata; Eneo verifies the media type of every
object it reads.

**5. Verify the copy.** Compare both sides, and check the object count against
what **Admin > Storage** reports for object storage — `rclone check` reports
"0 differences" for two empty prefixes just as happily as for a complete copy:
...
rclone check \
  ...
  new-store:eneo-object-content/v1/94e1a7e321a740b8959bd0ecdc8827e2/ \
  --checksum
rclone size new-store:eneo-object-content/v1/94e1a7e321a740b8959bd0ecdc8827e2/
```

Fix any difference and run the check again until it reports no errors.

**6. Switch.** In **Admin > Storage**, select **Change destination**, enter the
new endpoint, bucket, signing region, addressing style, and access keys, and
confirm. Eneo tests the destination, refuses a bucket that belongs to another
Eneo installation, and then switches in one step. The switch is refused while
any write could still reach a destination — if that happens, wait a moment and
try again. Backend and worker pick up the change without a restart.

**7. Confirm and finish.** Select object storage for new writes again. Open a
few existing files and let one reconciliation inventory cycle complete, then
check that storage health reports no missing objects. Eneo verifies the
SHA-256 of every file it reads, so a bad copy cannot pass unnoticed.

The previous destination stays listed as **Previous destination** so you can
select **Switch back** at any time; nothing on either bucket is deleted. When
you are satisfied, select **Forget** to remove the record and decommission the
old bucket at your provider. You may then drop the old origin from
`OBJECT_CONTENT_ADMIN_ALLOWED_ENDPOINT_ORIGINS`, but keep the new one there
permanently: Eneo re-checks the allowlist every time it builds a client for an
administrator-managed destination, so removing the origin in use stops object
storage from working.

If something looks wrong at any point, switch back, correct the copy, and
switch again.
...
The release that first retains knowledge originals requires one coordinated
maintenance window because its durable job envelope is not executable by an
older worker:
...
No. A platform administrator must queue bounded, verified moves from **Admin >
...
Existing references keep working for every file that was copied. A missing file
is reported by storage health after the next complete inventory, and reading it
fails clearly instead of returning wrong bytes. Switch back to the previous
destination, complete the copy, and switch again.
