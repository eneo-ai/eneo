...
- A policy change affects new writes only. Existing content never moves
  implicitly.
- Upgrading from Eneo 2.1 or earlier? Follow [Upgrade File and Icon
  Storage](/guides/file-icon-storage-upgrade) first. The one-time migration moves
  old files and icons into the shared content store, applies to PostgreSQL-only
  installations too, and must be complete before optional S3 moves.
...
| Move to another S3-compatible service             | [Change storage location](#move-to-another-s3-compatible-service) |
...
Removing a File from an Assistant or App only removes that use; the reusable
File remains until its owner deletes it. A File attached to a sent chat message
stays in use until the conversation is deleted (see [the
FAQ](#why-can-i-not-delete-an-uploaded-file)). Organization or user offboarding keeps
...
Any administrator can review the policy, effective limits, and capability
status read-only. Administrators with the **File storage** permission also see
the deployment-wide content inventory, which spans tenants, and can change the
target, limits, moves, and connection. **Admin > File storage** shows the
...
In **Admin > File storage**, keep the target for new files set to **PostgreSQL**
...
After readiness reports `ready`, an administrator can select **Object storage**
...
Set the root encryption key in `env_backend.env` so Eneo can store credentials
encrypted, and restart backend and worker so they read it. Do **not** enable
the bundled profile:
...
After you have saved the connection, verify readiness. Until then it reports
...
When the connection shows **Ready to use**, an administrator can select **Object
...
Installations upgrading from Eneo 2.1 or earlier run a one-time migration that
moves old files and icons into the content store. PostgreSQL-only installations
need it too; S3-compatible storage is optional.

Follow [Upgrade File and Icon Storage](/guides/file-icon-storage-upgrade) for
the checklist, the maintenance window, disk and duration estimates,
verification, cleanup, and rollback. The detailed engineering contract remains in the
...
optional. When the connection shows **Ready to use**, an administrator can
open **Admin > File storage** and move existing content in either direction:

1. Choose the destination in **Move to**. Under **Advanced settings**, **Items
   per batch** sets how many items one batch moves (1–100).
2. Select **Start move** and confirm. Start another batch when this one is
   complete.
3. Follow item and byte counts by destination, state, and failure reason.
4. Select **Pause** to stop new work claims, and **Continue** to resume. One
   item already being verified may finish before the pause takes effect.
...
until no moves are queued, then select **Pause** for moves. Nothing can then appear
...
Select **Change storage location**, enter the new endpoint, bucket, signing
region, addressing style, and access keys, then select **Test and switch**. Eneo tests the destination,
...
stopped early, or lost object metadata is refused rather than activated; the
API error states how many files are missing or mismatched. Byte equality is what your `rclone check
...
still target object storage, moves are not paused, or moves are still queued
or running, change that first; if an upload is in flight, wait a moment and
retry.
...
Select **Object storage** for new files again and select **Continue** for
moves. Open a few existing files, let one reconciliation inventory cycle
complete, and check that the **Failed** count under **Show storage usage
details** does not grow: content that the inventory cannot find is marked
failed. Eneo verifies the SHA-256 of every file it reads, so a bad copy cannot
pass unnoticed.

</Steps>

The old destination stays listed under **Previous storage location**, so you
can select **Restore as storage location**; nothing is deleted on either
bucket. Restoring only works while the buckets are still identical — once new
files have been stored in object storage after the switch, Eneo refuses it, and
the way back is a fresh copy in the other direction followed by **Change
storage location**. When you are satisfied, select **Remove configuration** and
decommission the old bucket at your provider.
...
verification period. See [Upgrade File and Icon
Storage](/guides/file-icon-storage-upgrade#troubleshooting-and-recovery) for pause
...
The upgrade to Eneo 2.2 first retains knowledge originals and moves File/Icon
content into the content store. It needs one coordinated maintenance window:
jobs queued by an old worker cannot run on the new version, and old workers
still write the legacy payload columns. Follow [Upgrade File and Icon
Storage](/guides/file-icon-storage-upgrade#upgrade-step-by-step): stop the API,
let the old worker finish its knowledge jobs, stop it, take the backup, run
`db-init`, and start the new release.
...
The policy migration creates revision 1 with **PostgreSQL** (`postgres_inline`) selected. It
...
select **PostgreSQL** in **Admin > File storage**. The change affects eligible
...
read and lifecycle path indefinitely. See [Upgrade File and Icon
...
No. An administrator must start bounded, verified moves from **Admin > File
...
- **PostgreSQL**, under **File content total**, counts the content bytes stored
  inline in the database. **Object storage** counts the content bytes recorded
...
returning wrong bytes. Restore the previous storage location, complete the
...
bucket back first and use **Change storage location**.
