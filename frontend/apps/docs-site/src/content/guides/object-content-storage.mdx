...
performs the switch safely around that copy. Object keys contain no endpoint or
bucket name, so a faithful copy is readable the moment you switch.
...
for a first connection. Add the new endpoint's origin to
`OBJECT_CONTENT_ADMIN_ALLOWED_ENDPOINT_ORIGINS` and restart backend and worker,
keeping the old origin until you finish. Back up the current bucket.

### Send new files to PostgreSQL

In **Admin > Storage**, select PostgreSQL as the target for new writes and wait
until no moves are queued. Nothing can then appear on the old destination while
you copy.

### Find the content prefix

Object keys live under `v1/<deployment-id-without-dashes>/`. Take
`OBJECT_CONTENT_DEPLOYMENT_ID` and remove the dashes:

```text
94e1a7e3-21a7-40b8-959b-d0ecdc8827e2   →   94e1a7e321a740b8959bd0ecdc8827e2
```

Confirm the prefix lists your objects before copying anything:
...
<Callout type="error">
  If this lists nothing, the prefix is wrong — stop. Copying an empty prefix
...
storage. A difference count alone is not enough: two empty prefixes also report
no differences.
...
refuses a bucket belonging to another Eneo installation, and switches in one
step without a restart.

The switch is refused while any write could still reach a destination. If new
files still target object storage, change that first; if an upload is in
flight, wait a moment and retry.

### Confirm and finish

Select object storage for new writes again. Open a few existing files, let one
reconciliation inventory cycle complete, and check that storage health reports
no missing objects. Eneo verifies the SHA-256 of every file it reads, so a bad
copy cannot pass unnoticed.

</Steps>

The previous destination stays listed so you can **Switch back** at any time;
nothing is deleted on either bucket. When you are satisfied, select **Forget**
and decommission the old bucket at your provider.

<Callout type="warning">
  Keep the active destination's origin in
  `OBJECT_CONTENT_ADMIN_ALLOWED_ENDPOINT_ORIGINS` permanently. Eneo re-checks
  the allowlist every time it builds a client, so removing the origin in use
  stops object storage from working. You may drop the retired one.
</Callout>
