...
**3. Copy the content namespace.** Copy only this deployment's content prefix.
Never copy or overwrite the `v1/.eneo-bindings/` prefix — it holds the marker
that pairs a bucket with one database, and Eneo manages it for you.

```bash
rclone copy \
  old-store:eneo-object-content/v1/<deployment-id>/ \
  new-store:eneo-object-content/v1/<deployment-id>/ \
  --checksum --transfers 16 --progress
```

Find `<deployment-id>` in `OBJECT_CONTENT_DEPLOYMENT_ID`, or ask your operator.
...
**4. Verify the copy.** Ask your tooling to compare both sides before you
switch:
...
**5. Switch.** In **Admin > Storage**, select **Change destination**, enter the
...
**6. Confirm and finish.** Select object storage for new writes again. Open a
