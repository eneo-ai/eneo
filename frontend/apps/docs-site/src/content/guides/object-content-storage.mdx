...
1. Alembic expands the schema, inventories the legacy variants, and prevents old
   code from changing those payloads. It does not copy or hash the payload bytes.
...
```mermaid
flowchart LR
  expand["Schema expansion\ninventory · write fence"] --> readable["Eneo starts\nlegacy content remains readable"]
