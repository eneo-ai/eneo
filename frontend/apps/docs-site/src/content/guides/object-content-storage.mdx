...
| Close the recovery window and reclaim database disk | [Finish adoption](#finish-adoption-and-reclaim-disk)          |
...
2. The new worker first classifies ledger rows in bounded metadata-only batches:
   deleted owners are cancelled, available references are accepted, and only
   rows that still need legacy bytes become ready. After that set is complete,
   Eneo freezes the capacity decision and adopts the ready payloads into object
   content in bounded, resumable batches while Eneo stays available.
...
each reference. A later contract release applies the fail-closed completion and
live-reference checks in [Finish adoption](#finish-adoption-and-reclaim-disk)
before removing the legacy columns.
...
flowchart LR
  ...
  readable --> admission["Bounded admission\nowner lock · reference recheck"]
  admission --> worker["Online worker\nbounded copy · SHA-256"]
...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `active`                   | The worker is finalizing bounded admission batches or adopting bounded byte batches. Keep it running and monitor foreground latency, database I/O, WAL, and free disk. |
...
The capacity decision is made only after every ledger row is durably classified.
If an unfinished File or Icon is deleted, a database trigger cancels its ledger
row. A wait therefore completes instead of remaining stuck when no source-bearing
owner remains.
...
The relevant number is the estimated payload size of ledger rows that still need
legacy bytes, not the total database size. The worker freezes that set only
after its bounded admission phase has locked and rechecked owners and existing
references. A reference that fails before its row is admitted therefore enters
the estimate instead of bypassing the capacity gate. Inline adoption stores a
second copy of those payloads in PostgreSQL. Both copies remain until the later
contract release and physical table maintenance.
...
committed, pending or ready items stay in the ledger, and legacy content remains
readable.
...
### Finish adoption and reclaim disk

Eneo deliberately does not delete the frozen legacy bytes when the online
worker reports `complete`. They remain a recovery source until you choose to
close the verification period by installing the later contract release. Use
this checklist:

| Step                                 | Who starts it?                                  | Safety boundary                                                     |
| ------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------------- |
| Online adoption and verification     | Eneo's bounded worker                           | Legacy bytes remain readable and unchanged                          |
| Remove the legacy schema             | Operator installs the later contract release    | Migration locks and rechecks every surviving reference as available |
| Return table space to the filesystem | Operator schedules `pg_repack` or `VACUUM FULL` | Restore-tested backup, capacity check, and an explicit work window  |

1. Confirm that the campaign is `complete` and that no `pending`, `ready`,
   `leased`, or `failed` ledger row remains. This is necessary but not the
   final proof: a `done` row's referenced content can fail later. `cancelled`
   rows belong to Files or Icons that were deleted safely.

   ```sql
   SELECT state, halt_reason FROM file_icon_backfill_campaign;
...
2. Keep the current release during your organization's verification period.
   Download representative old Files and Icons and confirm the workflows that
   use them. Do not edit the ledger or legacy columns.
3. Create and restore-test a current backup. PostgreSQL is sufficient when all
   authoritative bytes are inline. If any content is in object storage, test a
   matching PostgreSQL and object-store backup pair. Keep the pre-upgrade backup
   until this restore succeeds and your retention rules allow its removal.
4. When the contract release is available, install it in a planned deployment.
   In the same transaction that removes the legacy schema, its migration must
   lock and recheck that every surviving ledger key has its matching File or
   Icon reference and that the referenced object content is still `available`.
   Campaign and ledger completion alone are not sufficient. A missing or failed
   reference must abort before any legacy column is dropped. Installing that
   release closes direct rollback to the old image; recovery remains forward or
   through the retained coordinated backup.
5. After the contract release, measure whether filesystem space actually needs
   to be returned:
...
6. If reclaim is needed, schedule and test **one** physical rewrite. Prefer
   [`pg_repack`](https://github.com/reorg/pg_repack/blob/master/doc/pg_repack.rst)
   for a production system that must minimize blocking; it still requires the
   extension, a short final lock, and temporary free disk of roughly twice the
   target tables and indexes. The simpler alternative is a maintenance window
   with APIs and workers stopped:

   ```sql
   VACUUM (FULL, ANALYZE) files;
   VACUUM (FULL, ANALYZE) icons;
   ```

   `VACUUM FULL` takes an exclusive table lock and needs space for the rewrite.
   Ordinary `VACUUM` can make space reusable inside PostgreSQL but normally does
   not shrink these table files. See the
   [PostgreSQL VACUUM documentation](https://www.postgresql.org/docs/16/sql-vacuum.html).

Physical reclamation stays manual because its duration, temporary capacity, and
acceptable downtime differ by deployment. It must not run as hidden work in
Alembic or application startup.
...
### Are legacy bytes deleted and is disk space reclaimed automatically?

No. Successful online adoption keeps the frozen legacy columns as a recovery
source until the later contract release passes the fail-closed checks in
[Finish adoption](#finish-adoption-and-reclaim-disk). That release can drop the
old columns, but PostgreSQL treats the drop as a metadata change and does not
immediately return their physical table storage to the filesystem.

Plan a separate tested `pg_repack` or `VACUUM FULL` maintenance operation when
filesystem reclamation matters. `pg_repack` reduces blocking but needs temporary
capacity; `VACUUM FULL` rewrites and locks the table. Ordinary `VACUUM` does not
perform this shrink. Follow [Finish adoption and reclaim
disk](#finish-adoption-and-reclaim-disk) before closing the recovery window.
This concerns disk space, not application memory.
