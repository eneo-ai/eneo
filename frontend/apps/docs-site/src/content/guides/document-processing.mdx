...
days have passed. Use **Start crawl** to request a run manually; an active run
is reused. The general worker admits due crawls and the crawler worker executes them.
...
<Tabs items={["Basic crawl", "Sitemap based crawl"]}>
  <Tabs.Tab>
    ...
    default and request concurrency and delay are bounded. This is the only mode that can
...
Each crawl records progress, its terminal outcome and any failures. The website
page shows run history and lets you stop an active crawl or request another run.
If a website fails ten crawls in a row it is automatically disabled; change its
update interval to re-enable it.
...
- **Web**: bounded asynchronous HTTP crawling with main-content extraction
...
The general worker runs with `RUN_AS_WORKER=true`; crawler execution requires
the dedicated crawler service as well. Both use the backend image. Two
...
- `GET /api/healthz/crawler` reports crawler health, execution leases and recovery diagnostics
...
```bash
docker compose logs worker crawler | grep -i crawl
