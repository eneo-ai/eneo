...
5. **Choose optional storage changes later.** Once adoption is complete, you may
   ...
   legacy columns until you choose the cleanup command; completion
...
Version 2.2 includes optional cleanup. An ordinary upgrade preserves legacy
columns and starts adoption as before. The same image supports migration,
verification, and operation after cleanup; another release is not required.

1. Confirm campaign state `complete`, unpaused admission, and no `pending`,
   `ready`, `leased`, or `failed` ledger rows.
...
3. Create and restore-test a current backup. Use a paired PostgreSQL/object-store
   restore when any authoritative content is remote. Keep the pre-upgrade
   recovery point until the restore succeeds and your retention policy permits
   removal.
4. Stop all API, worker, and maintenance-worker processes, including replicas.
   Run this in a one-off container from the same 2.2 image:

   ```bash
   python -m eneo.object_content.file_icon_migration cleanup
   ```

5. Success reports `legacy_cleaned: true`; a repeat reports `changed: false`.
   Failed prerequisites exit 2; database or lock failures exit 3. Resolve the
   condition and retry. The command's transaction preserves legacy storage on
   failure. Restart the same image and verify old downloads and a new upload.
6. Reclaim filesystem space only if measurements justify it.

Cleanup uses READ COMMITTED isolation and a five-second lock-acquisition timeout.
It fences writers, verifies every live legacy source and exact surviving
File/Icon reference, and drops all eight columns in one transaction. Inline
bytes must match their recorded size and SHA-256. Remote verification uses
recorded database evidence; check remote availability and the backup pair before
stopping services. The verification scans retained payloads, so measure its
maintenance window on a representative restored database.

The upgrade commands, fallback readers, settings, and migration ledger remain in
2.2. After cleanup, `status` includes `legacy_cleaned: true`, `preflight` reports
`cleaned`, and the worker leaves legacy work complete. Ordinary content failures
remain visible but cannot reopen repair from deleted legacy data. Recovery is
forward or through a coordinated pre-cleanup backup; downgrading cannot recreate
these bytes, and restoring a backup discards later writes.
...
table files. Choose one tested physical rewrite after cleanup:
...
They are the recovery source during verification. The optional cleanup command
in 2.2 performs the final locked source and reference checks before removing the columns.
