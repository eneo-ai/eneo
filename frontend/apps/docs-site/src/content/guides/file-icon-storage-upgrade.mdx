...
5. Install the later contract release during a planned window. Its migration
   must fence writers and verify every remaining legacy source and exact
   surviving File/Icon reference in the same transaction that drops legacy
   columns. Matching available content must cover the actual sources; ledger
   completion alone is not enough. This closes direct rollback to the old image.
