...
| Decide what data is retained             | [Data Retention & Deletion](/guides/data-retention)                  |
...
### [Eneo Flows for API consumers](/guides/flows)
...
Use the [Flow Runtime API Reference](/guides/flows-api-guide) for complete field and endpoint details.

### [Data Retention & Deletion](/guides/data-retention)

Understand which Eneo data classes can be deleted automatically and which only become eligible for a later administrator action:

- Separate Flow run history from conversations, AI Builder sessions, and audit logs
- Configure Organization, Space, and Flow run-history policies
- Apply the 30/60/90-day override model without changing every Flow
- Choose when human review must precede a future purge
...
Turn on the functions (_Funktioner_) assistants can use, and understand how they run:
