...
Resume within 30 days of `approved_at`. After that deadline, the request returns
`400 flow_run_abandoned`, including the wait and deadline in `context.abandonment`.
The maintenance sweep then fails the run and cancels the approved checkpoint as
a system transition. Automatic abandonment preserves the approval timestamp,
