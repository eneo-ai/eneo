...
For a clean session, `transcript.done` also includes `"transcript_id": "<uuid>"`. Eneo commits that transcript before sending the event. This requires a `recording_id`, a valid `produced_samples` count exactly equal to the samples Eneo received, and normal upstream completion after the final commit for the client's stop. Refused, errored, interrupted, or mismatched sessions receive no transcript ID; a storage failure also leaves the preview usable without an ID. A done event received before stop is never stored.

The stored text is the authoritative done text. Timed passages are kept only when every delta supplies valid timing and the delta text matches the done text exactly; otherwise the text is stored without passages. Unbound transcripts use the tenant's runtime-upload abandonment retention period at creation. Once expired, they are removed with abandoned runtime uploads by the existing administrator-triggered purge. If that period is disabled, they have no expiry. Bound transcripts are excluded from this purge.
