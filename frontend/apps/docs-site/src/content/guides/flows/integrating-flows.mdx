...
Fresh deployments allow **256 MiB per session or knowledge file** and **1,073,741,819 bytes per audio file** (1 GiB minus 5 bytes), for both session and knowledge audio. The default PostgreSQL inline ceiling is also 1,073,741,819 bytes, its maximum valid payload. The Flow hard maximum stays at 2 GiB. Persistent assistant attachments default to 256 MiB in total per assistant and must also fit the selected model's input context.

The upload-policy migration raises each stored value only when it equals the old seed: 10 MiB for session/knowledge files or 200 MiB for audio. Other values, including deliberately lower policies, remain unchanged. Downgrade restores the old values only where a value equals the new default. Existing environment overrides remain effective; deployments retaining `OBJECT_CONTENT_INLINE_MAXIMUM_BYTES=209715200` must remove or update that override to admit the larger audio workload.
...
| --- | --- | --- |
| 5-hour recording at 128 kbps | Audio upload, decoded duration/bytes, transcript bytes, shared invocation budget | About 288 MB compressed fits the audio ceiling; 5 hours fits the duration ceiling. Decoded audio must stay within 2 GiB and transcript text within 8 MiB; all steps share 8 hours. |
