...
To give Codex or Claude Code this consumer workflow without exposing a private deployment's OpenAPI document, install the [Eneo Flows API Agent Skill](/guides/flows/agent-skill).
...
Authenticate every request with exactly one credential, unless you call as a module session:
...
A module session is the exception. A module acting for a signed-in user sends both headers on every request: the user's module token as the bearer token and the module's `sk_` service key as the API key. The module token alone gets `401`, and the key alone calls as the service key instead of the user. [Module authentication](/docs/module-authentication#call-flows-as-the-signed-in-user) describes how the module gets the token and which routes accept the pair.
...
Keep service keys and module tokens in server-side code, never in a browser; a browser frontend should use a user access token or call through your own backend. Do not put an API key in the bearer-token header. See [Authentication & OIDC](/guides/oidc-federation) for token setup.
...
2. **One credential per request, except for a module session.** `Authorization: Bearer` for a user token, `X-API-Key` for an API key, and never an API key in the bearer header. A module session sends both, its module token and its service key; see [Authenticate requests](#authenticate-requests).
...
5. **`Idempotency-Key` is required on both continuation endpoints and optional on run creation.** `POST .../review-checkpoints/{checkpoint_id}/resume/` and `POST .../review-checkpoints/{checkpoint_id}/approve-and-continue/` reject a request without it (`400 flow_review_idempotency_key_required`). `POST /runs/` accepts it and it is strongly recommended. Edit, approve, and reject do not accept it at all; they use `expected_checkpoint_revision` for concurrency instead. Every accepted key must be 1 to 255 characters after trimming, or the request returns `400 flow_run_invalid_idempotency_key`. That bound is enforced as a typed error rather than as a JSON Schema `maxLength`, so a validator generated from the OpenAPI document will not catch it for you.
6. **Approve does not resume.** Approving records the decision; the run stays at `awaiting_review` until you call resume. When your integration has no reason to hold the run between the two, call `POST .../review-checkpoints/{checkpoint_id}/approve-and-continue/` instead: it approves and resumes in one transaction (`202 Accepted`, `Idempotency-Key` required, same response shape as resume), so a failure between two requests can never leave an approved checkpoint waiting.
7. **Edit sends the value, not the payload.** `edited_value` is the corrected step output itself: a string when the step's `output_type` is `text`, a JSON object or array when it is `json`. The other shape returns `400 typed_io_validation_failed`. Do not send the checkpoint's `current_payload_json` back; the server rebuilds it, deriving a JSON step's `text` from the value so the two encodings cannot disagree, and carrying every runtime-owned key over from the stored checkpoint. Only a checkpoint whose `review_mode` is `edit` accepts the request (`400 flow_review_edit_not_allowed`), and an output whose text overflowed into a generated file cannot be edited at all (`400 flow_review_edit_file_backed_unsupported`) — approve or reject it instead.
...
Polling is the only mechanism a caller controls for observing run status, because there is no client-registered subscription, server-sent events stream, or run-status WebSocket. A flow author can design a terminal step that delivers its result over outbound HTTP, and that does signal successful completion to whatever receiver they configured, but it is not something the caller registers and it does not report failure or review states. A workable cadence:

- poll `GET /api/v1/flows/{id}/runs/{run_id}/status/` every **2 seconds** for the first 30 seconds, then every **5 seconds**, then every **15 seconds** after two minutes
...
The worked example starts with a known Flow id. If your app needs discovery, list flows with `GET /api/v1/flows/?published_only=true&limit=50&offset=0`, adding `space_id={space_id}` to cover one space.

- **Without `space_id`**, a signed-in user, a user-owned API key, or a module session (its module token and service key together, see [Authenticate requests](#authenticate-requests)) gets the flows of every space the user belongs to: their personal space, spaces shared with them directly or through a group, and the organization space if they are one of its admins. A space-scoped API key narrows the list to its space, so a module key scoped to a space the user does not belong to returns an empty list.
- **A service key must send `space_id`**, because a key belongs to no spaces. Without it the request returns `400` with code `flow_service_key_space_id_required`. A service key sees only published Flows in its scoped space.

Drafts appear only in spaces where the caller may edit flows, so an app that runs flows sends `published_only=true` to get published flows only. Each item carries `space_name` next to `space_id`: group the list on `space_id` and show `space_name` as the heading. Items come oldest first. `count` is the number of items in the current page, not the total; increase `offset` while `has_more` is true (`limit` is 1 to 200, default 50).

Seeing a flow in the list does not mean the caller may run it or that it is ready to run. After choosing an id, `GET /api/v1/flows/{id}/published/` returns the runtime-safe projection and canonical runtime paths. Do not use the draft `GET /api/v1/flows/{id}/` view from a service-key runtime app.
...
| Live text and speaker labels        | Show text while someone records, and let a run choose speaker labels.                                                     | `POST /api/v1/flows/{id}/steps/{step_id}/live-transcription-sessions/` (`create_flow_live_transcription_session`) |
...
| Poll run                    | `/api/v1/flows/{id}/runs/{run_id}/status/`                                                  | `GET`   | `200 OK`       | `get_flow_run_status`                             |
...
| Approve and continue        | `/api/v1/flows/{id}/runs/{run_id}/review-checkpoints/{checkpoint_id}/approve-and-continue/` | `POST`  | `202 Accepted` | `approve_and_continue_flow_run_review_checkpoint` |
...
```json
{
  ...
  "transcription": {
    ...
    "max_speakers": { "form_field": null, "participants_field": null }
  },
...
`transcription` describes the audio options for a flow that transcribes recorded audio, and is `null` for any other flow. See [Live text and speaker labels](#live-text-and-speaker-labels).

`security_classification` is the security classification of the flow's space, with its `name`, `description`, and `security_level`, so your app can tell users what information the flow may take; it is `null` when the space has none or the organization has turned security classifications off.
...
```text
GET /api/v1/flows/{id}/runs/{run_id}/status/
```

The content-free summary reports lifecycle and bounded dispatch diagnostics. It
does not return accepted input, terminal errors, results, files, usage, or
webhook deliveries.
...
Fetch the active checkpoint before showing the review screen. It returns `200 OK`
with the literal JSON `null` when no checkpoint is open, so check for that before
reading any field. The payload is a required audited read; Eneo returns `503`
without checkpoint content if the audit record cannot be committed.
...
Submit the corrected step output value with the revision the reviewer saw. This step is `json`, so the value is the structured object; a `text` step takes a plain string.
...
Resume within 30 days of `approved_at`. After that deadline, an approval the
sweep has not yet processed refuses resume with `400 flow_run_abandoned`,
including the wait and deadline in `context.abandonment`. Once the maintenance
sweep has failed the run and cancelled the checkpoint, resume returns
`400 flow_review_cancelled`. Automatic abandonment preserves the approval timestamp,
decision actor, history, and files. It never resumes the run automatically, and
`retryable: false` means you must inspect earlier effects before submitting a new run.
...
3. Call `runtime_paths.get_run_status_template` until the run is terminal or `awaiting_review`.
4. Call `runtime_paths.get_run_template` once to read the audited final result.
5. Call `runtime_paths.list_steps_template` only when you need per-step output, files, or diagnostics.
...
| `/api/v1/flows/{id}/runs/{run_id}/status/` | `GET`  | `200 OK`      | `get_flow_run_status`   |
...
2. Poll `runtime_paths.get_run_status_template`; when status is `awaiting_review`, show the review UI.
...
5. For edit review, send `edited_value`: the corrected step output itself, a string for a `text` output step or a JSON object or array for a `json` one, satisfying `output_contract` and staying within the inline UTF-8 limit.
6. Send nothing else. Runtime-owned payload keys carry over from the stored checkpoint, and the server derives a JSON step's `text` from the edited value so the two encodings cannot disagree.
7. Offer edit review only when `review_mode` is `edit`, and expect `flow_review_edit_file_backed_unsupported` for an output already stored as a generated file. PDF and DOCX artifact steps support view review, not edit review.
...
| `/api/v1/flows/{id}/runs/{run_id}/review-checkpoints/{checkpoint_id}/approve-and-continue/` | `POST`  | `202 Accepted` | `approve_and_continue_flow_run_review_checkpoint` |
| `/api/v1/flows/{id}/runs/{run_id}/`                                                         | `GET`   | `200 OK`       | `get_flow_run`                                    |
...
When a user needs to repeat completed work with corrected inputs, create a new
run. The original run and its step-attempt evidence remain immutable and can be
compared with the new run by their run identifiers.
...
3. List runs with `runtime_paths.list_runs`; service keys see only their own runs. Tenant admins and same-space admins and owners also see other people's runs, and `mine=true` lists only the caller's own, which in a module session are the signed-in user's.
...
## Live text and speaker labels

A flow that transcribes recorded audio reports live text availability, speaker-label options, and an optional speaker-count limit in the run contract's `transcription` object. For any other flow, `transcription` is `null`.

| Field                       | Meaning                                                                                                                                                                                                                                                                           |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `live.available`            | The flow's audio step can show live text while someone records                                                                                                                                                                                                                    |
| `live.reason`               | `null` when live text is available. Otherwise `transcription_disabled` (the flow does not transcribe audio), `transcription_service_mode` (an external transcription service transcribes with its own model), `model_unavailable` (the flow's transcription model is not available in its space), or `model_not_realtime` (the model does not support live transcription) |
| `speaker_labels.selectable` | A run may choose speaker labels with `speaker_labels`. It is `false` when no transcription service labels speakers, or when the flow requires labels                                                                                                                             |
| `speaker_labels.required`   | The flow needs speaker labels, because a step maps speakers to names                                                                                                                                                                                                              |
| `speaker_labels.default`    | The flow's own setting, which applies when a run makes no choice                                                                                                                                                                                                                  |

### Choose speaker labels for a run

When `transcription.speaker_labels.selectable` is `true`, your app can let the user decide whether the run's transcript labels speakers.

1. Show the choice, preset from `transcription.speaker_labels.default`.
2. Send the choice as `speaker_labels` (`true` or `false`) in the create-run body, next to `expected_flow_version`, `input_payload_json`, and `step_inputs`:
   ...
   To use the flow's own setting, omit `speaker_labels` or send `null`.

3. Check the run: `GET /api/v1/flows/{id}/graph/?run_id={run_id}` marks the audio step's node with `"speaker_identification": true` only when the run labels speakers.

When `selectable` is `false`, do not send `true` or `false`: Eneo refuses the request with `422` and code `flow_run_speaker_labels_not_selectable`. Labelling speakers makes the run take longer. A retry of a failed run and a transcript regeneration keep the run's choice. `speaker_labels` belongs at the top level of the request; inside `input_payload_json` it is a reserved key.

### Set a speaker-count limit

When `transcription.max_speakers` is present and the run labels speakers, you
may send `max_speakers` at the top level of the create-run request. It accepts
an integer of at least 1 as an upper bound on the number of speakers. Leave
it automatic when the count is uncertain; participant names do not determine
the limit.

If `transcription.max_speakers.form_field` names a field, use the flow's
existing count field instead of showing a second control. The request is
resolved in this order:

- An explicit `max_speakers` value overrides the form field. Explicit `null`
  selects automatic speaker detection, even if the form contains a count.
- When `max_speakers` is omitted, Eneo uses the form's count field if supplied.
- Otherwise, detection is automatic.

Eneo stores this decision with the run and preserves it through retry and
transcript regeneration. Reusing an idempotency key with a different limit
returns `flow_run_idempotency_conflict`. A positive limit on a run that labels
no speakers returns `422 flow_run_max_speakers_not_available`.
`max_speakers` is reserved inside `input_payload_json`; send it at the top
level of the request.

If `transcription.max_speakers.participants_field` names a field, that field
lists the people expected to speak. Your app may offer the number of names as
a prefill for `max_speakers`, as long as the user can see and change it. Eneo
never sets the limit from the names itself: a limit below the real number of
speakers would merge voices that are not on the list into one person.

### Start a new run with the same speaker options

A run response carries the run's own speaker options at the top level, next to
`input_payload_json`:

- `speaker_labels` is the run's speaker-label setting, fixed when the run was
  created: `true` when the flow requires labels, otherwise the run's own choice
  or the flow's default at that time. It is `null` when the run recorded no
  setting: the flow offers no speaker-label option, or the run was created
  before Eneo recorded it.
- `max_speakers` is the limit the run used, and `null` when detection was
  automatic or the run labels no speakers.

To start a new run with the same options, send `max_speakers` as it is. When
`speaker_labels` is `true` or `false`, send it too, but only when the run
contract's `transcription.speaker_labels.selectable` is `true`. When it is
`null`, omit it: the new run follows the flow's current default, which may
differ from what the earlier run used.

### Send a recording in several parts

A recorder that starts a new file during a long meeting, for example every 20 minutes or after a lost microphone, should mark the files as parts of one recording. Speakers are then labelled across all parts together, so a person keeps one label for the whole meeting instead of getting a new one in each part.
...
Send the flag only when the run contract's `transcription.single_recording` is `true`, which it is for every flow that transcribes audio. List the parts in recording order; a break between two parts, such as a paused meeting, is fine. Set `single_recording` only when every file of the step comes from the same recording; separately recorded or uploaded files stay separate. The setting takes effect when the run labels speakers with the external speaker service and the step has more than one file; otherwise each part is transcribed on its own, as without it. Either way the parts together must fit within the step's `max_recording_seconds`, and a longer recording fails its transcription step with `typed_io_audio_exceeds_limit`. Each part keeps its own place in the transcript, so playback, corrections and regeneration work per file as before. Only audio steps accept it; other steps receive `400 flow_run_single_recording_requires_audio_step`. It is part of the request's idempotency fingerprint, and a retry keeps it. A step result reports it as `runtime_input_single_recording`, so a new run started with the same files can send it again.

A recorder reads the recording's length from the audio step in `steps_requiring_input`:

- `max_recording_seconds`: the longest recording the step takes as the parts of one recording, overlaps included. A tenant admin sets it on the flow-settings page (Longest recording), within a ceiling the deployment sets. Stop recording before it.
- `recording_part_seconds`: the part length at which the step's `max_files` slots hold a recording of `max_recording_seconds`. Start a new part at this length, or earlier when a part nears `max_duration_seconds` (one file) or `max_file_size_bytes`.

Contracts from older Eneo versions lack both fields. There, treat `max_duration_seconds` as the limit for the parts of one recording together.

### Show live text while recording

Eneo can use the stored text of a clean live session when the run starts, avoiding a second transcription. It keeps no audio from the session, so record the audio in your app as usual and upload it when recording ends. Reuse requires one uninterrupted capture and exactly one audio file for the step.

Before you start, read `transcription.live.available` from the run contract. When it is `false`, record without live text. Requesting a ticket needs the same access as creating a run, and a service key can request one for a published flow.

1. **Request a ticket** when the recording starts:

   ```text
   POST /api/v1/flows/{id}/steps/{step_id}/live-transcription-sessions/
   ```

   `{step_id}` is the audio step from `steps_requiring_input`. The body is optional. To identify the recording for a stored transcript, send `{"recording_id": "recording_123"}`. This opaque client string must match `^[A-Za-z0-9_-]{8,64}$`; invalid values receive the usual `422` request validation response. Omit it or send `null` for preview only. The response is `201 Created`:
...
2. **Open the WebSocket within 30 seconds.** Connect to `websocket_path` on the API host, with `wss://` when the API uses HTTPS, and offer two subprotocols: `eneo-live.v1` and `ticket.<ticket>`. A ticket opens one socket only.
3. **Wait for `ready`.** Then send the recording as binary frames of mono 16-bit little-endian PCM at 16 kHz. Each frame holds 2 bytes to 64 KiB, an even number of bytes. Keep sending for as long as the recording runs, silence included.
4. **Show the text.** Append the `text` of each `transcript.delta` event to the preview.
5. **Stop.** When the recording ends, send `{"type": "stop", "produced_samples": 3200}`. Count every 16 kHz sample captured for this session **before** client-side queueing or discarding. The count must be a non-negative integer, not a boolean. The plain `{"type": "stop"}` remains valid. Missing or invalid counts leave the session ineligible for storage; unrelated extra fields remain ignored. Eneo answers with `transcript.done`, which carries the full preview text of this session, and closes the socket.
6. **Upload and run.** Upload the recording to the audio step and create the run, as in the [worked example](#worked-end-to-end-example). If `transcript.done` includes a `transcript_id` covering the whole recording, send it as `step_inputs[step_id].live_transcript_id` alongside that step's single `file_ids` entry. Otherwise omit it and the run transcribes the uploaded audio as usual.

The session works when the socket sends `ready` after it opens. If the socket sends `error` instead, its `code` says why.

#### Socket messages

| Direction   | Frame                                                                           | Meaning                                                                                                     |
| ----------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| App to Eneo | Binary                                                                          | Audio: mono 16-bit little-endian PCM at 16 kHz, 2 bytes to 64 KiB per frame                                 |
| App to Eneo | `{"type": "stop", "produced_samples": 3200}`                                    | The recording has ended; the sample count is optional. Text frames without `"type": "stop"` end the session with `invalid_message` |
| Eneo to app | `{"type": "ready", "sample_rate": 16000, "max_seconds": 18000}`                 | Eneo has connected to the model server. Start sending audio                                                 |
| Eneo to app | `{"type": "transcript.delta", "text": " världen"}`                              | Text to append to the preview                                                                               |
| Eneo to app | `{"type": "transcript.done", "text": "Hej världen"}`                            | The full preview text of this session after `stop`. The socket then closes                                  |
| Eneo to app | `{"type": "error", "code": "idle_timeout", "message": "...", "retryable": false}` | The session has ended. The socket then closes                                                               |

`message` is for people; branch on `code`. Every code is listed in [Live transcription socket codes](/guides/flows/reference/errors#live-transcription-socket-codes).

For a clean session, `transcript.done` also includes `"transcript_id": "<uuid>"`. Eneo commits that transcript before sending the event. This requires a `recording_id`, a valid `produced_samples` count exactly equal to the samples Eneo received, a successfully sent final commit for the client's stop, and matching audio evidence from the model server. The upstream done event must include `audio_seconds`, with `round(audio_seconds * 16000)` equal to Eneo's received sample count; servers without this extension still provide previews but no stored transcript. Refused, errored, disconnected, or mismatched sessions receive no transcript ID. A storage failure or a five-second write timeout also leaves the preview usable without an ID.

The stored text is the authoritative done text. Timed passages are kept only when every delta supplies valid timing and the delta text matches the done text exactly; otherwise the text is stored without passages. Unbound transcripts use the tenant's runtime-upload abandonment retention period at creation. Once expired, they are removed in bounded batches by the tenant administrator's Flow history purge, within its selected tenant, space, or flow scope. The purge response reports `transcript_candidate_count` and `transcript_purged_count` separately from run counts, even when no runs are eligible. The requested limit applies independently to runs and transcripts. A dry run reports transcript candidates, deletes nothing and reports zero deletions. Real purges include both transcript counts in the required audit record, without transcript text. If the retention period is disabled, transcripts have no expiry. A bound transcript is never an expiry candidate: it lives as long as a run uses its file, and the Flow history purge deletes it together with that file's last run. The transcript counts cover the expired, unbound transcripts only.

#### Use the live transcript in a run

```json
{
  "step_inputs": {
    "00000000-0000-0000-0000-000000000101": {
      "file_ids": ["00000000-0000-0000-0000-000000000201"],
      "live_transcript_id": "00000000-0000-0000-0000-000000000301"
    }
  }
}
```

The transcript must belong to the same tenant, user, flow, published version, step, and transcription model. Expiry limits how long an unbound transcript waits for its first run: an expired unbound transcript is refused, while one already bound to this file is accepted whatever its expiry, so a retry over the same file keeps using it. Admission binds it permanently to the uploaded file. The same transcript can be submitted again for that file; another file receives `409 flow_run_live_transcript_already_bound`. An unavailable, foreign, or expired unbound transcript receives `404 flow_run_live_transcript_not_found`. A step without exactly one audio file receives `400 flow_run_live_transcript_requires_one_audio_file`. The ID is part of the request's idempotency fingerprint.

Before using the text, Eneo decodes the file and compares its duration with the live session's received audio. The difference must be at most one second or 0.5% of the decoded duration, whichever is larger. With speaker labels off, the live passages become the transcript segments. With labels on, the existing speaker service labels those passages using the run's `max_speakers`. Reduced-precision alignment is retained and reported just as for batch transcription; a speaker-service failure follows the usual run failure and retry behavior.

The step's transcription evidence records `transcript_origin` as `live` or `batch`. If the requested live transcript cannot be used at execution, Eneo transcribes the file through the normal batch path and adds `live_fallback_reason`: `duration_mismatch`, `no_timing` (in either label mode), or `unavailable`. The existing `source` reference continues to identify the stored transcript for playback, corrections, and regeneration.

#### When a session ends with an error

An `error` event ends the session, and nothing carries over to a new one: each ticket starts a fresh session on the model server, and its `transcript.done` covers only that session's audio. The recording in your app is unaffected, and so is the run, which transcribes the uploaded file.

- **While the recording runs**, if `retryable` is `true`, you can request a new ticket and start a fresh session for the rest of the recording. Keep the text you already show, and append the new session's text after it. Speech from the gap between the sessions is missing from the preview, not from the run.
- **After you sent `stop`**, do not request a new ticket: a new session cannot recover the final text. Keep the preview text you have, and go on with the upload and the run.
- **If `retryable` is `false`**, go on without live text.

After any interrupted or restarted session, omit `live_transcript_id` for the recording. A later session's ID covers only that session's audio.

#### Limits and refusals

- A session accepts at most `max_seconds` of audio, the deployment's audio duration limit, and then ends with `duration_exceeded`.
- With the default settings, a session that receives no audio for 300 seconds ends with `idle_timeout`, and after `stop` Eneo waits up to 60 seconds for the final text before it ends the session with `upstream_timeout`.
- Eneo refuses the WebSocket handshake with HTTP `403` when a subprotocol is missing, when the ticket is unknown, used, or expired, or when the page's origin is not allowed. A refused handshake sends no event.

The ticket request answers `400 flow_not_published` for an unpublished flow (`404` for a service key), `400 flow_run_unknown_step_input` when the step does not take audio, `403` when the caller may not run the flow, and `409 flow_live_transcription_unavailable` when live text is not available. The `409` response carries one of the `live.reason` values in `context.reason`.

#### Connect from the browser or through your backend

The socket checks only the ticket and the page's origin. It does not authenticate with a service key or a module token, so those stay on your server. Eneo accepts a page served from the API's own host, from an origin the operator allows, or from a loopback host (`localhost`, `127.0.0.1`, or `::1`). It also accepts a server-side client that sends no `Origin` header.

- **Connect from the browser** when Eneo accepts your app's origin. Your backend requests the ticket with its credentials and passes the ticket to its signed-in browser, which opens the socket directly.
- **Relay through your backend** when Eneo does not accept your app's origin, or when you keep all traffic on your server. Your backend requests the ticket, opens the socket, and forwards audio frames from the browser to Eneo and events back.

Either way, the connection works when the browser receives `ready`. A module requests the ticket with its `sk_` key and the user's module token; see [Call Flows as the signed-in user](/docs/module-authentication#call-flows-as-the-signed-in-user).
...
export class EneoFlows {
  ...
  upload(flowId: string, stepId: string, file: File): Promise<{ id: string }> {
    ...
    return this.call(
      "POST",
      `/flows/${flowId}/steps/${stepId}/runtime-files/`,
      { form },
    );
  }
  ...
  createRun(
    ...
    return this.call("POST", `/flows/${flowId}/runs/`, {
      body: payload,
      idempotencyKey,
    });
  }
  ...
  activeCheckpoint(
    ...
    return this.call(
      "GET",
      `/flows/${flowId}/runs/${runId}/review-checkpoints/active/`,
    );
  }
  ...
  editCheckpoint(
    ...
    if (checkpoint.current_payload_json.text_overflow !== undefined) {
      ...
      throw new Error(
        "file-backed output: flow_review_edit_file_backed_unsupported",
      );
    }
    return this.call(
      "PATCH",
  ...
  async waitForRun(
    ...
    for (;;) {
      ...
      if (Date.now() > deadline)
        throw new Error(`run ${runId} still ${run.status} at deadline`);
...
| Audio duration              | Default 5 hours per input (`FLOW_AUDIO_MAX_DURATION_SECONDS=18000`), with a separate 2 GiB decoded-audio ceiling                                                                                                                                                     |

Fresh deployments allow **256 MiB per session or knowledge file** and **384 MiB per audio file** (402,653,184 bytes), for both session and knowledge audio. The default PostgreSQL inline ceiling is also 384 MiB, leaving room for `bytea` hex output and protocol framing during reads and backups. Recordings above this default require the existing object-store path and a matching upload policy; select object storage in **Admin > File storage**. The datatype hard bound remains 1 GiB minus 5 bytes. The Flow hard maximum stays at 2 GiB. Persistent assistant attachments default to 256 MiB in total per assistant and must also fit the selected model's input context.

The upload-policy migration raises each stored value only when it equals the old seed: 10 MiB for session/knowledge files or 200 MiB for audio. Other values, including deliberately lower policies, remain unchanged. Downgrade leaves policy values and metadata unchanged because matching values may be administrator choices. Existing environment overrides remain effective; deployments retaining `OBJECT_CONTENT_INLINE_MAXIMUM_BYTES=209715200` must remove or update that override to admit the larger audio workload.

Tenant policies can lower the effective allowances. A null file-count override restores the deployment default of 1000 files, rather than removing the ceiling. The run contract reports the resolved limits after storage and tenant policy are applied.

### Text and payload sizes

The inline limit is **8 MiB of UTF-8 bytes**, configured through `FLOW_MAX_INLINE_TEXT_BYTES=8388608`. It is a deployment setting.
...
Structured output uses the same 8 MiB ceiling, measured as serialized JSON including its framing. Retained rejected-output previews also use 8 MiB and end on a complete UTF-8 character. PDF extraction has an independent **128 MiB** ceiling for extracted text, including page markers; increasing the inline limit does not change it. Material-backed processing follows the effective file allowance, while structured output still has the inline ceiling.

### Execution, PDF extraction, and Builder

| Setting | Default | Applies to |
| --- | --- | --- |
| `OBJECT_CONTENT_INLINE_MAXIMUM_BYTES` | 402,653,184 (384 MiB) | Inline payload admission; larger recordings require object storage and a matching upload policy |
| `TASK_EXECUTION_TIMEOUT_SECONDS` | 28,800 s (8 hours) | One worker invocation, shared by all steps; review pauses and requeues start a new invocation |
| `FLOW_AUDIO_MAX_DURATION_SECONDS` | 18,000 s (5 hours) | Duration of each decoded audio input |
| `FLOW_AUDIO_MAX_DECODED_BYTES` | 2,147,483,648 (2 GiB) | Decoded audio size, independently of compressed upload size |
| `FLOW_MAPPED_STEP_MAX_PROVIDER_CALLS_DEFAULT` | 1000 | Provider calls per mapped step, including the native-JSON fallback reserve |
| `FLOW_PDF_MAX_PAGES` | 2000 | Pages in each PDF extracted for a Flow |
| `FLOW_PDF_MAX_EXTRACTED_BYTES` | 134,217,728 (128 MiB) | Extracted UTF-8 text, including page markers |
| `FLOW_PDF_EXTRACTION_TIMEOUT_SECONDS` | 900 s | Supervised extraction of one PDF |
| `AI_BUILDER_PROPOSAL_TIMEOUT_SECONDS` | 300 s | Provider silence, including the wait for the first chunk; the separate whole-call ceiling is 1800 s |
| `ATTACHMENT_MAX_FILES` | 100 | Persistent attachments per assistant; Builder separately allows up to 100 session attachments |

The 8-hour invocation default must land after heartbeat-based recovery (`eneo-dgh9`); release orchestration enforces this dependency. For longer recordings, operators change **`FLOW_AUDIO_MAX_DURATION_SECONDS` and `TASK_EXECUTION_TIMEOUT_SECONDS` together**. The invocation budget still reserves 60 seconds for finalization, giving steps a maximum of 28,740 seconds within the remaining shared budget; the minimum invocation budget remains 65 seconds. Upload bytes, decoded bytes, and model/provider capacity must also admit the recording. These provisional duration and execution defaults may be revised by the measurement receipt tracked as `eneo-hy7c`.

### Workload receipt

This table checks workloads against the default limits. It does not establish provider throughput or extraction time for every document.

| Workload | Limits it touches | Default admission |
| --- | --- | --- |
| 5-hour recording at 128 kbps | Audio upload, decoded duration/bytes, transcript bytes, shared invocation budget | About 275 MiB (288 MB) compressed fits the 384 MiB audio ceiling; 5 hours fits the duration ceiling. Decoded audio must stay within 2 GiB and transcript text within 8 MiB; all steps share 8 hours. |
| 2000-page PDF | File upload, page count, extracted text, extraction timeout | Up to 256 MiB uploaded, 2000 pages, 128 MiB extracted text, and 900 s extraction. Page count alone does not determine file size or processing time. |
| 300 documents | Runtime file count, per-file bytes, mapped provider calls, structured output | Fits the default 1000-file count and 1000-call mapped-step budget for one call per document, including room for fallback. Each file and the combined structured output must fit their respective byte limits. |
| 17-step Builder package with attachments | Proposal silence/whole-call deadlines, attachment count, selected model context | Step count does not consume attachment slots. Up to 100 Builder session attachments remain subject to tenant policy and model context; a streaming proposal has a 300 s silence deadline and an 1800 s whole-call ceiling. |
...
| Resume after review expiry | Approval persisted before `expires_at` remains resumable for 30 days after `approved_at`. An already-expired checkpoint returns `400 flow_review_expired` |
...
- **Exhausted queued dispatches have a 30-day limit**, measured from `dispatch_pending_since`. After that deadline, redrive returns `400 flow_run_abandoned`; the maintenance sweep fails the run while keeping its history and files.
...
Run-history policy resolves one complete `{mode, days}` pair from Flow, Space, then Organization. `preserve` defines manual purge eligibility; `review_required` adds a read-only human-review queue. Configuring a policy, reaching its age threshold, or reading that queue does not delete anything. No scheduled worker purges Flow-owned data, and this release exposes no administrator run-history purge action from the settings surface.

Runs and their files therefore remain stored, and create-run idempotency replay remains tied to the retained run row. Do not infer deletion from an eligibility timestamp.
...
Conversation history, Flow AI Builder sessions, and audit logs have separate retention behavior. Read [Flow run-history retention](/guides/flows/retention-policy) for the hierarchy and [Data retention and deletion](/guides/data-retention) before making a system-wide retention claim.
...
- **No client-registered run-status subscription.** No webhook you can register, no server-sent events, no run-status WebSocket. Polling is the contract for a client. The live transcription WebSocket carries preview text only, never run status. A flow author can separately configure a terminal step to POST its result to a destination, but that is designed into the flow, not requested by the caller.
