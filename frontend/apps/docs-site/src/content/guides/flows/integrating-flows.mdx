...
Send the flag only when the run contract's `transcription.single_recording` is `true`, which it is whenever a transcription service labels speakers. List the parts in recording order; a break between two parts, such as a paused meeting, is fine. Set `single_recording` only when every file of the step comes from the same recording; separately recorded or uploaded files stay separate. The setting takes effect when the run labels speakers with the external speaker service and the step has more than one file; otherwise each part is transcribed on its own, as without it. When it takes effect, the parts together must fit within the step's audio length limit, and a longer recording fails its transcription step with `typed_io_audio_exceeds_limit`. Each part keeps its own place in the transcript, so playback, corrections and regeneration work per file as before. Only audio steps accept it; other steps receive `400 flow_run_single_recording_requires_audio_step`. It is part of the request's idempotency fingerprint, and a retry keeps it. A step result reports it as `runtime_input_single_recording`, so a new run started with the same files can send it again.
