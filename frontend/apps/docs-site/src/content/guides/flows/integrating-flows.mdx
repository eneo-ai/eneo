...
The stored text is the authoritative done text. Timed passages are kept only when every delta supplies valid timing and the delta text matches the done text exactly; otherwise the text is stored without passages. Unbound transcripts use the tenant's runtime-upload abandonment retention period at creation. Once expired, they are removed in bounded batches by the tenant administrator's Flow history purge, within its selected tenant, space, or flow scope. A dry run deletes nothing. If the retention period is disabled, transcripts have no expiry. Bound transcripts are excluded from this purge.
