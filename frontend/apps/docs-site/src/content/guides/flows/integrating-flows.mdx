...
Live text is a preview. Eneo can store the final text of a clean session and return its identity; it keeps no audio from the session. Record the audio in your app as usual. When the recording ends, upload it and create the run; the flow's transcription model still transcribes the uploaded file. A stored live transcript does not yet change run execution.
...
6. **Upload and run.** Upload the recording to the audio step and create the run, as in the [worked example](#worked-end-to-end-example).
