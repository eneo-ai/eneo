...
5. **Stop.** When the recording ends, send the text frame `{"type": "stop"}`. Eneo answers with `transcript.done`, which carries the full preview text, and closes the socket.
...
| Eneo to app | `{"type": "transcript.done", "text": "Hej världen"}`                            | The full preview text after `stop`. The socket then closes                                                  |
| Eneo to app | `{"type": "error", "code": "idle_timeout", "message": "...", "retryable": false}` | The session has ended. The socket then closes                                                               |

`message` is for people; branch on `code`. When `retryable` is `true`, request a new ticket to continue the preview; the recording itself is unaffected. Every code is listed in [Live transcription socket codes](/guides/flows/reference/errors#live-transcription-socket-codes).
...
#### Relay the socket through your backend

Service keys and module tokens must stay on your server, and the socket accepts a browser page only when the page is served from the API's own host or from an origin the operator allows. An app that calls Eneo with server-side credentials therefore relays the socket through its own backend:

1. The browser streams the audio frames to your backend over a WebSocket of your own.
2. Your backend requests the ticket and opens the Eneo socket. A server-side client that sends no `Origin` header is accepted.
3. Your backend forwards audio frames to Eneo and events back to the browser.

The relay works when the browser receives `ready` through your backend. A module requests the ticket with its `sk_` key and the user's module token; see [Call Flows as the signed-in user](/docs/module-authentication#call-flows-as-the-signed-in-user).
