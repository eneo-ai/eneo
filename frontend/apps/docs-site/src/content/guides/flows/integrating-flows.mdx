...
A flow that transcribes recorded audio reports two options in the run contract's `transcription` object: whether your app can show live text while someone records, and whether a run may choose speaker labels. For any other flow, `transcription` is `null`.
