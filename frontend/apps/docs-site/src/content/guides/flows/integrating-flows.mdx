...
| Robustness                          | Use idempotency, version pins, polling backoff, and status capabilities for retry-safe clients.                | `POST /api/v1/flows/{id}/runs/` (`create_flow_run`)                                                              |
...
Use idempotency, version pins, polling backoff, and status capabilities for retry-safe clients.
...
3. Back off polling when `status_capabilities.should_poll` is false.
4. Use run and step polling as the source of truth for status changes.
...
Errors you must handle: `flow_run_idempotency_conflict`, `flow_run_concurrency_limit_reached`, `flow_worker_stalled`.
