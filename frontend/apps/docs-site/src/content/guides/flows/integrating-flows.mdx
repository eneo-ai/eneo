...
Live text is a preview. It never becomes the run's transcript, and Eneo keeps no audio or text from the session. Record the audio in your app as usual. When the recording ends, upload it and create the run; the flow's transcription model then transcribes the uploaded file.
...
1. **Request a ticket** when the recording starts:
   ...
   `{step_id}` is the audio step from `steps_requiring_input`. The request has no body. The response is `201 Created`:
...
5. **Stop.** When the recording ends, send the text frame `{"type": "stop"}`. Eneo answers with `transcript.done`, which carries the full preview text of this session, and closes the socket.
...
| App to Eneo | `{"type": "stop"}`                                                              | The recording has ended. Any other text frame ends the session with `invalid_message`                      |
