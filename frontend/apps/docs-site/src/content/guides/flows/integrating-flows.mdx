...
The transcript must belong to the same tenant, user, flow, published version, step, and transcription model, and must not have expired. Admission binds it permanently to the uploaded file. The same transcript can be submitted again for that file; another file receives `409 flow_run_live_transcript_already_bound`. An unavailable, expired, or foreign transcript receives `404 flow_run_live_transcript_not_found`. A step without exactly one audio file receives `400 flow_run_live_transcript_requires_one_audio_file`. The ID is part of the request's idempotency fingerprint.
