...
4. Use `inline_text` directly and treat `outbound_http.delivery_status: delivered` as a receipt, not as a copy of destination configuration or payload data.
5. Call `runtime_paths.list_steps_template` only when the UI needs intermediate results, diagnostics, or step progress.
6. Call `/api/v1/flows/runs/status-capabilities/` to get status meaning instead of hardcoding transitions.
