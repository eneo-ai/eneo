...
Run-history settings define when data becomes eligible for an administrator-requested purge. Configuring a window does not delete anything, and no scheduled worker purges Flow-owned data. Runs and their files persist until an administrator explicitly previews, confirms, and starts a purge. Because create-run idempotency replay is tied to the retained run row, a completed purge also ends replay against that historical row.
...
Once an administrator-requested purge has removed a run row, its create-run `Idempotency-Key` no longer matches anything, so replaying that key creates a new run instead of returning the old one. Keep the run id, not the key, as your durable handle.
