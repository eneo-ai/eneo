...
| **Flow worker**    | Celery                   | -            | Flow runtime execution                             |
| **Flow scheduler** | Celery beat              | -            | Flow stale-run recovery and outbox delivery        |
| **Database**       | PostgreSQL 16 + pgvector | 5432         | Data storage, vector search                        |
| **Redis**          | Redis 7                  | 6379         | Cache, ARQ queue, and Celery broker                |
...
`repository@sha256:<digest-suffix>`. One backend digest supplies the API, ARQ
worker, both Celery workers, Celery beat, and database initialization roles.
