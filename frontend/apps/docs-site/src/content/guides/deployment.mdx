...
Deploy Eneo in production with Docker Compose. This page is the primary
deployment guide: it takes you from prerequisites to a verified installation,
then links to focused guides for storage, authentication, and AI providers.
...
| Your task                           | Go to                                                                                                                               |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Install a new production instance   | [Quick start](#quick-start)                                                                                                         |
| Upgrade an existing instance        | [Updating Eneo](#updating-eneo)                                                                                                     |
| Choose PostgreSQL or object storage | [Choose Content Storage](/guides/object-content-storage)                                                                            |
| Configure OIDC or SSO               | [Single-Tenant OIDC](/guides/oidc-federation/single-tenant) or [Multi-Tenant OIDC Federation](/guides/oidc-federation/multi-tenant) |
| Configure model providers           | [AI Provider Configuration](/guides/ai-providers)                                                                                   |
| Add an optional module              | [Optional modules](#optional-modules)                                                                                               |
| Diagnose a failed deployment        | [Troubleshooting](#troubleshooting)                                                                                                 |

<Callout type="info">
  You do not need S3-compatible storage to start Eneo. The default deployment
  keeps bounded durable content in PostgreSQL. Add SeaweedFS, MinIO, or another
  compatible endpoint only when your capacity and recovery requirements call for
  it.
</Callout>

## Architecture overview

Eneo uses a small service stack. Durable content defaults to bounded
PostgreSQL-inline bytes; an optional private object-store byte plane can be
enabled when needed.

<div className="bg-white dark:bg-white p-4 rounded-lg border border-gray-200 dark:border-gray-700 my-8">

![Eneo System Architecture showing Client Layer, Reverse Proxy (Traefik), Frontend (SvelteKit), Backend API (FastAPI), Database (PostgreSQL with pgvector), Redis Cache/Queue, ARQ workers, and AI Providers](/diagrams/eneo-architecture.svg)
...
| **General worker**     | ARQ                      | -            | Uploads, crawling, integrations, and audit jobs        |
| **Execution worker**   | ARQ                      | -            | Flow runtime execution on its own capacity queue       |
| **Maintenance worker** | ARQ                      | -            | Flow recovery, expiry, and outbox scheduling           |
| **Database**           | PostgreSQL 16 + pgvector | 5432         | Data storage, vector search                            |
| **Redis**              | Redis 7                  | 6379         | Cache and independent ARQ task queues                  |
...
<Callout type="info">
  **Traefik is provided as an example reverse proxy**, not a requirement. You
  can use nginx, HAProxy, Caddy, or any reverse proxy that supports the [routing
  requirements](#reverse-proxy-requirements) below.
</Callout>
...
- **An API key for at least one AI provider.** Providers are added after the
  first login in **Admin → Models** ([see AI Provider
  Configuration](/guides/ai-providers))
...
- set all five required base-stack image digest suffixes in `.env`;
- leave the optional object-store block commented unless you have chosen and
  prepared that storage path.
...
curl -O https://raw.githubusercontent.com/eneo-ai/eneo/develop/docs/deployment/env_module_ttt.template
```

<Callout type="info">
  The `develop` branch carries the deployment files for the next release. If you
  run a released version, take the files from that release's tag or branch
...
Set all five image digest suffixes in `.env`. Enter only the 64-character
hexadecimal value after `sha256:`:
...
Redis release digests for the remaining values. Preserve the complete set with
the deployment backup.

Edit `env_db.env`:

- `POSTGRES_PASSWORD` - paste the generated value (the template ships
  `POSTGRES_USER=postgres` and `POSTGRES_DB=eneo`; keep them unless you know
...
- `JWT_SECRET` - listed for parity with the backend; the SvelteKit app does
  not read it
- `ENEO_BACKEND_URL` - the external backend URL used by server-side rendering
  (e.g., `https://eneo.example.com`)
- `ENEO_BACKEND_SERVER_URL` - the internal Docker URL that SSR requests are
  rewritten to, skipping the reverse proxy (`http://backend:8000`)
- `PUBLIC_ENEO_BACKEND_URL` - the backend origin used by the browser
  (e.g., `https://eneo.example.com`). **Do not append `/api`**: the client
  adds `/api/v1/...` itself.
- `PUBLIC_ORIGIN` - your external URL (e.g., `https://eneo.example.com`)
...
After setting the required image digests, the default `.env` needs no
object-store values. Eneo uses bounded PostgreSQL-inline content. To enable the
optional SeaweedFS profile or an external compatible endpoint, follow
[Object Content Storage](/guides/object-content-storage) before deployment.
...
Then add your first model provider in **Admin → Models** (see
...
| --------------------------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `proxy_tier` (external)                 | Traefik, frontend, backend, general worker, execution worker, maintenance worker | Ingress and outbound access for APIs, OIDC, crawls, model calls, and webhooks |
| `data_net` (`internal: true`)           | db, redis, backend, all three workers, db-init                                   | Data layer without internet egress, isolated from Traefik and frontend        |
| `module_net` (`internal: true`)         | Traefik, backend, optional module containers                                     | Modules reach only `backend:8000`; no internet egress                         |
| `object_content_net` (`internal: true`) | optional object-content, backend, general worker, execution worker               | Private S3-compatible byte plane when enabled; never routed publicly          |
...
The templates in
[`docs/deployment/`](https://github.com/eneo-ai/eneo/tree/develop/docs/deployment)
...
<Tabs items={['Backend', 'Frontend', 'Database']}>
  <Tabs.Tab>
    ...
    **Security (required):**
    ...
    | `FILE_REFERENCE_BASE_URL` | unset (falls back to `PUBLIC_ORIGIN`) | Base URL written into the signed file links handed to MCP tools. Set it when MCP servers reach the backend on another host than browsers do. See [Signed file references](#signed-file-references). |
    ...
    **Flows live transcription:**

    | Variable | Default | Description |
    |----------|---------|-------------|
    | `FLOW_LIVE_TRANSCRIPTION_IDLE_TIMEOUT_SECONDS` | `300` | Ends a live transcription session that receives no audio for this many seconds. A recording sends audio the whole time, silence included, so only a paused or stalled recording reaches it |
    | `FLOW_LIVE_TRANSCRIPTION_FINAL_TEXT_TIMEOUT_SECONDS` | `60` | How long Eneo waits for the model server's final text after a recording stops. Raise it for a server that falls behind, such as a CPU server or a busy GPU |

    Both values must be greater than zero. A live session also accepts at most
    `FLOW_AUDIO_MAX_DURATION_SECONDS` of audio (template value `18000`, 5 hours),
    the same limit as an uploaded recording. To set up a model for live
    transcription, see [Live transcription](/guides/ai-providers#live-transcription).
    ...
    **Dynamic Credential Management:**
    ...
    <Callout type="info">
      When `TENANT_CREDENTIALS_ENABLED=true`, you can add, update, or rotate AI provider API keys per tenant via the [Credentials API](#credentials-api) without restarting the backend. Credentials are encrypted with Fernet (AES-128-CBC + HMAC). Requires `ENCRYPTION_KEY` to be set; the endpoints answer 404 while the flag is `false`.
    </Callout>

    **Integrations:**

    <Callout type="info">
      **Setting up SharePoint?** Webhook subscriptions cannot be created until `SHAREPOINT_WEBHOOK_CLIENT_STATE` is set; Microsoft echoes it back as `clientState` and Eneo rejects notifications that do not match. See the [SharePoint Integration Guide](/guides/sharepoint-integration) for complete setup instructions including Azure AD app registration.
    </Callout>
  ...
  <Tabs.Tab>
    ...
    <Callout type="warning">
      **Important:** Eneo frontend uses **SvelteKit**, NOT Next.js. Use `ENEO_*` variables, not `NEXT_PUBLIC_*`.
    </Callout>
    ...
    **Required:**
    ...
    | Variable | Template value | Description |
    |----------|----------------|-------------|
    | `ENEO_BACKEND_URL` | `https://your-domain.com` | Backend URL for server-side rendering: the **external** https URL |
    | `ENEO_BACKEND_SERVER_URL` | `http://backend:8000` | Internal server-to-server URL. SSR requests to `ENEO_BACKEND_URL` are rewritten to this address, skipping the reverse proxy |
    | `PUBLIC_ENEO_BACKEND_URL` | `https://your-domain.com` | Backend origin used by the browser. **Bare origin, no `/api`** - the client appends `/api/v1/...` |
    | `ORIGIN` | `https://your-domain.com` | Cookie/CORS origin (same as `PUBLIC_ORIGIN`) |
    | `PUBLIC_ORIGIN` | `https://your-domain.com` | External URL; required for OIDC login and logout |
    | `JWT_SECRET` | empty | Listed for parity with the backend; the web app does not read it |

    OIDC and federation settings are read by the backend, so set them in
    `env_backend.env`. The frontend template lists the same
    `OIDC_*`/federation keys so the two files can be kept in sync; the web app
    itself needs only `PUBLIC_ORIGIN` for the login flow.

    **Optional:** the web app reads the UI flags `SHOW_WEB_SEARCH` and
    `SHOW_HELP_CENTER`, plus `HELP_CENTER_URL` and
    `REQUEST_INTEGRATION_FORM_URL`. The remaining commented lines in
    `env_frontend.template` (`FEEDBACK_FORM_URL`, `SUPPORT_EMAIL`,
    `SALES_EMAIL`) and `LOGLEVEL` are not read by the current web app.
    ...
    <Callout type="info">
      **Docker networking tip:** In Docker, `localhost` means "this container" - not your host machine. Use the Docker service name `backend` for container-to-container communication:
      - `ENEO_BACKEND_SERVER_URL=http://backend:8000` (correct - uses service name)
      - `ENEO_BACKEND_SERVER_URL=http://localhost:8000` (wrong - will fail)
    </Callout>
  ...
  <Tabs.Tab>
    ...
    `env_db.env` initializes the PostgreSQL container. The same three values
    must appear in `env_backend.env`, and the Compose healthcheck runs
    `pg_isready -U postgres`.

    | Variable | Template value | Description |
    |----------|----------------|-------------|
    | `POSTGRES_USER` | `postgres` | Database user |
    | `POSTGRES_PASSWORD` | `changeme` | Database password (**change from default**) |
    | `POSTGRES_DB` | `eneo` | Database name |
...
Both Eneo's built-in file tool and external MCP servers receive the same
link. The built-in tool verifies the token locally and never makes an HTTP
request. An external MCP server redeems the link over HTTP, so **the Eneo
backend must be reachable from the network where that MCP server runs**.
This is independent of where the file bytes live: originals stored in
PostgreSQL and originals stored in object storage are served through the
same endpoint.
...
- Every redemption is audited as `file_original_downloaded` under **File
  operations**, with the caller's address and user agent, next to the
  `file_original_download_link_created` entry written when the link was
  minted.
- The token is stripped from tool-call arguments and results before they are
  stored, shown in the chat, or replayed to the model. Only the tool that was
  called ever holds the live link. Deleting the file revokes every
  outstanding link for it.
...
On a private network with no route from the MCP server to the backend, the
tool call fails. The model is told the link is valid, so it reports that the
tool cannot reach the file rather than asking the user to upload it again.
The fix is always network reachability or a corrected base URL, never a
re-upload.

## Object Content Storage

<Callout type="warning">
  When object-store rows exist, PostgreSQL and that endpoint form one recovery
  unit. Inline-only bytes are already part of PostgreSQL backups.
</Callout>

The reference stack defaults to bounded PostgreSQL-inline content. An optional
Compose profile runs private, Eneo-built SeaweedFS. Deployments that need an
existing storage platform or multi-node availability can instead use another
compatible endpoint, including MinIO. Both object-store paths use the same
contract and fail closed when configuration or the database/bucket binding does
not match.
...
Policy changes take effect without a backend or worker restart. They affect
eligible new writes only; existing content stays in its recorded backend until
an explicit verified migration moves it. The product has one provider-neutral
object-store path, not vendor-specific storage modes. **Admin > File storage** also
compares Eneo's recorded file-content size with the entire PostgreSQL database
on disk. These figures overlap: inline file content is part of both totals, and
the PostgreSQL figure also includes searchable text, pgvector embeddings,
tables, indexes, and internal storage. The page does not report remaining host
or bucket capacity. See [What do the storage-usage figures
mean?](/guides/object-content-storage#what-do-the-storage-usage-figures-mean)
for the exact boundaries.

The same page presents current status before configuration: the active target,
object-store health, file-content distribution, and migration state appear
first. PostgreSQL allocation and detailed effective limits remain available as
technical details instead of competing with routine storage decisions.

The page also shows the portable multipart envelope when it constrains
object-store session
uploads. FastAPI/Starlette multipart parsing happens before route admission and
may use temporary disk. Eneo rejects an oversized File or Icon before its own
capture/spool or any storage mutation. Operators must use ingress/request-body
limits and configure and monitor temporary-disk capacity to protect that earlier
parsing boundary.
...
| File                                                                                                                    | Purpose                                                                                                                             |
| ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| [`docker-compose.modules.yml`](https://github.com/eneo-ai/eneo/blob/develop/docs/deployment/docker-compose.modules.yml) | The overlay. Every module sits behind a profile (`speech-to-text`, or `all-modules`) and waits for the backend `/api/healthz` check |
| [`env_modules.template`](https://github.com/eneo-ai/eneo/blob/develop/docs/deployment/env_modules.template)             | Defaults shared by all module containers (`LOG_LEVEL`, `NODE_ENV`, `ENEO_API_KEY_HEADER_NAME`)                                      |
| [`env_module_ttt.template`](https://github.com/eneo-ai/eneo/blob/develop/docs/deployment/env_module_ttt.template)       | Per-module secrets for the speech-to-text module (`TAL_TILL_TEXT_FLOW_ID`, `ENEO_API_KEY`, `SESSION_SECRET`)                        |
| [`MODULES.md`](https://github.com/eneo-ai/eneo/blob/develop/docs/deployment/MODULES.md)                                 | Operator guide: install the module in **Admin → Modules**, mint the service key, smoke test, rotate keys, uninstall                 |

The module domain must point at the same Traefik ingress; the overlay routes
it over `eneo_module_net`. See [Module
Authentication](/docs/module-authentication) for the login handoff, ticket
exchange, and session refresh contract, and
[Module administration](#module-administration) below for the admin-side
prerequisites.
...
| `/scim/*`       | Backend (port 8000)  | SCIM 2.0 provisioning (mounted at `/scim/v2`; see [SCIM](/guides/scim-provisioning)) |
...
- WebSocket support on `/api/*`. The backend serves the `/api/v1/ws` socket and, for live transcription, `/api/v1/live-transcription`. A proxy that upgrades only `/api/v1/ws` needs the same rule for `/api/v1/live-transcription`.
...
<Callout type="info">
  **Service Account Pattern:** We recommend creating a dedicated admin account
  (service account) for automated user provisioning via the admin API endpoints,
...
<Callout type="warning">
  **Prerequisite:** Set `TENANT_CREDENTIALS_ENABLED=true` and `ENCRYPTION_KEY`
  ...
  [Dynamic Credential Management](#environment-configuration) in the Backend
  configuration.
</Callout>

<div className="bg-white dark:bg-white p-4 rounded-lg border border-gray-200 dark:border-gray-700 my-8">

![LiteLLM Architecture showing credential flow from Sysadmin API to encrypted storage to AI providers](/diagrams/litellm-overview.svg)
...
<Callout type="info">
  To use the web crawler or upload documents for processing, the **worker
  service must be running** and at least one **embedding model must be enabled**
  in Admin → Models → Embedding models.
</Callout>

### Apps (Voice/Audio Features)

<Callout type="info">
  **Can't create Apps?** This feature requires a transcription model. Go to
  Admin → Models → Transcription models tab and enable a model like Whisper.
</Callout>
...
| Flow remains queued                   | Execution worker not running                            | Check `task-execution-worker`; its queue is separate from the general worker                   |
| Flow recovery or outboxes are delayed | Maintenance worker not running                          | Check `task-maintenance-worker` and `/api/healthz/flows`                                       |
...
| SCIM provisioning returns 404         | Reverse proxy does not forward `/scim`                  | Route `/scim/*` to the backend (see [Reverse Proxy Requirements](#reverse-proxy-requirements)) |
...
The backend is not published on a host port in the reference stack, so run
these through your public domain (or from inside a container).
...
# Crawler queue and worker diagnostics
...
# Last 100 lines for the API and all worker roles
...
<Callout type="warning">
  The production Compose file requires immutable digests for Traefik, frontend,
  backend, pgvector, and Redis. Missing or empty digest inputs stop Compose
  before deployment.
</Callout>

Set the five digest suffixes in `.env`. Compose builds each image identity as
`repository@sha256:<digest-suffix>`. One backend digest supplies the API,
general ARQ worker, platform execution worker, platform maintenance worker, and
database initialization roles.

**Find Eneo image digests:**

- GitHub Releases: https://github.com/eneo-ai/eneo/releases
- Download `IMAGE-DIGESTS.txt` from the matching release.

<Callout type="info">
  Review release notes and test a complete digest set before upgrading. Keep the
  previous set with the matching backup for rollback.
</Callout>

### Backups

<Callout type="warning">
  **Take one paired recovery point when object-store rows exist.** Inline-only
  control and bytes are both included in the PostgreSQL backup.
</Callout>
...
gunzip -c backup_20250108.sql.gz | docker compose exec -T db psql -U postgres eneo
```
...
If object-store rows exist, also archive `eneo_eneo_object_content_data` while
the backend, all three workers, and the object-content service are stopped, or
take the matching versioned snapshot in your external provider. Follow the
[object-content backup and restore
procedure](/guides/object-content-storage#back-up-and-restore), including
checksum and skew-recovery checks.

### Updating Eneo

<Steps>
### Update the image digest set

Replace all five digest suffixes in `.env`. Keep the previous `.env` with the
matching backup.
...
Keep `backend`, `worker`, `task-execution-worker`, and
`task-maintenance-worker` stopped through the recovery point and `db-init`.

Follow the [File and Icon storage upgrade
checklist](/guides/file-icon-storage-upgrade#before-you-upgrade). Keep the API
closed until the new migration and same-release worker are ready.

### Take the stopped-writer recovery point

After the drain is verified and every old writer is stopped, create the database
backup:
...
If object-store rows exist, take the matching endpoint snapshot while writers
remain stopped and label both artifacts as one recovery point. Inline-only
content needs no second backup artifact.
...
docker compose up -d
docker compose ps
curl -fsS https://eneo.example.eu/version
```
...
docker compose \
  ...
  -f docker-compose.yml \
  -f docker-compose.object-content.yml \
  stop backend worker task-execution-worker task-maintenance-worker
docker compose \
  --profile object-content \
  -f docker-compose.yml \
  -f docker-compose.object-content.yml \
...
When any object-store endpoint is configured, verify its explicit readiness
code. HTTP 200 alone can also represent an intentionally degraded object store
while inline content remains available:
...
- `active` needs no operator action beyond capacity and application monitoring.
  Confirm `complete` with the durable campaign-state query in the storage guide;
  an empty installation may emit no completion log.
- `waiting_for_capacity` means Eneo remains available, but the estimated inline
  copy exceeds the automatic threshold. Confirm payload, WAL, replica, backup,
  and safety headroom before setting the reported acknowledgement.
- `waiting_for_object_store` means this release cannot adopt legacy bytes
  directly to the selected object store. Selecting PostgreSQL inline changes
  the deployment-wide target for eligible new writes. Keep it selected until
  adoption is complete, then reselect object storage and queue verified moves,
  or wait for a compatible release.
- `halted` means the destination changed, or an invalid, oversized, or
  retry-exhausted item needs action. Fix the logged cause before increasing the
  resume revision. The worker retries older failures in bounded scheduled
  batches; a repeated failure waits for the next higher revision.

Do not edit the ledger or remove the database trigger. Existing legacy content
remains readable while the worker waits or is stopped. See [Upgrade File & Icon
Storage](/guides/file-icon-storage-upgrade) for capacity examples, realistic
duration, state queries, pause behavior, rollback, and cleanup.
...
**Rollback:** Restore the previous `.env` digest set, stop writes, and restore
the paired backup. PostgreSQL-inline and external-endpoint deployments use plain
...
docker compose \
  ...
  -f docker-compose.yml \
  -f docker-compose.object-content.yml \
  stop backend worker task-execution-worker task-maintenance-worker object-content
# Restore the matching PostgreSQL dump and object-store snapshot as described
# in the linked procedure, then start the previous digest set:
docker compose \
  --profile object-content \
  -f docker-compose.yml \
  -f docker-compose.object-content.yml \
...
Restore the matching object-store snapshot before reopening traffic. See
[Back up and restore](/guides/object-content-storage#back-up-and-restore) for
skew recovery.
...
- [SCIM Provisioning](/guides/scim-provisioning) - Automated user and group provisioning
- [SharePoint Integration](/guides/sharepoint-integration) - Connect SharePoint and OneDrive
- [Module Authentication](/docs/module-authentication) - Login handoff contract for optional modules
