...
Follow the exact job query and recovery sequence in [Upgrade, restore, and
rollback](/guides/object-content-storage#upgrade-restore-and-rollback). Keep the
API closed until the new migration and same-release worker are ready.
...
`db-init` expands the schema and inventories legacy File/Icon variants; it does
not copy payload bytes. The new worker adopts them online in bounded batches.
Follow its state after the deployment:
...
remains readable while the worker waits or is stopped. See [Adopt legacy File
and Icon
content](/guides/object-content-storage#adopt-legacy-file-and-icon-content) for
capacity examples, the state query, pause behavior, and recovery rules.
