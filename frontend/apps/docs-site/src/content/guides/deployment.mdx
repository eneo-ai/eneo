...
<Callout type="info">
  **Rollback procedure:** If issues occur after an update, restore from your
  backup: ```bash docker compose down gunzip -c backup_YYYYMMDD.sql.gz | docker
  compose exec -T db psql -U eneo eneo docker compose up -d ``` If using pinned
  versions, also revert the version tags in `docker-compose.yml`.
</Callout>
