...
<Tabs items={['Backend', 'Frontend', 'Database']}>
  <Tabs.Tab>
    ...
    **Upload policy:**
    ...
    <Callout type="info">
      Platform admins set byte limits and new-write placement in **Admin >
      Storage**. Those changes take effect without restarting backend or worker.
...
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Platform admin | Choose one deployment-wide target for eligible new File and Icon writes and set business upload limits in **Admin > Storage**           |
...
shows the portable multipart envelope when it constrains object-store session
