...
Eneo uses a small service stack with a separate durable byte plane.
...
<Callout type="info">
  **Preflight Checklist** - Before running `docker compose up`:
  ...
  - [ ] Configure `.env`, including a verified object-store image digest, credentials, and stable deployment UUID
...
- pin `ENEO_SEAWEEDFS_IMAGE` to the digest reference in the single
  `seaweedfs manifest` row of the release's `IMAGE-DIGESTS.txt`, not an
  architecture-specific SBOM row;
- generate independent high-entropy `OBJECT_CONTENT_ACCESS_KEY_ID` and
  `OBJECT_CONTENT_SECRET_ACCESS_KEY` values;
- generate `OBJECT_CONTENT_DEPLOYMENT_ID` once with `uuidgen` and preserve it
  through upgrades and restores; changing it makes existing object-content rows
  unavailable and requires a separately verified migration.
...
| `object_content_net` (`internal: true`) | object-content, backend, worker | Private S3-compatible byte plane; never routed publicly |
...
atomically pairs it with a private marker in the configured bucket. Startup,
readiness, and every reconciliation run require the same pair before mutating
content state. A reachable empty or foreign bucket reports
`configuration_required`; Eneo never silently adopts or overwrites it. The
marker is internal and must be preserved with the bucket backup—it is not an
administrator or tenant setting.
...
<Callout type="warning">
  **Always take one paired recovery point before updating.** A PostgreSQL dump
  without the matching object-content snapshot—or the reverse—is incomplete.
</Callout>
...
Also archive `eneo_eneo_object_content_data` while backend, worker, and the
object-content service are stopped, or take the matching versioned snapshot in
your external provider. Follow the [paired backup/restore procedure](https://github.com/eneo-ai/eneo/blob/develop/docs/deployment/OBJECT_CONTENT.md#paired-backup-and-restore),
including checksum and skew-recovery checks.
...
Take the matching object-content snapshot before resuming writes.
