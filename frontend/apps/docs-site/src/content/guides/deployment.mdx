...
- pin `ENEO_SEAWEEDFS_IMAGE` to the exact attested manifest digest from the
  release's `IMAGE-DIGESTS.txt`;
...
This configuration is a deployment prerequisite: backend and worker refuse to
start when the mandatory object-content settings are absent or invalid, even
before an existing byte producer adopts the foundation.
