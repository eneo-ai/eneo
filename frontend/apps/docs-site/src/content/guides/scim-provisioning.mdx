...
A token is shown **only once** on creation. Eneo stores only a SHA-256 hash of the token — there is no way to retrieve it later. If a token is lost, issue a new one; creating a token replaces the existing one.

The sysadmin endpoints are authenticated with the super admin API key in the `X-API-Key` header (`ENEO_SUPER_API_KEY` in the backend `.env`).
...
The response (`201 Created`) contains the raw token, a 43-character URL-safe random string with no prefix:

```json
{
  "tenant_id": "f1e2d3c4-...",
  "token": "kY0v3nZ8QwF1bT9eL2mN7pR4sU6xA0cD5gH8jK1lM3o"
}
```
...
The response is `204 No Content`. After revocation, all SCIM requests to that tenant return `401 Unauthorized`. To rotate a token, simply call the create endpoint again — the new token replaces the old one immediately, so update the IdP configuration right away.
...
For Group mappings, the defaults also work. Members are referenced by the user's SCIM resource `id` (the Eneo user UUID returned when the user was provisioned) in `members[].value`, not by `externalId`.
...
Group members are referenced by the user's SCIM resource `id` (the Eneo user UUID) in `members[].value`. A user must therefore have been provisioned through SCIM — so the IdP knows their Eneo `id` — before they can be added to a group, and every member must belong to the same tenant (`400 Bad Request` otherwise). Check the user provisioning log first — group sync runs after user sync.
...
- **Token rotation**: SCIM tokens do not expire. Rotate them periodically by creating a new token — it replaces the old one — and updating the IdP.
...
- See the [Authentication & OIDC guides](/guides/oidc-federation) for SSO setup that pairs with SCIM
