...
cleanup sequence. Two consecutive Alembic revisions own the expand phase. The first
creates the ledger, makes legacy columns nullable, installs the payload-write
fence, and commits. The second scans owners and obtains exact logical source
...
[Verify and clean up](/guides/file-icon-storage-upgrade#verify-and-clean-up).
...
(for example `4.40-eneo.3`). Because every input is pinned, the build is
reproducible and runs only when those inputs change on `develop`. Each version
