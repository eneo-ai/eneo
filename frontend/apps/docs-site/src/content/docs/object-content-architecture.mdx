...
Legacy File and Icon adoption uses an expand, online backfill, and optional
cleanup sequence. Two consecutive Alembic revisions perform the initial expand
and inventory; later 2.2 revisions add admission, recovery, pause, and cleanup
tracking. The first creates the ledger, makes legacy columns nullable, installs
the payload-write fence, and commits. The second scans owners and obtains exact logical source
sizes in resumable, idempotent groups without hashing, converting, or copying
external payload bytes. It never inherits
...
only source-bearing rows ready. The initial capacity decision is frozen after
admission. If an adopted reference later fails verification, its replacement
is re-admitted against the cumulative logical-byte total and requires a higher
acknowledgement only above the automatic threshold. The worker adopts each ready
variant in bounded, resumable PostgreSQL-inline transactions. The optional 2.2
cleanup command applies the fail-closed completion check, verifies every
retained legacy source against its adopted copy and every surviving ledger
reference, and only then removes the legacy columns, as described in
[Clean up the old columns](/guides/file-icon-storage-upgrade#clean-up-the-old-columns-optional).
...
flowchart LR
  ...
  complete --> contract["Optional cleanup\nEneo stopped · exclusive lock · full recheck · legacy removal"]
```
...
Administrators with the Storage permission use **Admin > File storage** to choose
...
A record has exactly one byte backend at a time. Only an explicit move changes
it, after the copy at the destination is verified. Eneo never silently retries
an inline write in object storage, or copies a failed object-store write into
...
available control row, and exactly one first owner reference atomically. New
File, Icon, and knowledge uploads to object storage reserve an opaque key,
upload and verify the bytes, and then commit available content and its first
reference in one short transaction. The older `pending` intent path remains for
existing rows and recovery.
...
```mermaid
stateDiagram-v2
  [*] --> available: inline payload or verified object + first reference
  [*] --> pending: older object intent (existing rows, recovery)
...
PostgreSQL reads the exact logical size. Oversized content is rejected before the
more expensive SHA-256 and copy. The content repository then verifies the
expected size and digest while copying accepted frozen bytes directly into the
inline payload table. The Python worker coordinates the transaction but never
materializes those bytes. Payload, content control, reference, and ledger
completion commit atomically. Legacy text requires PostgreSQL `server_encoding`
`UTF8`, which makes the raw text byte length an exact, non-converting preflight;
other encodings fail before byte production. A deleted owner is cancelled, not
recreated. If a reference exists but its content is unavailable, the same
transaction creates a verified replacement, switches the reference, and lets
the normal lifecycle decrement the failed content's reference count and schedule
cleanup when no references remain. An invalid source, integrity failure, or
oversized payload fails its item immediately;
...
uploads captured bytes under a bounded PostgreSQL reservation, then checks them
with `HEAD` and a full read-back SHA-256,
...
Connections created in **Admin > File storage** generate and persist it
...
The image has its own workflow and its own version, `docker/seaweedfs/VERSION`
(for example `4.40-eneo.4`). Because every input is pinned, the build is
reproducible and runs on `develop` only when those inputs, the reference
Compose files, or the workflow change. Each version
tag is published once and never moved; changing the source pin, the downstream
patch, or the builder or runtime images means bumping the version. Application
releases record which store version they were verified against instead of
rebuilding it.
...
Only `develop` runs the privileged attestation and publication jobs. Pull
requests and manual runs from other branches run the same source policy, build,
reproducibility check, smoke test, SBOM generation, and vulnerability gate
against an ephemeral local registry with read-only permissions, so a broken
image is caught before merge without anything reaching the public registry. A
weekly scheduled run repeats the source policy and scans both published
platform digests with the current vulnerability databases without rebuilding
or publishing, so new advisories surface as a red scheduled run instead of a
blocked application deployment.
...
- The optional cleanup command fails closed until legacy adoption is complete,
  then locks the tables and verifies every retained legacy source and every
  surviving ledger reference as `available` in the same transaction that drops
  the legacy columns. Operators choose when to close that recovery window.
