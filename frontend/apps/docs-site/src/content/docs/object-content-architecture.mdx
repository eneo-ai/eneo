...
sequence. The expand migration creates the ledger, makes legacy columns
nullable, freezes their payload facts with database triggers, and inventories
stored-size estimates without copying or hashing the payloads. New code reads a
verified object-content reference when present and otherwise reads the frozen
legacy source. A scheduled worker then adopts each variant in bounded,
resumable PostgreSQL-inline transactions. The later contract must refuse to
remove the legacy schema until no actionable ledger item remains.

```mermaid
flowchart LR
  expand["Expand\nschema · inventory · write fence"] --> fallback["Read fallback\nlegacy remains authoritative"]
  fallback --> backfill["Online backfill\nbounded lease · SHA-256 · reference flip"]
...
legacy payload immutable, while the per-item transaction locks the owner and
rechecks whether another path already published the same variant before it
creates the inline payload and reference atomically. A deleted owner is
cancelled, not recreated. An invalid source, integrity failure, or oversized
payload fails its item immediately; interrupted attempts retry until their
configured limit. Failed items halt the campaign with a stored reason and
require a higher operator resume revision after correction.
