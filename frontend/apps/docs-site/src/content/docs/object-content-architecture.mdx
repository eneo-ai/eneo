...
source. A scheduled worker then adopts each variant in bounded, resumable
PostgreSQL-inline transactions. The later contract must refuse to remove the
legacy schema until no actionable ledger item remains.
