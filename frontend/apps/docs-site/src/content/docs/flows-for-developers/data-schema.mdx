...
- Every table with a `tenant_id` column references `tenants` with `ON DELETE CASCADE`. Those lines are omitted; deleting a tenant deletes all of its Flow rows.
...
erDiagram
  ...
  flow_run_audit_outbox }o--|| flows : "RESTRICT"
  ...
  flow_run_step_input_files }o--|| flows : "CASCADE"
...
erDiagram
  ...
  flow_run_review_checkpoints }o--|| flows : "CASCADE"
