...
erDiagram
  flow_provider_calls {
    ...
    uuid resolved_inputs_attempt_id "nullable"
    integer ordinal "required"
    varchar call_kind "required"
    varchar status "required"
    ...
    numeric audio_seconds "nullable"
  ...
  flow_provider_calls }o--o| flow_step_attempt_resolved_inputs : "resolved_inputs_attempt_id ondelete=NO ACTION"
...
| `flow_provider_calls`                    | `FlowProviderCalls`                   | Run execution, results, and files        | one durable lifecycle row per Flow provider request.               | FlowProviderCallRepository.                  | preserve credential-free request identity, terminal outcome, token or audio usage, and mapped source evidence outside attempt JSONB. |
