...
<details>
<summary>19 error codes</summary>
...
| `flow_worker_stalled`             | Run error payload                  | The worker stopped updating an active run within the recovery window. | Redispatch only if capabilities allow it, then escalate recurring stalls. | Retry or contact support with the run ID.             |
...
| Code                                         | Surface                            | Cause                                                                       | Consumer action                                                            | User action                                             |
| -------------------------------------------- | ---------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| `flow_definition_checksum_mismatch`          | Run error payload                  | The published definition checksum changed before execution.                 | Refetch the flow contract and start a run against the current publication. | Reload the flow and start a new run.                    |
...
| `flow_definition_schema_version_missing`     | Run error payload                  | The published definition is missing its schema version.                     | Require republish through the current editor before retrying.              | Ask a flow editor to republish the flow.                |
| `flow_definition_schema_version_unsupported` | Run error payload                  | The published definition uses a schema version this runtime cannot execute. | Republish with the current editor or upgrade the runtime before retrying.  | Ask a flow editor to republish with the current editor. |
| `flow_definition_flow_id_invalid`            | Run error payload                  | The published snapshot references an invalid flow id.                       | Treat the publication as corrupt and require republish before running.     | Ask a flow editor to republish the flow.                |
| `flow_definition_steps_invalid`              | Run error payload                  | The published snapshot has invalid step definitions.                        | Block execution and show the editor that the flow must be republished.     | Ask a flow editor to fix the steps and republish.       |
...
| `flow_input_contract_inapplicable`           | Run error payload                  | A step input contract conflicts with its authored question binding.         | Remove the conflicting contract or question binding, then republish.       | Ask a flow editor to fix the step input settings.       |
| `flow_published_form_schema_invalid`         | API response and run error payload | The published flow form configuration is invalid.                           | Ask an editor to fix form fields and republish before running.             | Ask a flow editor to fix the form fields.               |
...
<details>
<summary>42 error codes</summary>
...
| Code                                                     | Surface                            | Cause                                                                        | Consumer action                                                               | User action                                                |
| -------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `flow_review_policy_invalid`                             | Run error payload                  | A reviewed step has an invalid review policy.                                | Fix the review policy on an executable step and republish.                    | Ask a flow editor to fix review settings.                  |
...
| `flow_template_not_accessible`         | API response and run error payload | The template file is not accessible to the caller or runtime actor. | Use a template stored in the flow or space with the right permissions. | Choose a template you can access.                   |
...
| `flow_template_missing_content`        | API response and run error payload | The selected template record has no readable binary content.        | Ask for a new upload or restore the template file content.             | Choose or upload the template again.                |
...
| Source                                                                               | Guard                                                           |
| ------------------------------------------------------------------------------------ | --------------------------------------------------------------- |
| `backend/src/intric/flows/flow_api_error_code.py`                                    | Every `FlowApiErrorCode` value must have one taxonomy entry.    |
| `backend/src/intric/flows/flow_error_taxonomy.py`                                    | Every taxonomy entry must stay short, categorized, and typed.   |
