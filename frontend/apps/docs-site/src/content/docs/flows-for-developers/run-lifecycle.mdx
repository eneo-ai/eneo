...
unbounded wait to resume. After 30 days, resume returns `flow_run_abandoned`
without writing terminal state; the maintenance sweep owns that write.
...
refuse overdue waits even before the sweep reaches them.
