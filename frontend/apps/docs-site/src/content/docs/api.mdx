...
- `POST /api/v1/conversations/{session_id}/feedback/` records feedback on the whole conversation.
- `PUT /api/v1/conversations/{session_id}/messages/{message_id}/feedback/` rates one answer with `{"value": 1}` (good) or `{"value": -1}` (bad) and an optional `"text"` comment, replacing an earlier rating of that answer. `DELETE` on the same path clears it (`204`). Each message in a conversation carries its rating as `feedback`.
