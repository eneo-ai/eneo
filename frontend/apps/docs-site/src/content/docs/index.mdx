# Documentation

Welcome to the Eneo technical documentation! This section covers the core concepts, architecture, and API reference for the Eneo platform.

## Getting Started

New to Eneo? Start here:

### [Getting Started](/docs/getting-started)

Quick start guide to set up your Eneo instance:

- Installation and setup
- First-time configuration
- Creating your first AI assistant
- Basic usage examples

## Technical Documentation

### [Architecture](/docs/architecture)

Understand how Eneo is built:

- System architecture overview
- Technology stack (FastAPI, Next.js, PostgreSQL)
- Database schema and migrations
- Authentication and authorization
- Real-time communication (SSE, WebSockets)

### [Flows](/docs/flows)

Understand how Eneo Flows work end-to-end:

- Draft vs published lifecycle
- Sequential steps and step configuration
- Transcription, template fill, knowledge, and RAG
- Ownership, permissions, and evidence
- Security classification and supported external effects

### [AI Builder](/docs/ai-builder)

Draft and revise Flows through a guided authoring conversation:

- Form fields, runtime input, JSON contracts, and final artifacts
- Server-owned step wiring and Underlag composition
- Explicit external-data and delivery boundaries
- Token usage transparency for builder activity

### [Audit Logging](/docs/audit-logging)

Audit trails for compliance and security:

- How audit logs are created and stored
- Retention policies and conversation retention hierarchy
- Viewing, exporting, and configuring audit logs
- Adding audit logging to new features

### [Token Counting](/docs/token-counting)

How Eneo tracks token usage for LLM requests:

- Accurate counts from major providers (OpenAI, Anthropic, Azure)
- Estimation for self-hosted models
- How token data flows to Insights dashboards

### [Release SBOMs](/docs/release-sboms)

Downloadable SBOMs and attestations for released artifacts:

- CycloneDX and SPDX files for security tooling
- Frontend source dependency inventory from the release lockfile
- Human-readable package lists for quick review
- Image digests, checksums, and provenance verification

### [API Reference](/docs/api)

Complete API documentation for developers:

- REST API endpoints
- Authentication and API keys
- Request/response formats
- WebSocket events
- Error handling

## Core Concepts

- **Assistants**: AI agents with customizable system prompts and provider configurations
- **Productions**: Collaborative workspaces within organizations
- **Knowledge**: Document collections for RAG (Retrieval-Augmented Generation)
- **Organizations**: Multi-tenant structure with role-based access control

## Development

For development setup and contributing guidelines, visit:

- [GitHub Repository](https://github.com/eneo-ai/eneo)
- [Contributing Guide](https://github.com/eneo-ai/eneo/blob/develop/docs/CONTRIBUTING.md)
- [Security for Contributors](/contributing/security)
- [Installation Guide](https://github.com/eneo-ai/eneo/blob/develop/docs/INSTALLATION.md)

## Need Help?

- **Guides**: Check out our [practical guides](/guides) for specific use cases
- **Forum**: Join discussions at [forum.eneo.ai](https://forum.eneo.ai)
- **Issues**: Report bugs on [GitHub](https://github.com/eneo-ai/eneo/issues)
- **Contact**: digitalisering@sundsvall.se (for public sector organizations)
