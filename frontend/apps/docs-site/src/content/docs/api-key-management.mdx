...
Tenant admins see a space's content only after joining it with a recorded reason ([Space Oversight](/guides/space-oversight#content-requires-membership)). That guarantee applies to people: a tenant-scoped service key already reaches every space's content without joining, so create tenant-scoped service keys only for vetted integrations.
