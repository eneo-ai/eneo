...
3. For tenant-scoped admin keys: verifies the owner still has **admin** permissions
...
Only `ACTIVE` keys proceed. Revoked, expired, rotation-grace-expired, and suspended keys return `401` during API-key authentication.
...
- **Origin validation** (`pk_` keys): The request must include an `Origin` header. For new keys, the origin must match one of the key's configured `allowed_origins`.
...
Migrated keys have the full v2 lifecycle, audit, scope, and policy machinery, and appear in the API key management UI as "Legacy API key". v1 keys that were never used after the v2 system launched were never migrated and no longer authenticate — mint a new key via `/api/v1/api-keys` to replace them.
