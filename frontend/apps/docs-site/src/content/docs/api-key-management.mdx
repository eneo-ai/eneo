...
- **Deterministic ID**: Generated from `uuid5(NAMESPACE_URL, "service-key:{tenant_id}")` so all service keys for a tenant share the same synthetic identity
...
Only tenant administrators can create service keys. Service keys with tenant-wide write or admin scope require either an **IP allowlist** or an **expiration date** as a guardrail against misuse.
</Callout>
...
Two feature flags control enforcement behavior:

| Flag | Default | Purpose |
|------|---------|---------|
| `api_key_scope_enforcement` | Enabled | Enforce scope boundaries |
| `api_key_strict_mode` | Disabled | Deny ambiguous list requests for scoped keys |

Both flags can be toggled globally and per-tenant, allowing staged rollout.
