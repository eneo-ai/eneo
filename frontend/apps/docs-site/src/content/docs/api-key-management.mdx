...
| **Resource permissions** | Supported                              | Supported for `none`/`read` only (`jobs`, `prompts`, and `flow_evidence` are not available) |
| **Origin restrictions**  | Not allowed                            | Required for creation (at least one)                                                        |
| **IP restrictions**      | Optional                               | Not allowed                                                                                 |

<Callout type="info">
  The key prefix (`sk_` or `pk_`) is embedded in the key string itself and
  validated on every request. A `pk_` key that requests write or admin
  permission is rejected by policy validation.
</Callout>
...
3. For tenant-scoped keys, whatever their permission level: verifies the owner still has the tenant **admin** permission (`403 owner_permission_revoked` otherwise)
4. For space/assistant/app-scoped keys: verifies the owner is still a **member** of the target space (`403 owner_membership_revoked` otherwise)
...
<Callout type="warning">
  Only tenant administrators can create service keys. Service keys with write or
  admin permission require either an **IP allowlist** or an **expiration date**
  as a guardrail against misuse, regardless of scope.
</Callout>
...
The ordinary file API is the important exception to "delete existing resources": files are user-owned today. Service keys cannot use the general file upload or delete endpoints. General file upload returns `403` with `code: "service_key_cannot_create_resources"`, and general file delete returns `403` with `code: "service_key_cannot_delete_files"`.

Published-Flow runtime routes are a separate, Flow-scoped exception, and they are what an external integration actually needs. A service key with `resource_permissions.flows = write`—or an equivalent top-level write grant on a key without resource permissions—can:

- upload runtime inputs through `POST /api/v1/flows/{flow_id}/steps/{step_id}/runtime-files/`;
- **create runs of a published Flow** through `POST /api/v1/flows/{flow_id}/runs/`;
- cancel its own runs, and edit, approve, reject, or resume human-review checkpoints for runs the same key created;
- delete only its own unattached upload through `DELETE /api/v1/flows/{flow_id}/runtime-files/{file_id}/`. Once a runtime file is attached to persisted run or rerun state, cleanup returns `409` with `code: "flow_runtime_file_attached"` instead of deleting it.

`flows = read` covers the read half of that journey: published flows, runtime paths, run contracts, own run status, and active review checkpoints. Authoring—draft create, update, delete, publish, template management, and AI Builder—stays with user principals.
...
The key hash is looked up against the `api_keys_v2` table using the `idx_api_keys_v2_key_hash` index. Existing v2 keys found via the legacy SHA256 hash are upgraded to HMAC-SHA256 on use. Keys that were migrated from the retired v1 system keep their original `inp_`/`ina_` prefixes and resolve like any other v2 key. A key that does not resolve returns `401` with `code: "invalid_api_key"`.

### 2. Ownership Resolution

Based on the `ownership` field:

- **User keys**: The owner is fetched from the database. The system verifies the owner exists and belongs to the key's tenant (`401 invalid_api_key` otherwise), is active (`403 owner_inactive`), and still has the required permissions for the key's scope (`403 owner_permission_revoked` or `403 owner_membership_revoked`).
- **Service keys**: A synthetic user is constructed. No database user lookup occurs.

Ownership is resolved before the key's own state is checked, so an expired key whose owner has been deactivated returns `403 owner_inactive` rather than `401`.
...
Only `ACTIVE` keys proceed. Revoked, expired, rotation-grace-expired, and suspended keys return `401` with `code: "invalid_api_key"` during API-key authentication.

### 4. Guardrail Checks

- **IP validation** (`sk_` keys): If `allowed_ips` is configured, the client IP must match at least one CIDR block
- **Origin validation** (`pk_` keys): The request must include an `Origin` header, and the origin must match one of the key's configured `allowed_origins`. A `pk_` key with no origins configured is rejected. Failures return `403` with `code: "origin_not_allowed"`.
...
The window length is set by `API_KEY_RATE_LIMIT_WINDOW_SECONDS` (default 3600) and the per-scope defaults by the matching `API_KEY_RATE_LIMIT_*_DEFAULT` settings. Per-key overrides are possible via the `rate_limit` field: a positive integer replaces the default, and `-1` removes the limit for that key. Only tenant administrators may set `-1`, and it is rejected whenever the tenant policy `max_rate_limit_override` is set, because that policy acts as an absolute ceiling. When the limit is hit the request fails with `429` and `code: "rate_limit_exceeded"`; `Retry-After`, `X-RateLimit-Limit`, and `X-RateLimit-Remaining` headers are sent only on that response. Session-token requests are not rate limited by this mechanism. See the [API Reference](/docs/api#rate-limiting) for the full contract.
...
**Resource-level** (feature-gated): If the key has `resource_permissions` configured, per-resource-type permissions are checked for `flows`, `flow_evidence`, `assistants`, `apps`, `spaces`, `knowledge`, `conversations`, `files`, `jobs`, and `prompts`. For `sk_` keys, the top-level `permission` is derived from the highest configured resource permission when the key is created or updated. For `pk_` keys, every resource permission is capped to `none` or `read`, and `jobs`, `prompts`, and `flow_evidence` are rejected.

<Callout type="warning">
  Once `resource_permissions` is set it replaces the top-level permission for
  every listed resource type, and a type left unset counts as `none`. A key
  configured without `flows` is refused on every Flow route with `403` and
  `code: "insufficient_resource_permission"`, even when its top-level
  `permission` is `admin`. Set `flows` explicitly on any key that drives Flow
  runs.
</Callout>

### 7. Scope Enforcement

On scope-enforced routes, the system verifies that the key's scope covers the requested resource. See [Scope & Permissions](#scope--permissions) for details.
...
For the ordinary file API, `write` allows a user key to upload files owned by that user. `admin` is required to delete files, and deletion is still limited to files owned by the key's user. Service keys cannot use those ordinary upload/delete routes. The published-Flow runtime routes described above are the narrow exception, covering service-owned pre-run uploads, run creation, orphan cleanup, and own-run review decisions.
...
Keys are created via `POST /api/v1/api-keys` (request fields: `name`, `description`, `key_type`, `permission`, `scope_type`, `scope_id`, `ownership`, `allowed_origins`, `allowed_ips`, `expires_at`, `rate_limit`, `resource_permissions`), or from the web app under **Account → API keys** and, for tenant administrators, **Admin → API keys**. Creation requires a session login and the `api_keys` role permission. `GET /api/v1/api-keys/policy-constraints` returns the tenant limits a client should respect before submitting (expiration rules, rate-limit ceiling, rotation grace). The policy service validates:
...
- Resource permission shape and scope compatibility
- `sk_` effective permission derivation from the highest resource permission
- `pk_` resource permission caps (`none`/`read` only)
...
2. The old key gets a `rotation_grace_until` timestamp (default: 24 hours, configurable via tenant policy)
3. Both keys work during the grace period
4. After the grace period, the old key stops working

This enables zero-downtime key rotation for integrations. `POST /api/v1/api-keys/{id}/rotate` accepts `disable_grace_period` (revoke the old key immediately) and `update_expiration` together with `expires_at` (give the new key a different expiry).
...
- `POST /api/v1/api-keys/{id}/extend` changes the expiration of an active key; passing `expires_at: null` removes it where the tenant policy allows
- `GET /api/v1/api-keys/expiring-soon?days=30` summarises keys that expire within the look-ahead window (1 to 365 days)

### Purge

Revoked and expired keys stay listed until they are purged. `POST /api/v1/api-keys/{id}/purge` permanently deletes such a key while preserving its audit history; active or suspended keys cannot be purged, revoke them first.

### Usage

`GET /api/v1/api-keys/{id}/usage` returns the usage and authentication-failure audit events for a key you manage, cursor-paginated with `limit` (1 to 200) and `cursor`.
...
```
revoked_at > expires_at > rotation_grace_until > suspended_at > active
```

A key that is both suspended and expired is treated as expired. A key that is both expired and revoked is treated as revoked. A rotated key whose `rotation_grace_until` has passed is treated as revoked.
...
| `auto_expire_unused_days`    | null    | Mark keys unused for N days as `EXPIRED` (audited as `API_KEY_EXPIRED`) |
...
Use a `pk_` key only for browser-based integrations. `pk_` keys are read-only, must list allowed origins, cannot use IP restrictions, and can only use `none` or `read` resource permissions. The `jobs`, `prompts`, and `flow_evidence` resource permissions are rejected for `pk_` keys. Because `pk_` keys never reach `write`, they cannot upload Flow runtime files or create Flow runs; those calls belong to the application's own server.
...
- `read` allows read-style requests such as `GET`
- `write` allows create/update-style requests such as `POST`, `PUT`, and `PATCH`
- `admin` allows delete-style requests such as `DELETE`

Routes can override the method mapping when a request is semantically read-only. If `resource_permissions` are set, the key must also have enough permission for that resource type, and a resource type left unset counts as `none`. See [Permission Enforcement](#6-permission-enforcement) above for the resource types, the `pk_` restrictions, and why a key that drives Flows must set `flows` explicitly.

### How do scopes affect access?

Scope limits where the key can operate:

- `tenant` keys can access tenant-wide resources, subject to permission checks
- `space` keys are limited to one space
- `assistant` keys are limited to one assistant and its conversations
- `app` keys are limited to one app and its runs

Tenant-scoped user keys require the owner to remain a tenant administrator, whatever the key's own permission level. Endpoint-level guards additionally apply the owner's live role permissions. Space-, assistant-, and app-scoped user keys require the owner to remain a member of the target space.
...
Create assistants, apps, spaces, group chats, prompts, collections, websites, ordinary files, info blobs, roles, and MCP servers with a user account first. Then use the service key for supported automation around those resources. The general file API remains user-owned; the narrow exception is the published-Flow runtime routes documented above, which let the same service principal upload runtime inputs, create runs, clean up its own unattached uploads, and resolve its own runs' review checkpoints.

### What happens to our old keys?

The legacy v1 key system is retired. Keys that were used while the compatibility path existed were migrated into `api_keys_v2` and keep working unchanged, with their original `inp_`/`ina_` prefixes:

- Migrated user keys are tenant-scoped `sk_`-type keys tied to the original owner (`admin` if the owner had tenant admin permission at migration, otherwise `write`).
- Migrated assistant keys are assistant-scoped `sk_`-type keys with read permission.

Migrated keys have the full v2 lifecycle, audit, scope, and policy machinery, and appear in the API key management UI like any other key. v1 keys that were never used after the v2 system launched were never migrated and no longer authenticate — mint a new key via `/api/v1/api-keys` to replace them.

We still recommend replacing migrated keys with purpose-made v2 keys: create a new key with the narrowest practical scope and permission, deploy it, confirm traffic and audit logs show the new key being used, then revoke the migrated key. For production automations owned by a team rather than a person, prefer a service `sk_` key with an IP allowlist or expiration date.
...
- New `pk_` keys **cannot** have admin permission and **must** specify at least one allowed origin
- Each origin must include an `http://` or `https://` scheme and a host (`http://localhost:3000` is valid)
...
- **Tenant admins do not gate per-key origin lists**. Tenant-level policy controls _whether_ a user may mint `pk_` keys (via roles and quotas) and may enforce rate limits, but does not vet the origins on each key.
