...
Flow runtime input routes are a separate, Flow-scoped exception. A service key with `resource_permissions.flows = write`—or an equivalent top-level write grant on a key without resource permissions—can upload runtime inputs to a published Flow through `POST /api/v1/flows/{flow_id}/steps/{step_id}/runtime-files/`. It can delete only its own unattached upload through `DELETE /api/v1/flows/{flow_id}/runtime-files/{file_id}/`. Once a runtime file is attached to persisted run or rerun state, cleanup returns `409` with `code: "flow_runtime_file_attached"` instead of deleting it.
