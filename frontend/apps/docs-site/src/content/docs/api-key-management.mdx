...
- Legacy user keys become tenant-scoped `sk_` keys tied to the original owner. They become `admin` only if the owner currently has tenant admin permission; otherwise they become `write`.
- Legacy assistant keys become assistant-scoped `sk_` keys with read permission.
- Existing SHA256-backed v2 keys upgrade to HMAC-SHA256 when they resolve through the v2 lookup path.
