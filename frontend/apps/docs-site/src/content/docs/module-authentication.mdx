...
session JWT is rejected by module-facing routes. Most routes under
`/api/v1/flows/` are the exception: they accept a module user token together
with its module's `sk_` key (see [Call Flows as the signed-in user](#call-flows-as-the-signed-in-user)).
A token minted for one module cannot be replayed against another module's
routes.
...
## Call Flows as the signed-in user

A module can run flows for the user who signed in to it. Send the module's
`sk_` key and the user's module token on the same request to a route under
`/api/v1/flows/`:
...
Eneo checks the pair as for any module resource call. The key passes normal
API-key authentication, including its scope and its `flows` resource
permission, and it must be the key registered for the token's module. The token
and the user are then validated against live state. The request runs as the
user:

- **Access needs both credentials.** The key's scope and `flows` permission
  apply, and so does the user's own access to the space and the flow. A
  tenant-wide key does not let a user run a flow that the user cannot run.
- **The user is the actor.** Runs the module creates belong to the user, and
  the audit log records the user as the actor.

This includes requesting a live transcription ticket. The socket itself checks
only the ticket and the page's origin, so the module's backend can pass the
ticket to its browser when Eneo accepts the module's origin, or relay the
socket itself; see
[Show live text while recording](/guides/flows/integrating-flows#show-live-text-while-recording).

Other resource routes refuse the pair: the Flow AI Builder routes
(`/api/v1/flows/ai-builder/`), Flow package export
(`/api/v1/flows/{id}/package-exports/`), and every resource route outside
`/api/v1/flows/`. The broker's own routes under `/api/v1/module-auth/` follow
the handoff above, and two of them take this same pair: token refresh
(`POST /api/v1/module-auth/{module_key}/token/refresh/`) and session validation
(`GET /api/v1/module-auth/{module_key}/session/`).

| Request | Result |
| --- | --- |
| Module token without the `sk_` key | `401` |
| The pair on a route that does not accept it | `401` |
| `sk_` key without the `flows` resource permission | `403` |
| Token of one module with another module's key | `403`, message `API key is not registered for this module.` |

To verify the setup, call `GET /api/v1/flows/runs/capacity/` with the pair. The
response is `200`, and its `tenant_id` is the user's tenant.
...
2. **Install it in Admin → Modules** with the stable module key, the
