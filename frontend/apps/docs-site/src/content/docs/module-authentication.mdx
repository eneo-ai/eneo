...
session JWT is rejected by module-facing routes. Routes under `/api/v1/flows/`
are the exception: they accept a module user token together with its module's
`sk_` key (see [Call Flows as the signed-in user](#call-flows-as-the-signed-in-user)).
...
This includes starting a live transcription session. Relay its WebSocket
through the module's backend, as described in
[Show live text while recording](/guides/flows/integrating-flows#show-live-text-while-recording).

The Flow AI Builder routes (`/api/v1/flows/ai-builder/`), Flow package export
(`/api/v1/flows/{id}/package-exports/`) and every route outside
`/api/v1/flows/` refuse the pair.
