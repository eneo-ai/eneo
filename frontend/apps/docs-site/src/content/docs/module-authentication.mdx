...
| Module user token (`aud` = `eneo-module:<module_key>`) | Module server session, never the browser | **60 min**, no refresh (`MODULE_AUTH_TOKEN_EXPIRY_MINUTES`) | Proves *which user* the module acts for |
...
request — there is no cached module authorization to wait out. The one
exception is the stateless module user token itself: an Eneo browser logout
does not revoke an already minted token; its residual lifetime is bounded by
the short token TTL.
...
## Lifetimes and long-running work

Two settings control the broker's lifetimes:

| Setting | Default | What it bounds |
| --- | --- | --- |
| `MODULE_AUTH_TICKET_TTL_SECONDS` | 30 | Redirect gap between Eneo issuing the ticket and the module exchanging it |
| `MODULE_AUTH_TOKEN_EXPIRY_MINUTES` | 60 | The module user token, and therefore the maximum module session length |
...
The token TTL follows one design rule:

<Callout type="warning">
  **The token TTL must comfortably exceed the module's longest single
  request.** There is deliberately no refresh endpoint: a fresh token requires
  a fresh handoff. A module that performs long uploads (for example
  speech-to-text audio) must fit each upload inside the remaining token
  lifetime.
</Callout>
...
- **Expiry is checked when a request starts.** A single long request that
  begins while the token is valid is not aborted mid-flight when the token
  expires. What fails is the *next* request.
- **Renewal is a new handoff, and it is cheap.** While the user's Eneo
  session is alive, redirecting through `/module-login` again issues a fresh
  ticket without any IdP interaction — the user typically only sees a quick
  redirect. Modules should treat this as the normal renewal path, not as an
  error case.
- **Modules should preflight long operations.** The reference module stores
  `expires_at` next to its session. Before starting an operation that cannot
  be cheaply retried (a long recording, a large upload), compare the remaining
  lifetime with the expected duration; if it will not fit, persist the pending
  work (for example the recorded audio) and re-run the handoff *first*. A
  top-level redirect discards page state, so stash before you redirect.
...
The reference module implementation additionally caps its own session cookie
at `min(expires_in, 60 min)`, so the module session can never outlive the
token it wraps.

### Tuning

Both settings are deployment-wide backend environment variables. Raising
`MODULE_AUTH_TOKEN_EXPIRY_MINUTES` lengthens the window in which a stateless
token stays valid after logout for **all** modules, so raise it only as far
as the slowest legitimate module operation requires. If modules with very
different needs appear (a 5-minute lookup tool next to a 3-hour batch tool),
a per-module TTL in the client config is the intended evolution path rather
than a deployment-wide increase.
...
| Resource call fails after login worked | Token expired, user/tenant deactivated, module unassigned, or key revoked since login | Re-handoff (token expiry) or operator action (state changes are immediate by design) |
...
- `module_key` (the module's unique `name`) is the public machine identity
  ...
  module identity.
- The **client config** lives on the tenant-module assignment. Only the bound
  ...
  binding.
- **Disabling a module for a tenant deletes its client config.** Re-enabling
...
1. **Register the module key** (once, global): `POST /api/v1/modules/` with the
   stable `module_key` as `name`.
2. **Enable it for the tenant**: `PUT /api/v1/modules/{tenant_id}/{module_id}/`
   (idempotent, preserves other modules).
3. **Mint a dedicated `sk_` service key** as a tenant admin: service-owned,
...
4. **Bind callback and key**:
   `PATCH /api/v1/modules/{tenant_id}/{module_id}/client-config/` with the
   exact `redirect_uris` and the key's `service_key_id`.
5. **Deploy the module** with `MODULE_KEY`, the Eneo URL, the service key and
...
client-config changes and tenant assignment changes all produce audit events
...
- **No token refresh.** Renewal is always a new handoff. This is deliberate:
  it keeps the token contract small and forces renewal through the central
  session gate, but it also puts a hard ceiling on unattended module work.
- **Logout latency.** A stateless module token survives Eneo logout until it
  expires. Everything the token can reach still re-validates live user and
  tenant state per request, so deactivating the user closes access
  immediately even while the token is unexpired.
- **User identity is email-based.** Like Eneo session JWTs, the module token
  identifies the user by email (`sub`); the broker then requires that user to
  exist and be active in the service key's tenant.
