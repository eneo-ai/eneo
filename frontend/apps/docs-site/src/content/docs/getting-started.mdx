...
# Terminal 4: Flow worker
cd backend && uv run flow-worker

# Terminal 5: Flow scheduler
cd backend && uv run flow-beat
```
...
Flow runs require the Flow worker and scheduler:
```bash
cd backend && uv run flow-worker
cd backend && uv run flow-beat
```
