...
For the bundled object store, copy the exact manifest and platform digests from
`IMAGE-DIGESTS.txt`. Verify provenance and the platform SPDX SBOM through the
OCI registry, constraining the signer workflow:
