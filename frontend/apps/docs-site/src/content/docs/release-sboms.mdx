...
- `ghcr.io/eneo-ai/eneo-seaweedfs:<Eneo source SHA>`
...
3. It waits for the matching backend, frontend, and Eneo SeaweedFS images in GitHub Container
   Registry.
...
8. It verifies the SeaweedFS image provenance and both platform SBOM
   attestations against the expected repository, workflow, tag, and source SHA.
...
commit. These attestations establish Eneo's build identity and inputs; they do
not claim that the image or source commit was signed by the SeaweedFS supplier.
...
gh attestation verify \
  ...
  --repo eneo-ai/eneo \
  --signer-workflow github.com/eneo-ai/eneo/.github/workflows/build_and_push_images.yml \
  --bundle-from-oci
