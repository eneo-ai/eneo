...
## Configure material in the assistant editor

The **Assistentens material** (Assistant material) section brings persistent
file attachments and existing knowledge selectors together. **Bilagor**
(Attachments) contains all attached files and the file uploader. Each file
shows whether it is loaded before every answer or opened on demand; changing
its reading mode does not move it out of Attachments.

**Kunskap** (Knowledge) and **Organisationskunskap** (Organization knowledge)
are separate sections below the attachments and uploader. The knowledge summary
states whether searches run on demand or before every answer. Personal and
organization knowledge retain their existing selectors and origins.

Use **Ändra** (Change) on a file to choose between **Öppna vid behov** (Open
when needed) and **Läs in inför varje svar** (Load before every answer). The
editor explains when the selected model, file type, stored original or deployment
prevents on-demand access. Changing the model does not overwrite
the saved file preference.

**Hantering av material** (Material handling) shows the knowledge search timing
and chat-document handling controls directly, without a collapsed section. These
remain independent: the knowledge setting controls searches for relevant passages, the
chat setting controls user-uploaded documents, and each persistent attachment
has its own setting. Rules, tone and procedures are configured in **Prompt** or
**Skills**, both under **Instruktioner** (Instructions).

This organization does not change knowledge uploads, extraction, indexing or
the assistant save flow. Newly uploaded assistant attachments default to opening
on demand. When on-demand access is unavailable, their content is loaded upfront
instead. Existing attachments retain their saved reading mode. Chat document
text is still included directly, and knowledge still uses injected retrieval
unless configured otherwise. On-demand file attachments are not automatically
indexed as knowledge.

The **Fast kontext** (Fixed context) meter estimates prompt text and directly
loaded attachments. It is not a total request estimate: chat history, skills,
tool definitions, references and retrieved content also consume context.
Including content does not guarantee the assistant will follow it.
...
protocol session ID, and any healthy backend worker can serve a loopback call.
...
| MCP protocol state | Stateless | Stateless (fresh connection per completion) |
...
## Conversation replay of tool use

Eneo opens a fresh MCP connection per completion and does not persist MCP
protocol sessions. Cross-turn continuity lives in the conversation's tool
trace instead: tool metadata records the exact prefixed name, arguments,
result status, and result text needed to replay prior tool use to the model.
MCP resource blocks are captured generically, regardless of whether the
provider was internal or external. A server that holds state across turns
returns an explicit handle in its tool results; because prior results are
replayed, the model can pass that handle to later calls.

| State | Internal loopback | External stateful server |
| --- | --- | --- |
| Conversation tool trace | Persisted | Persisted |
| Cross-turn server state | None | Keyed to handles the server returns in tool results |
| Data authority | Current Eneo services and authorization | Remote server's own authorization |
...
| `FILE_REFERENCE_BASE_URL` | Base URL placed in signed file references for URL-consuming MCP tools. It must resolve from those tools' network: an internal address when the servers share the backend's private network, the public origin when they run outside it. The download route is authenticated by the signed token alone. Unset, it falls back to `PUBLIC_ORIGIN`. See the deployment guide's [Signed file references](/guides/deployment#signed-file-references) section. |
...
Both retrieval paths run the same pgvector query: chunks in scope are ordered by
cosine distance to the query embedding. The embedding column has no
approximate-nearest-neighbour index (it is declared without a fixed dimension,
so one cannot be created), and the repository disables sequential scans for the
query to keep PostgreSQL on the scope indexes. Retrieval latency therefore
grows with the size of the searched corpus rather than staying flat.
