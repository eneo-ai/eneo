...
vi.mock("./use-assistant", () => ({ useUpdateAssistant: () => ({ mutateAsync: update, isPending: false }) }));
vi.mock("@/features/spaces/use-space", () => ({ useSpace: () => ({ space: { enabled_capabilities: ["web_search", "image_generation"] } }) }));
vi.mock("@/components/composites/use-autosave", () => ({ useAutosave: () => (operation: () => Promise<unknown>) => operation() }));
...
const assistant = (enabled: ("web_search" | "image_generation")[], available: boolean) => ({
  ...
  available_capabilities: [{ purpose: "web_search", available }, { purpose: "image_generation", available }],
...
describe("assistant capabilities", () => {
  it("saves the complete selection when enabling a function", async () => {
    ...
    await waitFor(() => expect(update).toHaveBeenCalledWith({ enabled_capabilities: ["image_generation", "web_search"] }));
  });
