...
import { capabilityMarker } from "./policy-draft";
...
export function McpRestrictionSection({ draft }: { draft: PolicyDraft }) {
  ...
  const generalServers = allMcpServers.filter(
    (server) => !server.purpose || server.purpose === "general"
  );
      ...
      {!mcpEnabled ? (
        ...
        <>
          ...
          <fieldset className="divide-border divide-y overflow-hidden rounded-lg border">
            <legend className="sr-only">{t("capabilities")}</legend>
            {CAPABILITIES.map((capability) => {
              const id = capabilityMarker(capability.purpose);
              const selected = Boolean(mcpSelections[id]);
              const available = allMcpServers.find((server) => server.id === id)?.is_available;
              return (
                <div key={id} className={selected ? "bg-muted/40" : ""}>
                  <div className="flex items-center gap-3 py-2.5 pr-4 pl-1">
                    <span
                      className="flex size-8 shrink-0 items-center justify-center"
                      aria-hidden="true"
                    >
                      <capability.icon className="text-muted-foreground size-4" />
                    </span>
                    <Switch
                      checked={selected}
                      disabled={!selected && !available}
                      onCheckedChange={(on) => toggleCapability(capability.purpose, on)}
                      aria-label={t("governance_mcp_allow_aria", { name: t(capability.purpose) })}
                    />
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground text-xs">
                        {available
                          ...
                          : t("tools_readiness_no_active_provider")}
                      </p>
                    </div>
                    {selected && (
                      <label className="flex shrink-0 items-center gap-2">
                        ...
                        <Switch
                          checked={mcpSelections[id]?.isDefaultEnabled ?? true}
                          onCheckedChange={(on) => toggleCapabilityDefault(capability.purpose, on)}
                          aria-label={t("governance_mcp_default_aria", {
                            name: t(capability.purpose)
                          })}
                        />
                      </label>
                    )}
            ...
            <fieldset className="divide-border divide-y overflow-hidden rounded-lg border">
              ...
              {generalServers.map((server) => {
                ...
                const enabledCount = tools.filter(
                  (tool) => !disabledMcpToolIds.has(tool.id)
                ).length;
                return (
                  <div key={server.id} className={isSelected ? "bg-muted/40" : ""}>
                    <div className="flex items-center gap-3 py-2.5 pr-4 pl-1">
                      <button
                        ...
                        aria-label={
                          isExpanded
                            ? t("governance_mcp_hide_tools")
                            : t("governance_mcp_show_tools")
                        }
