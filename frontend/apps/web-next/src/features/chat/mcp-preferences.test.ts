...
describe("MCP server preferences", () => {
  ...
  it("uses the existing preference key for both MCP servers and capability markers", () => {
    const storage = new MemoryStorage();
    saveMcpServerPreferences(
      context,
      ["server-a", "capability:image_generation"],
      new Set(["capability:image_generation"]),
      storage
    );
    expect(storage.values.has("eneo:chat-mcp:v1:tenant-1:user-1:assistant-1")).toBe(true);
    expect(loadMcpServerPreferences(context, storage)).toEqual({
      serverStates: { "server-a": true, "capability:image_generation": false }
    });
  });
