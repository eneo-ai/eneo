...
<script lang="ts">
  ...
  let hasNoActivity = $derived(data.sessions.length === 0 && data.questions.length === 0);
...
<div class="flex w-[calc(100%_-_256px)] flex-col">
  ...
  <div class="border-default flex items-center justify-between gap-4 border-b px-5 py-3.5">
    ...
    <div class="flex items-center gap-1">
      ...
      >
        ...
        <span
          class="absolute right-0 bottom-0 left-0 h-0.5 origin-left bg-[var(--accent-default)] transition-transform duration-200
                     {filter === 'sessions' ? 'scale-x-100' : 'scale-x-0'}"
        ></span>
      </button>
      <button
        class="group relative flex items-center gap-2 px-3 py-2 text-sm font-medium whitespace-nowrap transition-colors duration-200
      ...
      >
        ...
        <span
          class="absolute right-0 bottom-0 left-0 h-0.5 origin-left bg-[var(--accent-default)] transition-transform duration-200
                     {filter === 'questions' ? 'scale-x-100' : 'scale-x-0'}"
        ></span>
      </button>
    </div>
    ...
    <div
      class="flex items-center rounded-lg border border-[var(--border-dimmer)] bg-[var(--background-secondary)] p-1"
    >
      <button
  ...
  <div class="relative h-full w-full px-8 pt-6 pb-5">
    ...
    {:else}
      ...
      {#if partialDataInfo}
        <div
          class="absolute bottom-6 left-10 flex items-center gap-1.5 text-[10px] text-[var(--text-muted)]"
        >
