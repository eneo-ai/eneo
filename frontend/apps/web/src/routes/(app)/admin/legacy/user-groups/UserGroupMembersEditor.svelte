...
<Dialog.Root bind:open={showDialog}>
  <Dialog.Trigger>
    {#snippet child({ props })}
      <Button {...props} variant="outline">{m.manage_members()}</Button>
    {/snippet}
  </Dialog.Trigger>
      ...
      <div class={dialogLayout.section}>
        <div class="border-default border-b px-4 py-4">
          ...
          <div class="mt-3 flex justify-end gap-2">
            ...
            {/if}
            <Button disabled={selectedUsers.length === 0 || addUsers.isLoading} onclick={addUsers}>
