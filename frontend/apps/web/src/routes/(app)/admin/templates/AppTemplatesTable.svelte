<!--
    Copyright (c) 2024 Sundsvalls Kommun

    Licensed under the MIT License.
-->

<script lang="ts">
  import type { components } from "@intric/intric-js";
  import { Table } from "@intric/ui";
  import { m } from "$lib/paraglide/messages";
  import TemplateActions from "./TemplateActions.svelte";
  import TemplateNameCell from "./TemplateNameCell.svelte";
  import TemplateCategoryBadge from "./TemplateCategoryBadge.svelte";
  import dayjs from "dayjs";

  type AppTemplate = components["schemas"]["AppTemplateAdminPublic"];

  let { templates }: { templates: AppTemplate[] } = $props();

  const table = Table.createWithResource(templates);

  const viewModel = table.createViewModel([
    table.column({
      accessor: "name",
      header: m.template_name(),
      cell: (item) => {
        const row = item.row as import("svelte-headless-table").DataBodyRow<AppTemplate>;
        return Table.renderComponent(TemplateNameCell, {
          name: item.value,
          description: row.original.description,
          isDefault: row.original.is_default,
          iconName: row.original.icon_name
        });
      },
      plugins: {
        sort: {
          getSortValue(value) {
            return value;
          }
        },
        tableFilter: {
          getFilterValue(value) {
            return value;
          }
        }
      }
    }),

    table.column({
      accessor: "category",
      header: m.category(),
      cell: (item) => {
        return Table.renderComponent(TemplateCategoryBadge, {
          category: item.value,
          type: "app"
        });
      },
      plugins: {
        sort: {
          getSortValue(value) {
            return value;
          }
        },
        tableFilter: {
          getFilterValue(value) {
            return value;
          }
        }
      }
    }),

    table.column({
      accessor: "input_type",
      header: m.input_type(),
      plugins: {
        sort: {
          getSortValue(value) {
            return value;
          }
        },
        tableFilter: {
          getFilterValue(value) {
            return value;
          }
        }
      }
    }),

    table.column({
      accessor: (row) => row.usage_count,
      id: "usage_count",
      header: m.usage(),
      cell: (item) => {
        const count = item.value || 0;
        return `${count} ${count === 1 ? m.instance() : m.instances()}`;
      },
      plugins: {
        sort: {
          getSortValue: (v) => v || 0
        }
      }
    }),

    table.column({
      accessor: "created_at",
      header: m.created_date(),
      cell: (item) => {
        return dayjs(item.value).format("YYYY-MM-DD HH:mm");
      },
      plugins: {
        sort: {
          getSortValue(value) {
            return new Date(value).getTime();
          }
        }
      }
    }),

    table.columnActions({
      cell: (item) => {
        return Table.renderComponent(TemplateActions, { template: item.value, type: "app" });
      }
    })
  ]);

  $effect(() => {
    table.update(templates);
  });
</script>

<Table.Root {viewModel} resourceName={m.app_templates()} displayAs="list" />
