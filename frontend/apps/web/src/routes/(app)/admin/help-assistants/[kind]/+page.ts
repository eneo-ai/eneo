...
export const load: PageLoad = async (event) => {
  ...
  const role = await eneo.helpAssistants.admin.getRole({ kind: kind as "prompt_guide" });
  if (!role) {
    throw error(404);
  }

  const [assistant, orgSpace] = await Promise.all([
    eneo.assistants.get({ id: role.assistant_id }),
    eneo.spaces.getOrganizationSpace()
  ]);
  ...
  const promptGuideAvailability =
    kind === "prompt_guide"
      ? null
      : await eneo.helpAssistants.runs
