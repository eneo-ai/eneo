...
<script lang="ts">
  ...
  // Only from xl is the column sticky with its own scroll, and only then is
  // it a focusable region, so a keyboard can scroll it.
...
<section
  ...
  tabindex={wide.current ? 0 : undefined}
  class="border-default bg-primary flex min-w-0 flex-col gap-4 self-start rounded-xl border p-4 xl:sticky xl:top-4 xl:max-h-[calc(100dvh-8rem)] xl:overflow-auto"
>
  <div class="flex flex-col gap-1">
    <h2 id="review-preview-title" bind:this={heading} tabindex="-1" class="text-base font-semibold">
