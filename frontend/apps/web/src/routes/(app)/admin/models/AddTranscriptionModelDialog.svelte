...
<script lang="ts">
  ...
  $: availableModels = (capabilities?.providers[selectedProviderType]?.models?.transcription ?? []).map((m: CapabilityModel) => typeof m === "string" ? m : m.name ?? "") as string[];
      ...
      <form on:submit|preventDefault={handleSubmit} class="flex flex-col gap-4 p-4">
        ...
        {#if providerHasNoSupport}
          <div class="flex items-start gap-3 rounded-lg border border-label-default bg-label-dimmer px-4 py-3 text-sm label-warning">
            ...
            <div>
              <p class="font-medium text-label-stronger">{m.provider_no_support_title({ providerType: selectedProviderType, modelType: "transcription" })}</p>
