...
<script lang="ts" generics="T extends CompletionModel | EmbeddingModel | TranscriptionModel">
  ...
  function uniqueDetailParts(parts: Array<string | null | undefined>) {
    const seen: string[] = [];
  ...
  function modelDetails(model: SelectableModel<T>, groupLabel: string) {
    ...
    return uniqueDetailParts([providerName, providerType, underlyingName]).filter(
      (part) => normalize(part) !== normalize(groupLabel)
    );
  }
          ...
          <div class="flex min-w-0 items-center gap-2">
            ...
            <div class="min-w-0">
              ...
              <p class="text-muted text-xs tabular-nums">
                {enabledInGroup}/{group.models.length}
              </p>
            </div>
          </div>
          <span class="text-muted shrink-0 text-xs">{groupCountLabel(group.models.length)}</span>
        </div>
