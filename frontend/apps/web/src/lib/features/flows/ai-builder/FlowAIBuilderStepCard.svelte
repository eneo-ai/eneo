<script lang="ts">
  ...
  import { Badge, Card, Collapsible, Separator } from "@eneo/ui";
  ...
  <Collapsible.Root bind:open={showDetails}>
    <Card.Root
      class="group/step overflow-hidden transition-all duration-300 ease-out
        {changeKind === 'added'
          ? 'border-l-[3px] border-l-positive-default'
          : changeKind === 'modified'
            ? 'border-l-[3px] border-l-warning-default'
            : ''}
        {showDetails
          ? 'border-accent-default/25 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.1)] ring-1 ring-accent-default/10'
          : 'hover:border-stronger/60 hover:shadow-[0_4px_20px_-8px_rgba(0,0,0,0.08)]'}"
    >
      <Collapsible.Trigger class="w-full cursor-pointer text-left transition-colors hover:bg-hover-dimmer/30">
        <Card.Header class="gap-3 px-5 py-4">
          <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex shrink-0 items-center gap-2">
              {#if changeKind === "added"}
                <Badge variant="outline" class="border-positive-default/25 bg-positive-dimmer/60 text-positive-stronger">
                  {m.ai_builder_badge_new()}
                </Badge>
              {:else if changeKind === "modified"}
                <Badge variant="outline" class="border-warning-default/25 bg-warning-dimmer/60 text-warning-stronger">
                  {m.ai_builder_badge_modified()}
                </Badge>
              {/if}
              <div class="flex size-6 items-center justify-center rounded-md transition-colors group-hover/step:bg-hover-default">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 16 16"
                  fill="currentColor"
                  class="size-3.5 text-muted transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] {showDetails ? 'rotate-180' : ''}"
                >
          ...
          <div class="flex flex-wrap items-center gap-2 pl-11 text-xs">
            <span class="inline-flex items-center gap-1.5 rounded-md bg-secondary/40 px-2 py-0.5 text-muted">
      ...
      <Collapsible.Content>
        <Separator />
        <Card.Content class="flex flex-col gap-4 bg-secondary/20 px-5 py-4 pl-[3.5rem]">
          <!-- Instructions -->
          {#if step.assistant_spec.instructions}
            <div class="space-y-1.5">
              <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">
                {m.ai_builder_step_instructions()}
              </span>
              <div class="relative {!instructionsExpanded ? 'max-h-28 overflow-hidden' : ''}">
                <p class="max-w-[60ch] whitespace-pre-wrap text-[13px] leading-relaxed text-secondary">
                  {step.assistant_spec.instructions}
                </p>
                {#if !instructionsExpanded}
                  <div class="pointer-events-none absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-secondary/20 to-transparent"></div>
                {/if}
              </div>
              {#if step.assistant_spec.instructions.length > 300}
                <button
                  class="text-xs font-medium text-accent-default transition-colors hover:text-accent-stronger hover:underline"
                  onclick={(e) => { e.stopPropagation(); instructionsExpanded = !instructionsExpanded; }}
                >
          ...
          {#if step.assistant_spec.model_ref || step.assistant_spec.knowledge_refs.length > 0}
            <div class="grid gap-x-6 gap-y-2 sm:grid-cols-2">
              {#if step.assistant_spec.model_ref}
                <div>
                  <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">{m.ai_builder_step_model()}</span>
                  <p class="mt-0.5 text-[13px] text-secondary">{resolveModelName?.(step.assistant_spec.model_ref) ?? step.assistant_spec.model_ref}</p>
                </div>
              {/if}
              {#if step.assistant_spec.knowledge_refs.length > 0}
                <div>
                  <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">{m.ai_builder_step_knowledge()}</span>
                  <p class="mt-0.5 text-[13px] text-secondary">{step.assistant_spec.knowledge_refs.join(", ")}</p>
                </div>
              {/if}
            </div>
          {/if}
          ...
          {#if step.input_bindings}
            <div>
              <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">{m.ai_builder_step_bindings()}</span>
              <pre class="mt-1.5 overflow-x-auto rounded-lg border border-default/60 bg-primary px-3 py-2 font-mono text-xs leading-relaxed text-secondary">{JSON.stringify(step.input_bindings, null, 2)}</pre>
            </div>
          {/if}
          ...
          {#if step.output_contract?.properties}
            <div>
              <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">{m.ai_builder_step_output_contract()}</span>
              <div class="mt-1.5 space-y-0.5">
                {#each Object.entries(step.output_contract.properties) as [name, schema]}
                  <div class="flex items-baseline gap-2 text-[13px]">
                    <code class="font-semibold text-accent-default">{name}</code>
          ...
          {#if step.input_contract?.properties}
            <div>
              <span class="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted">{m.ai_builder_step_input_contract()}</span>
              <div class="mt-1.5 space-y-0.5">
          ...
          {#if step.mcp_policy === "restricted"}
            <div class="flex items-center gap-2 rounded-lg border border-warning-default/30 bg-warning-dimmer/50 px-3 py-2 text-xs text-warning-stronger">
          ...
          {#if planStatus === "proposed"}
            <button
              class="mt-1 inline-flex w-fit items-center gap-1.5 rounded-full border border-accent-default/15 bg-accent-default/5 px-3.5 py-1.5 text-xs font-medium text-accent-default transition-all duration-200 hover:border-accent-default/30 hover:bg-accent-default/10 hover:shadow-sm active:scale-[0.98]"
              onclick={(e) => { e.stopPropagation(); onsuggestchange?.(`${m.ai_builder_suggest_change_prefix()} '${step.name}': `); }}
            >
