<script lang="ts">
	...
	const showDraftPage = $derived(
		targetKind === "create" &&
		!service.hasSession &&
		service.recoverableCreateDrafts.length >= 2
	);
...
{:else}
	...
	{#if wasAutoResumed && service.hasSession && service.messages.length > 0}
		<div class="resume-banner">
			...
			<button class="resume-banner-link" onclick={() => { wasAutoResumed = false; service.startFreshSession("create"); }}>
				{m.ai_builder_resumed_start_fresh()}
			</button>
			<button class="resume-banner-dismiss" onclick={() => { wasAutoResumed = false; }} aria-label={m.ai_builder_dismiss()}>
