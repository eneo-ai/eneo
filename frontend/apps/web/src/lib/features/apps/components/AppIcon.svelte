...
>
  <div
    ...
    class:bg-[var(--dynamic-transparent)]={!iconUrl}
    class:font-mono={!iconUrl}
    class:font-bold={!iconUrl}
  >
    {#if iconUrl}
...
<style lang="postcss">
  ...
  .large div {
    @apply h-24 max-h-24 min-h-24 w-24 max-w-24 min-w-24 rounded-[1.5rem] text-[2.6rem];
  }
  .medium {
    @apply rounded-[0.9rem] border-b-4 p-1;
  }
  .medium div {
    @apply h-12 max-h-12 min-h-12 w-12 max-w-12 min-w-12 rounded-[0.7rem] text-[1.8rem];
  }
  .app-icon {
    box-shadow:
