...
describe("AttachmentUploadIconButton", () => {
  it("accepts the same file twice in a row", async () => {
    const intric = createIntricMock();
    const screen = render(AttachmentUploadIconButtonTestHost, { intric });
    ...
    selectFile();
    await vi.waitFor(() => expect(intric.files.upload).toHaveBeenCalledTimes(1));
    ...
    selectFile();
    await vi.waitFor(() => expect(intric.files.upload).toHaveBeenCalledTimes(2));
  });
});
