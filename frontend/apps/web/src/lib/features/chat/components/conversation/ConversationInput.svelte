<script lang="ts">
  ...
  $effect(() => {
    ...
    try {
      window.localStorage.setItem(TOKEN_USAGE_STORAGE_KEY, tokenUsageEnabled ? "true" : "false");
    } catch (error) {
  ...
  const modelInfo = $derived(
    (chat.partner as { model_info?: { max_input_tokens?: number | null } } | null)?.model_info ||
      null
  );
...
>
  ...
  {#if modelInfo && tokenLimit > 0}
    <div class="absolute top-2 right-2.5 z-10">
      <Tooltip
        text={tokenUsageEnabled ? m.hide_token_details() : m.show_token_details()}
        placement="top"
      >
        <button
          type="button"
  ...
  {#if modelInfo && tokenLimit > 0 && tokenUsageEnabled}
    <div transition:slide={{ duration: 350, easing: quintOut, axis: "y" }}>
