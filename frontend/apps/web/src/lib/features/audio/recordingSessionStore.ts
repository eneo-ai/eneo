// IndexedDB-backed segment ledger for the multi-segment recorder.
//
// Long recordings (3 h+) cannot afford to lose data on a tab refresh, so each
// finalized segment is persisted as it arrives and the upload step patches the
// record with the server-side file_id. When the browser refuses to durably
// store the Blob (older Safari profiles, Firefox under quota pressure), the
// store transparently degrades to memory-only and tells the consumer so it can
// surface a "no recovery if you refresh" warning.
