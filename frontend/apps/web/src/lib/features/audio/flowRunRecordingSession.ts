// Pure helpers for the multi-segment recording flow used by FlowRunDialog.
//
// Why a separate module: the dialog already owns a lot of wizard/upload
// state, so the per-step recording bookkeeping (session ids, segment
// counts, IDB persistence, resume reattach) is much easier to read and
// unit-test in isolation than as another 200 lines tucked inside a
// 1.4kloc Svelte file. Everything in here is either pure or limited to
// well-defined IO (recordingSessionStore + intric.files).
...
import { buildSegmentFilenameBase, generateSessionId } from "./recordingSession";
import {
...
// Map MediaRecorder mime types to the file extension Whisper accepts.
// Strips codec parameters (`audio/webm;codecs=opus` → `audio/webm`)
// before matching so chrome and firefox normalise identically.
