...
<div class="space-y-1" role="group" aria-label={m.mcp_servers()}>
  ...
  {:else}
    {@const knowledgeSources = availableServers.filter((s) => s.is_knowledge_source ?? false)}
    {@const toolServers = availableServers.filter((s) => !(s.is_knowledge_source ?? false))}

    {#snippet serverRow(server: MCPServer)}
    ...
    {#snippet serverGroup(label: string, group: MCPServer[])}
      {#if group.length > 0}
        <div>
          <h3 class="text-muted mb-1.5 px-1 text-[11px] font-medium tracking-wider uppercase">
            {label}
          </h3>
