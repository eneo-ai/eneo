<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      await intric.credentials.set(provider.toLowerCase(), formData);
      ...
      const err = error as { status?: number; response?: { detail?: { errors?: unknown } } };
      if (err.status === 422 && err.response?.detail?.errors) {
        const validationErrors = err.response.detail.errors;
...
<Dialog.Root {openController}>
  <Dialog.Content width="dynamic">
    <Dialog.Title
      >{m.configure_provider_credentials({ provider: displayProviderName })}</Dialog.Title
    >
      ...
      <form on:submit|preventDefault={handleSubmit} class="space-y-4">
        ...
        <div class="rounded-md bg-gray-50 p-3 text-xs text-gray-600">
          <p>
            ...
            {m.credentials_encrypted_notice()}
          </p>
        </div>
      </form>
    </Dialog.Section>
