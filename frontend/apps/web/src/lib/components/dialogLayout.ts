...
export const dialogLayout = {
  content: (width: DialogWidth = "small", className?: string) =>
    cn(
      "flex max-h-[calc(100dvh-2rem)] max-w-[calc(100%-2rem)] flex-col gap-0 overflow-hidden p-0 data-[size=default]:max-w-[calc(100%-2rem)] sm:max-h-[85dvh]",
      widths[width],
      className
    ),
  header: "shrink-0 gap-1.5 border-b px-6 py-4 pr-12",
  body: "flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-6 py-5",
